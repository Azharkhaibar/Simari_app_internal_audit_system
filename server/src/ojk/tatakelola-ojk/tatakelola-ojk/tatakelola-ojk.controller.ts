...
export class TatakelolaController {
  constructor(private readonly tatakelolaService: TatakelolaService) {}

  // === CRUD UTAMA ===

  @Get()
  @ApiOperation({ summary: 'Get all Tatakelola data or by year/quarter' })
  ...
  async findAll(
    ...
    if (year && quarter) {
      ...
      const result = await this.tatakelolaService.findByYearQuarter(
        yearNum,
        quarterNum,
      );
      ...
      return {
        ...
        message: result
          ? `Data tatakelola ditemukan untuk ${yearNum} Q${quarterNum}`
          : `Data tatakelola belum tersedia untuk ${yearNum} Q${quarterNum}`,
      };
    }

    // Jika tidak ada parameter, return semua data
    const allData = await this.tatakelolaService.getAll();
    return {
      success: true,
      data: allData,
  ...
  @Get('active')
  @ApiOperation({ summary: 'Get active Tatakelola data' })
  @ApiResponse({
    status: 200,
  ...
  async getActive() {
    const result = await this.tatakelolaService.findActive();
    if (!result) {
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get Tatakelola by ID' })
  @ApiParam({ name: 'id', type: Number })
  @ApiResponse({ status: 200, description: 'Data retrieved successfully' })
  @ApiResponse({ status: 404, description: 'Data not found' })
  async findOne(@Param('id', ParseIntPipe) id: number) {
    const result = await this.tatakelolaService.findById(id);
    if (!result) {
  ...
  @Post()
  @ApiOperation({ summary: 'Create new Tatakelola data' })
  @ApiBody({ type: CreateTatakelolaDto })
  @ApiResponse({ status: 201, description: 'Data created successfully' })
  @ApiResponse({ status: 400, description: 'Bad request' })
  async create(@Body() createDto: CreateTatakelolaDto, @Request() req) {
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.create(createDto, userId);
  }

  @Put(':id')
  @ApiOperation({ summary: 'Update Tatakelola data' })
  @ApiParam({ name: 'id', type: Number })
  @ApiBody({ type: UpdateTatakelolaDto })
  @ApiResponse({ status: 200, description: 'Data updated successfully' })
  @ApiResponse({ status: 404, description: 'Data not found' })
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() updateDto: UpdateTatakelolaDto,
    @Request() req,
  ) {
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.update(id, updateDto, userId);
  }
  ...
  async updateSummary(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.updateSummary(id, summaryDto, userId);
  }
  ...
  async updateActiveStatus(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.updateActiveStatus(id, isActive, userId);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Delete Tatakelola data' })
  @ApiParam({ name: 'id', type: Number })
  @ApiResponse({ status: 200, description: 'Data deleted successfully' })
  @ApiResponse({ status: 404, description: 'Data not found' })
  async remove(@Param('id', ParseIntPipe) id: number) {
    return this.tatakelolaService.remove(id);
  }

  // === OPERASI PARAMETER ===

  @Get(':id/parameters')
  @ApiOperation({ summary: 'Get all parameters for specific Tatakelola' })
  @ApiParam({ name: 'id', type: Number })
  @ApiResponse({
    status: 200,
    description: 'Parameters retrieved successfully',
  })
  async getParameters(@Param('id', ParseIntPipe) tatakelolaId: number) {
    const tatakelola = await this.getTatakelolaByIdOrThrow(tatakelolaId);
    return tatakelola.parameters || [];
  }
  ...
  @ApiResponse({ status: 404, description: 'Data not found' })
  async addParameter(
    @Param('id', ParseIntPipe) tatakelolaId: number,
    @Body() createParamDto: CreateParameterDto,
    @Request() req,
  ) {
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.addParameter(
      tatakelolaId,
      createParamDto,
      userId,
    );
  ...
  async updateParameter(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.updateParameter(
      tatakelolaId,
      parameterId,
      updateParamDto,
      userId,
  ...
  async reorderParameters(
    ...
    return this.tatakelolaService.reorderParameters(tatakelolaId, reorderDto);
  }
  ...
  async copyParameter(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.copyParameter(
      tatakelolaId,
      parameterId,
      userId,
    );
  ...
  async removeParameter(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.removeParameter(
      tatakelolaId,
      parameterId,
      userId,
    );
  ...
  async getNilai(
    ...
    const parameter = tatakelola.parameters?.find((p) => p.id === parameterId);
    if (!parameter) {
  ...
  async addNilai(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.addNilai(
      tatakelolaId,
      parameterId,
      createNilaiDto,
      userId,
  ...
  async updateNilai(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.updateNilai(
      tatakelolaId,
      parameterId,
      nilaiId,
      updateNilaiDto,
  ...
  async reorderNilai(
    ...
    return this.tatakelolaService.reorderNilai(parameterId, reorderDto);
  }
  ...
  async copyNilai(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.copyNilai(
      tatakelolaId,
      parameterId,
      nilaiId,
      userId,
  ...
  async removeNilai(
    ...
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.removeNilai(
      tatakelolaId,
      parameterId,
      nilaiId,
      userId,
  ...
  @ApiResponse({ status: 404, description: 'Data not found' })
  async exportToExcel(@Param('id', ParseIntPipe) tatakelolaId: number) {
    return this.tatakelolaService.exportToExcel(tatakelolaId);
  }
  ...
  async importFromExcel(@Body() importData: ImportExportDto, @Request() req) {
    const userId = req.user?.id || 'system';
    return this.tatakelolaService.importFromExcel(importData, userId);
  }
  ...
  async getReferences(@Query('type') type?: string) {
    return this.tatakelolaService.getReferences(type);
  }
  ...
  async validateModelTerstruktur(
    ...
    return this.tatakelolaService.validateModelTerstruktur(tatakelolaId);
  }
  ...
  async checkExists(
    ...
    const exists = await this.tatakelolaService.findByYearQuarter(
      year,
      quarter,
    );
  ...
  private async getTatakelolaByIdOrThrow(tatakelolaId: number) {
    const tatakelola = await this.tatakelolaService.findById(tatakelolaId);
