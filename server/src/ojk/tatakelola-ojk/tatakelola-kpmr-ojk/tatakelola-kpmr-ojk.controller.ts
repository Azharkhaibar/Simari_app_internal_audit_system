// tatakelola-kpmr-ojk.controller.ts
import {
  Controller,
  Get,
  Post,
...
@ApiTags('TatakelolaKpmr')
@Controller('kpmr-tatakelola')
@UseInterceptors(ClassSerializerInterceptor)
export class KpmrTatakelolaController {
  constructor(private readonly kpmrService: TatakelolaKpmrService) {}

  // ========== KPMR ENDPOINTS ==========
  @Post()
  @ApiOperation({ summary: 'Buat KPMR baru' })
  async create(
    @Body() createDto: CreateKpmrTatakelolaOjkDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrTatakelolaOjkDto,
  ): Promise<FrontendKpmrResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createAspek(
    ...
    @Body() createDto: CreateKpmrAspekTatakelolaDto,
  ...
  async updateAspek(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrAspekTatakelolaDto,
  ): Promise<FrontendAspekResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createPertanyaan(
    ...
    @Body() createDto: CreateKpmrPertanyaanTatakelolaDto,
  ...
  async updatePertanyaan(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPertanyaanTatakelolaDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
