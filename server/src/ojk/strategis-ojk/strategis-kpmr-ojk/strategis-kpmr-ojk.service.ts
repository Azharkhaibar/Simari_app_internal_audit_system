...
export class KpmrStrategisService {
  ...
  async createKpmr(
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Strategis ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanStrategis, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekStrategis, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrStrategisOjk, { id });
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekStrategis, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanStrategis, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
