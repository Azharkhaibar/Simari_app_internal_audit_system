...
export class StrategisService {
  ...
  constructor(
    ...
    private nilaiRepository: Repository<StrategisNilai>,
    @InjectRepository(StrategisReference)
    private referenceRepository: Repository<StrategisReference>,
  ...
  async update(id: number, updateDto: UpdateStrategisDto, userId: string) {
    ...
    if (updateDto.summary !== undefined) strategis.summary = updateDto.summary;
  ...
  async exportToExcel(strategisId: number) {
    ...
    return {
      metadata: {
        ...
        totalNilai:
          strategis.parameters?.reduce(
            (total, param) => total + (param.nilaiList?.length || 0),
            0,
          ) || 0,
      },
      parameters:
        strategis.parameters?.map((param) => ({
          id: param.id,
          nomor: param.nomor,
          judul: param.judul,
