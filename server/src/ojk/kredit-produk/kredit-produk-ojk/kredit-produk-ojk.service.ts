...
export class KreditService {
  private readonly logger = new Logger(KreditService.name);

  constructor(
    @InjectRepository(Kredit)
    private kreditRepository: Repository<Kredit>,
    ...
    @InjectRepository(KreditReference)
    private referenceRepository: Repository<KreditReference>,
  ...
  // RECALCULATE SUMMARY
  // ============================================
  private async recalculateSummary(kreditId: number): Promise<void> {
    this.logger.log(
      `📊 Recalculating summary for Kredit ID: ${kreditId}`,
    );
    try {
      ...
      if (!kredit) {
        this.logger.warn(`⚠️ Kredit with ID ${kreditId} not found`);
        return;
      }

      let totalWeighted = 0;
      if (kredit.parameters && kredit.parameters.length > 0) {
        for (const param of kredit.parameters) {
      ...
      await this.kreditRepository.save(kredit);
      this.logger.log(
        `✅ Summary recalculated for Kredit ID ${kreditId}: totalWeighted=${totalWeighted.toFixed(2)}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Error recalculating summary for Kredit ${kreditId}: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async create(createDto: CreateKreditDto, userId: string) {
    try {
      ...
      if (existing) {
        ...
        if (!existing.isActive) {
          ...
          await this.kreditRepository.save(existing);
        }
        return existing;
      }
      ...
      this.logger.error(
        `Error creating Kredit: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findActive(): Promise<Kredit | null> {
  ...
  async findByYearQuarter(
    ...
    try {
      ...
      this.logger.log(
        `findByYearQuarter: Data ditemukan - ID: ${kredit.id}`,
      );
      return kredit;
    } catch (error) {
      this.logger.error(
  ...
  async findById(id: number): Promise<Kredit | null> {
  ...
  async update(id: number, updateDto: UpdateKreditDto, userId: string) {
  ...
  async updateSummary(
    ...
    kredit.updatedBy = userId;
    return this.kreditRepository.save(kredit);
  }
  ...
  async remove(id: number) {
    ...
    try {
      ...
      await queryRunner.manager.delete(Kredit, { id });
  ...
  async addParameter(
    ...
    if (!kredit)
      throw new NotFoundException(
        `Data dengan ID ${kreditId} tidak ditemukan`,
      );
    ...
    try {
      ...
      await this.recalculateSummary(kreditId);
    ...
    try {
      for (let i = 0; i < reorderDto.parameterIds.length; i++)
        await queryRunner.manager.update(
          KreditParameter,
          { id: reorderDto.parameterIds[i], kreditId },
          { orderIndex: i },
        );
      await queryRunner.commitTransaction();
    ...
    try {
      ...
      if (originalParam.nilaiList?.length)
        for (const nilai of originalParam.nilaiList) {
          ...
          await queryRunner.manager.save(KreditNilai, {
            ...nilaiWithoutId,
  ...
  async removeParameter(
    ...
    try {
      if (parameter.nilaiList?.length)
        await queryRunner.manager.delete(KreditNilai, { parameterId });
      await queryRunner.manager.delete(KreditParameter, {
        id: parameterId,
      });
      await queryRunner.commitTransaction();
  ...
  async updateNilai(
    ...
    if (nilai.parameter.kreditId !== kreditId)
      throw new BadRequestException('Nilai tidak termasuk dalam kredit');
  ...
  async validateModelTerstruktur(
    ...
    if (!kredit) {
      result.errors.push(`Data ID ${kreditId} tidak ditemukan`);
  ...
  async exportToExcel(kreditId: number) {
    ...
    if (!kredit)
      throw new NotFoundException(`Data ID ${kreditId} tidak ditemukan`);
  ...
  async importFromExcel(importData: any, userId: string) {
    ...
    try {
      ...
      await queryRunner.commitTransaction();
      await this.recalculateSummary(savedKredit.id);
      return savedKredit;
    } catch (error) {
      await queryRunner.rollbackTransaction();
      throw error;
