...
@Injectable()
export class KpmrKreditProdukService {
  private readonly logger = new Logger(KpmrKreditProdukService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrKreditOjk)
    private readonly kpmrRepository: Repository<KpmrKreditOjk>,

    @InjectRepository(KpmrAspekKredit)
    private readonly aspekRepository: Repository<KpmrAspekKredit>,

    @InjectRepository(KpmrPertanyaanKredit)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanKredit>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrKreditOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrKreditOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekKredit> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private checkKpmrLocked(kpmr: KpmrKreditOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekKredit, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanKredit, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrKreditOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Kredit Produk ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrKreditOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrKreditOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanKredit, {
              aspekId: aspek.id,
            });
          }
        }

        await queryRunner.manager.delete(KpmrAspekKredit, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrKreditOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekKreditDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekKreditDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekKredit, reorderDto.aspekIds[i], {
            orderIndex: i,
          });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanKreditDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanKreditDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanKredit)
          .findOne({
  ...
  async getKpmrStatistics(id: number): Promise<any> {
    ...
    return {
      ...
      averageScore: Number(
        (scoreCount > 0 ? totalScore / scoreCount : 0).toFixed(2),
      ),
