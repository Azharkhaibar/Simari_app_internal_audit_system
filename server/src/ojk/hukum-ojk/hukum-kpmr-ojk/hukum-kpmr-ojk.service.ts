...
export class KpmrHukumService {
  ...
  constructor(
    ...
    @InjectRepository(KpmrPertanyaanHukum)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanHukum>,
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekHukum> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async validateTotalBobot(
    ...
    const query = this.aspekRepository
      ...
      .where('aspek.kpmrId = :kpmrId', { kpmrId });
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekHukum, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanHukum, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekHukumDto,
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanHukumDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanHukumDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    this.logger.log(`📝 Update pertanyaan ID: ${id}`);

    const pertanyaan = await this.getPertanyaanWithRelations(id);
    this.checkKpmrLocked(pertanyaan.aspek.kpmr, 'mengupdate pertanyaan');
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanHukum)
          .findOne({
  ...
  async reorderPertanyaan(aspekId: number, reorderDto: ReorderPertanyaanDto): Promise<void> {
    ...
    this.checkKpmrLocked(aspek.kpmr, 'mengubah urutan pertanyaan');
