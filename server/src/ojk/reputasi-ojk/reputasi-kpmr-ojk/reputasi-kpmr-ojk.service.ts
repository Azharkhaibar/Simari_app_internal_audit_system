...
export class KpmrReputasiService {
  ...
  constructor(
    ...
    private readonly kpmrRepository: Repository<KpmrReputasiOjk>,

    @InjectRepository(KpmrAspekReputasi)
    private readonly aspekRepository: Repository<KpmrAspekReputasi>,

    @InjectRepository(KpmrPertanyaanReputasi)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanReputasi>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrReputasiOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrReputasiOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekReputasi> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanReputasi> {
  ...
  private checkKpmrLocked(kpmr: KpmrReputasiOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekReputasi, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanReputasi, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrReputasiOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Reputasi ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrReputasiOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrReputasiOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanReputasi, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekReputasi, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrReputasiOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekReputasiDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekReputasiDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekReputasi, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanReputasiDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanReputasiDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanReputasi)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanReputasi, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
