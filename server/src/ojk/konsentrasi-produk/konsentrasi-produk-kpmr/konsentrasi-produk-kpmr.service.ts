...
@Injectable()
export class KonsentrasiKpmrService {
  private readonly logger = new Logger(KonsentrasiKpmrService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrKonsentrasiOjk)
    private readonly kpmrRepository: Repository<KpmrKonsentrasiOjk>,

    @InjectRepository(KpmrAspekKonsentrasi)
    private readonly aspekRepository: Repository<KpmrAspekKonsentrasi>,

    @InjectRepository(KpmrPertanyaanKonsentrasi)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanKonsentrasi>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrKonsentrasiOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrKonsentrasiOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekKonsentrasi> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanKonsentrasi> {
  ...
  private checkKpmrLocked(kpmr: KpmrKonsentrasiOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekKonsentrasi, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanKonsentrasi, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrKonsentrasiOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Konsentrasi Produk ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrKonsentrasiOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrKonsentrasiOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanKonsentrasi, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekKonsentrasi, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrKonsentrasiOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekKonsentrasiDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekKonsentrasiDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekKonsentrasi, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanKonsentrasiDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanKonsentrasiDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanKonsentrasi)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanKonsentrasi, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
