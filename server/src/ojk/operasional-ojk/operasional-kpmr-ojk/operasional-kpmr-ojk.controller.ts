...
@ApiTags('OperasionalKpmr')
@Controller('kpmr-operasional')
@UseInterceptors(ClassSerializerInterceptor)
export class KpmrOperasionalController {
  constructor(private readonly kpmrService: KpmrOperasionalService) {}

  // ========== KPMR ENDPOINTS ==========
  @Post()
  @ApiOperation({ summary: 'Buat KPMR baru' })
  async create(
    @Body() createDto: CreateKpmrOperasionalOjkDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrOperasionalOjkDto,
  ): Promise<FrontendKpmrResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createAspek(
    ...
    @Body() createDto: CreateKpmrAspekOperasionalDto,
  ...
  async updateAspek(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrAspekOperasionalDto,
  ): Promise<FrontendAspekResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createPertanyaan(
    ...
    @Body() createDto: CreateKpmrPertanyaanOperasionalDto,
  ...
  async updatePertanyaan(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPertanyaanOperasionalDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
