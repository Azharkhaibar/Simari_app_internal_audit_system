...
import { KpmrAspekInvestasi } from './entities/investasi-kpmr-aspek.entity';
...
export class KpmrInvestasiService {
  ...
  constructor(
    ...
    private readonly kpmrRepository: Repository<KpmrInvestasiOjk>,

    @InjectRepository(KpmrAspekInvestasi)
    private readonly aspekRepository: Repository<KpmrAspekInvestasi>,

    @InjectRepository(KpmrPertanyaanInvestasi)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanInvestasi>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrInvestasiOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrInvestasiOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekInvestasi> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanInvestasi> {
  ...
  private checkKpmrLocked(kpmr: KpmrInvestasiOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekInvestasi, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanInvestasi, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrInvestasiOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Investasi ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrInvestasiOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrInvestasiOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanInvestasi, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekInvestasi, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrInvestasiOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekInvestasiDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekInvestasiDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekInvestasi, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanInvestasiDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanInvestasiDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanInvestasi)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanInvestasi, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
