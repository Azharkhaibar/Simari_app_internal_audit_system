...
@ApiTags('InvestasiKpmr')
@Controller('kpmr-investasi')
@UseInterceptors(ClassSerializerInterceptor)
export class KpmrInvestasiController {
  constructor(private readonly kpmrService: KpmrInvestasiService) {}

  // ========== KPMR ENDPOINTS ==========
  @Post()
  @ApiOperation({ summary: 'Buat KPMR baru' })
  async create(
    @Body() createDto: CreateKpmrInvestasiOjkDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrInvestasiOjkDto,
  ): Promise<FrontendKpmrResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createAspek(
    ...
    @Body() createDto: CreateKpmrAspekInvestasiDto,
  ...
  async updateAspek(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrAspekInvestasiDto,
  ): Promise<FrontendAspekResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createPertanyaan(
    ...
    @Body() createDto: CreateKpmrPertanyaanInvestasiDto,
  ...
  async updatePertanyaan(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPertanyaanInvestasiDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
