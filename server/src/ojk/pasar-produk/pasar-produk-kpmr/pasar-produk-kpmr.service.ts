...
import { KpmrAspekPasarProduk } from './entities/pasar-produk-kpmr-aspek.entity';

@Injectable()
export class KpmrPasarProdukService {
  private readonly logger = new Logger(KpmrPasarProdukService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrPasarProdukOjk)
    private readonly kpmrRepository: Repository<KpmrPasarProdukOjk>,

    @InjectRepository(KpmrAspekPasarProduk)
    private readonly aspekRepository: Repository<KpmrAspekPasarProduk>,

    @InjectRepository(KpmrPertanyaanPasarProduk)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanPasarProduk>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrPasarProdukOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrPasarProdukOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekPasarProduk> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanPasarProduk> {
  ...
  private checkKpmrLocked(kpmr: KpmrPasarProdukOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekPasarProduk, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanPasarProduk, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrPasarProdukOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Pasar Produk ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  // findAll, findOne, findOneEntity, findByYearQuarter, updateKpmr, deleteKpmr,
  // getActiveKpmr, lockKpmr, unlockKpmr, duplicateKpmr,
  // createAspek, updateAspek, deleteAspek, reorderAspek,
  // createPertanyaan, updatePertanyaan, updateSkor, bulkUpdateSkor,
  // deletePertanyaan, reorderPertanyaan,
  // getSummary, updateSummary,
  // findAllAspek, findOneAspek, getKpmrStatistics, validateKpmrData
  // SEMUA METHOD LAINNYA TETAP SAMA IDENTIK
  ...
  async findOneEntity(id: number): Promise<KpmrPasarProdukOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrPasarProdukOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanPasarProduk, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekPasarProduk, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrPasarProdukOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekPasarProdukDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekPasarProdukDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekPasarProduk, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanPasarProdukDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanPasarProdukDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanPasarProduk)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanPasarProduk, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
