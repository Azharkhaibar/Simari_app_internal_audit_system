...
export class KpmrKepatuhanService {
  private readonly logger = new Logger(KpmrKepatuhanService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrKepatuhanOjk)
    private readonly kpmrRepository: Repository<KpmrKepatuhanOjk>,

    @InjectRepository(KpmrAspekKepatuhan)
    private readonly aspekRepository: Repository<KpmrAspekKepatuhan>,

    @InjectRepository(KpmrPertanyaanKepatuhan)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanKepatuhan>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrKepatuhanOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrKepatuhanOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekKepatuhan> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanKepatuhan> {
  ...
  private checkKpmrLocked(kpmr: KpmrKepatuhanOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekKepatuhan, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanKepatuhan, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrKepatuhanOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Kepatuhan ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrKepatuhanOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrKepatuhanOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanKepatuhan, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekKepatuhan, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrKepatuhanOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekKepatuhanDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekKepatuhanDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekKepatuhan, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanKepatuhanDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanKepatuhanDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanKepatuhan)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanKepatuhan, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
