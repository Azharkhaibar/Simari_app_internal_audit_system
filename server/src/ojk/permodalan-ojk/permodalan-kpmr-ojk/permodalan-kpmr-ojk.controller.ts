...
@ApiTags('PermodalanKpmr')
@Controller('kpmr-permodalan')
@UseInterceptors(ClassSerializerInterceptor)
export class KpmrPermodalanController {
  constructor(private readonly kpmrService: KpmrPermodalanService) {}

  // ========== KPMR ENDPOINTS ==========
  @Post()
  @ApiOperation({ summary: 'Buat KPMR baru' })
  async create(
    @Body() createDto: CreateKpmrPermodalanOjkDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPermodalanOjkDto,
  ): Promise<FrontendKpmrResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createAspek(
    ...
    @Body() createDto: CreateKpmrAspekPermodalanDto,
  ...
  async updateAspek(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrAspekPermodalanDto,
  ): Promise<FrontendAspekResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createPertanyaan(
    ...
    @Body() createDto: CreateKpmrPertanyaanPermodalanDto,
  ...
  async updatePertanyaan(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPertanyaanPermodalanDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
