...
import { KpmrAspekPermodalan } from './entities/permodalan-kpmr-aspek.entity';

@Injectable()
export class KpmrPermodalanService {
  private readonly logger = new Logger(KpmrPermodalanService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrPermodalanOjk)
    private readonly kpmrRepository: Repository<KpmrPermodalanOjk>,

    @InjectRepository(KpmrAspekPermodalan)
    private readonly aspekRepository: Repository<KpmrAspekPermodalan>,

    @InjectRepository(KpmrPertanyaanPermodalan)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanPermodalan>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrPermodalanOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrPermodalanOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekPermodalan> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanPermodalan> {
  ...
  private checkKpmrLocked(kpmr: KpmrPermodalanOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekPermodalan, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanPermodalan, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrPermodalanOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Permodalan ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrPermodalanOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrPermodalanOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanPermodalan, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekPermodalan, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrPermodalanOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekPermodalanDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekPermodalanDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekPermodalan, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanPermodalanDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanPermodalanDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanPermodalan)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanPermodalan, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
