...
export class PermodalanService {
  private readonly logger = new Logger(PermodalanService.name);

  constructor(
    @InjectRepository(Permodalan)
    private permodalanRepository: Repository<Permodalan>,
    @InjectRepository(PermodalanParameter)
    private parameterRepository: Repository<PermodalanParameter>,
    @InjectRepository(PermodalanNilai)
    private nilaiRepository: Repository<PermodalanNilai>,
    @InjectRepository(PermodalanReference)
    private referenceRepository: Repository<PermodalanReference>,
  ...
  // ============================================
  private async recalculateSummary(permodalanId: number): Promise<void> {
    this.logger.log(
      `📊 Recalculating summary for Permodalan ID: ${permodalanId}`,
    );
    try {
      const permodalan = await this.permodalanRepository.findOne({
        where: { id: permodalanId },
        relations: ['parameters', 'parameters.nilaiList'],
      });
      if (!permodalan) {
        this.logger.warn(`⚠️ Permodalan with ID ${permodalanId} not found`);
        return;
      }

      let totalWeighted = 0;
      if (permodalan.parameters && permodalan.parameters.length > 0) {
        for (const param of permodalan.parameters) {
      ...
      await this.permodalanRepository.save(permodalan);
      this.logger.log(
        `✅ Summary recalculated for Permodalan ID ${permodalanId}: totalWeighted=${totalWeighted.toFixed(2)}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Error recalculating summary for Permodalan ${permodalanId}: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  // ============================================
  // CRUD UTAMA (Permodalan)
  // ============================================

  async create(createDto: CreatePermodalanDto, userId: string) {
    try {
      const existing = await this.permodalanRepository.findOne({
      ...
      if (existing) {
        ...
        if (!existing.isActive) {
          ...
          await this.permodalanRepository.save(existing);
        }
        return existing;
      }

      const permodalan = this.permodalanRepository.create({
        year: createDto.year,
      ...
      const saved = await this.permodalanRepository.save(permodalan);
      ...
      this.logger.error(
        `Error creating Permodalan: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findActive(): Promise<Permodalan | null> {
    ...
    try {
      ...
      this.logger.log(`findActive: Data ditemukan - ID: ${permodalan.id}`);
      return permodalan;
    } catch (error) {
  ...
  async findByYearQuarter(
    ...
    try {
      ...
      this.logger.log(
        `findByYearQuarter: Data ditemukan - ID: ${permodalan.id}`,
      );
      return permodalan;
    } catch (error) {
      this.logger.error(
  ...
  async findById(id: number): Promise<Permodalan | null> {
  ...
  async getAll() {
    ...
    return this.permodalanRepository.find({
      relations: ['parameters'],
      order: { year: 'DESC', quarter: 'DESC' },
    });
  }

  async update(id: number, updateDto: UpdatePermodalanDto, userId: string) {
    ...
    if (updateDto.year !== undefined) permodalan.year = updateDto.year;
    if (updateDto.quarter !== undefined)
      permodalan.quarter = updateDto.quarter;
    if (updateDto.isActive !== undefined)
      permodalan.isActive = updateDto.isActive;
    if (updateDto.summary !== undefined)
      permodalan.summary = updateDto.summary;
    if (updateDto.isLocked !== undefined)
      permodalan.isLocked = updateDto.isLocked;
    if (updateDto.lockedBy !== undefined)
      permodalan.lockedBy = updateDto.lockedBy;
    if (updateDto.lockedAt !== undefined)
      permodalan.lockedAt = updateDto.lockedAt;
    if (updateDto.notes !== undefined) permodalan.notes = updateDto.notes;
    permodalan.updatedBy = userId;

    const result = await this.permodalanRepository.save(permodalan);
  ...
  async updateSummary(
    ...
    permodalan.updatedBy = userId;
    return this.permodalanRepository.save(permodalan);
  }
  ...
  async updateActiveStatus(id: number, isActive: boolean, userId: string) {
    ...
    if (!permodalan)
      throw new NotFoundException(`Data dengan ID ${id} tidak ditemukan`);
    if (isActive) {
      await this.permodalanRepository
        .createQueryBuilder()
        .update(Permodalan)
        .set({ isActive: false })
        .execute();
    }
    permodalan.isActive = isActive;
    permodalan.updatedBy = userId;
    return this.permodalanRepository.save(permodalan);
  }
  ...
  async remove(id: number) {
    ...
    try {
      for (const parameter of permodalan.parameters || []) {
        await queryRunner.manager.delete(PermodalanNilai, {
          parameterId: parameter.id,
        });
      }
      await queryRunner.manager.delete(PermodalanParameter, {
        permodalanId: id,
      });
      await queryRunner.manager.delete(Permodalan, { id });
  ...
  async addParameter(
    ...
    this.logger.log(
      `addParameter: Menambahkan parameter - Permodalan ID: ${permodalanId}`,
    );
    const permodalan = await this.permodalanRepository.findOne({
      where: { id: permodalanId },
    });
    ...
    try {
      ...
      await this.recalculateSummary(permodalanId);
      await this.permodalanRepository.update(permodalanId, {
        updatedBy: userId,
        updatedAt: new Date(),
      });
      return savedParam;
  ...
  async updateParameter(
    ...
    const parameter = await this.parameterRepository.findOne({
      where: { id: parameterId, permodalanId },
    });
    if (!parameter)
      throw new NotFoundException(
    ...
    try {
      for (let i = 0; i < reorderDto.parameterIds.length; i++)
        await queryRunner.manager.update(
          PermodalanParameter,
          { id: reorderDto.parameterIds[i], permodalanId },
          { orderIndex: i },
        );
      await queryRunner.commitTransaction();
    ...
    try {
      ...
      if (originalParam.nilaiList?.length)
        for (const nilai of originalParam.nilaiList) {
          ...
          await queryRunner.manager.save(PermodalanNilai, {
            ...nilaiWithoutId,
  ...
  async removeParameter(
    ...
    try {
      if (parameter.nilaiList?.length)
        await queryRunner.manager.delete(PermodalanNilai, { parameterId });
      await queryRunner.manager.delete(PermodalanParameter, {
        id: parameterId,
      });
      await queryRunner.commitTransaction();
  ...
  async updateNilai(
    ...
    if (nilai.parameter.permodalanId !== permodalanId)
      throw new BadRequestException('Nilai tidak termasuk dalam permodalan');
  ...
  async validateModelTerstruktur(
    ...
    if (!permodalan) {
      result.errors.push(`Data ID ${permodalanId} tidak ditemukan`);
      result.isValid = false;
      return result;
    }
    const terstrukturParams =
      permodalan.parameters?.filter(
  ...
  async exportToExcel(permodalanId: number) {
    const permodalan = await this.permodalanRepository.findOne({
      where: { id: permodalanId },
      relations: ['parameters', 'parameters.nilaiList'],
      order: {
    ...
    if (!permodalan)
      throw new NotFoundException(`Data ID ${permodalanId} tidak ditemukan`);

    return {
      metadata: {
        year: permodalan.year,
        quarter: permodalan.quarter,
        exportedAt: new Date().toISOString(),
        totalParameters: permodalan.parameters?.length || 0,
        totalNilai:
          permodalan.parameters?.reduce(
            (total, param) => total + (param.nilaiList?.length || 0),
            0,
          ) || 0,
      },
      parameters:
        permodalan.parameters?.map((param) => ({
          id: param.id,
          nomor: param.nomor,
          judul: param.judul,
  ...
  async importFromExcel(importData: any, userId: string) {
    ...
    try {
      ...
      const savedPermodalan = await queryRunner.manager.save(
        Permodalan,
        permodalan,
      );
      ...
      for (let i = 0; i < importData.parameters.length; i++) {
        ...
        const parameter = {
          ...
          permodalanId: savedPermodalan.id,
      ...
      await queryRunner.commitTransaction();
      await this.recalculateSummary(savedPermodalan.id);
      return savedPermodalan;
    } catch (error) {
      await queryRunner.rollbackTransaction();
      throw error;
    } finally {
