...
import { KpmrAspekLikuiditasProduk } from './entities/likuiditas-kpmr-aspek.entity';
@Injectable()
export class KpmrLikuiditasProdukService {
  private readonly logger = new Logger(KpmrLikuiditasProdukService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrLikuiditasProdukOjk)
    private readonly kpmrRepository: Repository<KpmrLikuiditasProdukOjk>,

    @InjectRepository(KpmrAspekLikuiditasProduk)
    private readonly aspekRepository: Repository<KpmrAspekLikuiditasProduk>,

    @InjectRepository(KpmrPertanyaanLikuiditasProduk)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanLikuiditasProduk>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrLikuiditasProdukOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrLikuiditasProdukOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekWithRelations(
    id: number,
  ): Promise<KpmrAspekLikuiditasProduk> {
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekLikuiditasProduk> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanLikuiditasProduk> {
  ...
  private checkKpmrLocked(kpmr: KpmrLikuiditasProdukOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekLikuiditasProduk, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanLikuiditasProduk, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrLikuiditasOjkDto,
  ...
  async findOne(
    ...
    let kpmr: KpmrLikuiditasProdukOjk;
  ...
  async findOneEntity(id: number): Promise<KpmrLikuiditasProdukOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrLikuiditasOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanLikuiditasProduk, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekLikuiditasProduk, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrLikuiditasProdukOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekLikuiditasDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekLikuiditasDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekLikuiditasProduk, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanLikuiditasDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanLikuiditasDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanLikuiditasProduk)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanLikuiditasProduk, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
