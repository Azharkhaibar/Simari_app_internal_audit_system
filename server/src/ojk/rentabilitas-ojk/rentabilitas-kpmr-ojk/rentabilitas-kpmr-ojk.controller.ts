...
@ApiTags('RentabilitasKpmr')
@Controller('kpmr-rentabilitas')
@UseInterceptors(ClassSerializerInterceptor)
export class KpmrRentabilitasController {
  constructor(private readonly kpmrService: KpmrRentabilitasService) {}

  // ========== KPMR ENDPOINTS ==========
  @Post()
  @ApiOperation({ summary: 'Buat KPMR baru' })
  async create(
    @Body() createDto: CreateKpmrRentabilitasOjkDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrRentabilitasOjkDto,
  ): Promise<FrontendKpmrResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createAspek(
    ...
    @Body() createDto: CreateKpmrAspekRentabilitasDto,
  ...
  async updateAspek(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrAspekRentabilitasDto,
  ): Promise<FrontendAspekResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
  ...
  async createPertanyaan(
    ...
    @Body() createDto: CreateKpmrPertanyaanRentabilitasDto,
  ...
  async updatePertanyaan(
    @Param('id') id: string,
    @Body() updateDto: UpdateKpmrPertanyaanRentabilitasDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    const idNum = parseInt(id, 10);
    if (isNaN(idNum)) {
      throw new BadRequestException('ID harus berupa angka');
