// rentabilitas-kpmr-ojk.entity.ts
import {
  Entity,
...
import { KpmrAspekRentabilitas } from './rentabilitas-kpmr-aspek.entity';

@Entity('kpmr_rentabilitas_ojk')
@Index(['year', 'quarter'])  // ✅ HAPUS { unique: true }
@Index(['isActive', 'year', 'quarter'])
@Index(['createdAt'])
export class KpmrRentabilitasOjk {
  @PrimaryGeneratedColumn()
  id: number;
  ...
  @OneToMany(() => KpmrAspekRentabilitas, (aspek) => aspek.kpmrOjk, {
    cascade: true,
    eager: false,
  })
  aspekList?: KpmrAspekRentabilitas[];
