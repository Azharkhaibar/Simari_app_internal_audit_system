...
} from './dto/rentabilitas-kpmr.dto';
...
import { KpmrAspekRentabilitas } from './entities/rentabilitas-kpmr-aspek.entity';

@Injectable()
export class KpmrRentabilitasService {
  private readonly logger = new Logger(KpmrRentabilitasService.name);
  private readonly MAX_BOBOT_TOTAL = 100;
  private readonly BOBOT_TOLERANCE = 0.01;

  constructor(
    @InjectRepository(KpmrRentabilitasOjk)
    private readonly kpmrRepository: Repository<KpmrRentabilitasOjk>,

    @InjectRepository(KpmrAspekRentabilitas)
    private readonly aspekRepository: Repository<KpmrAspekRentabilitas>,

    @InjectRepository(KpmrPertanyaanRentabilitas)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanRentabilitas>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrRentabilitasOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrRentabilitasOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekRentabilitas> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanRentabilitas> {
  ...
  private checkKpmrLocked(kpmr: KpmrRentabilitasOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekRentabilitas, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanRentabilitas, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  async createKpmr(
    createDto: CreateKpmrRentabilitasOjkDto,
    ...
    const kpmr = this.kpmrRepository.create({
      ...
      notes:
        createDto.notes ||
        `KPMR Rentabilitas ${createDto.year} Q${createDto.quarter}`,
      createdBy: createdBy,
  ...
  async findOneEntity(id: number): Promise<KpmrRentabilitasOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrRentabilitasOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
      ...
      if (kpmr.aspekList?.length) {
        for (const aspek of kpmr.aspekList) {
          if (aspek.pertanyaanList?.length) {
            await queryRunner.manager.delete(KpmrPertanyaanRentabilitas, {
              aspekId: aspek.id,
            });
          }
        }
        
        await queryRunner.manager.delete(KpmrAspekRentabilitas, {
          kpmrOjkId: id,
        });
      }

      await queryRunner.manager.delete(KpmrRentabilitasOjk, { id });
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekRentabilitasDto,
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekRentabilitasDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekRentabilitas, reorderDto.aspekIds[i], { orderIndex: i });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanRentabilitasDto,
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanRentabilitasDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanRentabilitas)
          .findOne({
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
          await manager.update(KpmrPertanyaanRentabilitas, reorderDto.pertanyaanIds[i], { orderIndex: i });
        }
      });
