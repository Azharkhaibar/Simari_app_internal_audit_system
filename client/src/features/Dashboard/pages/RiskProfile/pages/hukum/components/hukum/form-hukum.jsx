...
import { computeWeighted } from '../../utils/hukum/calc';
...
export default function FormHukum({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Hukum', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('HUKUM', `Menambahkan data hukum baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/hukum',
          isSuccess: true,
          metadata: {
      ...
      onReset();
      alert('Data hukum berhasil ditambahkan!');
    } catch (error) {
      console.error('Error adding hukum:', error);

      if (logCreate && typeof logCreate === 'function') {
        await logCreate('HUKUM', `Gagal menambahkan data hukum - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/hukum',
          isSuccess: false,
          metadata: {
            error: error.message,
      ...
      alert('Gagal menambahkan data hukum: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('HUKUM', `Memperbarui data hukum - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/hukum/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
      ...
      alert('Data hukum berhasil diperbarui!');
    } catch (error) {
      console.error('Error saving hukum:', error);

      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('HUKUM', `Gagal memperbarui data hukum - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/hukum/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: {
            error: error.message,
          },
      ...
      alert('Gagal memperbarui data hukum: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data hukum dan analisis risiko secara komprehensif</p>
            </div>
          </div>
