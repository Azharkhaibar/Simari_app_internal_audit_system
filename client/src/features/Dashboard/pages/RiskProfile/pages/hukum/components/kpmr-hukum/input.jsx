...
export function ReadOnlyField({ label, value, hint, className = '' }) {
  return (
    <FieldWrap label={label} hint={hint} className={className}>
      <div className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-gray-50 text-gray-600">{value || '-'}</div>
    </FieldWrap>
  );
}
...
export function RiskField({ label, value, onChange, color = '#93D24D', textColor = '#111827', placeholder, className = '' }) {
  return (
    <div className={`border border-gray-300 rounded-lg overflow-hidden ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-center font-semibold text-xs h-7 px-2 truncate" style={{ background: color, color: textColor }}>
        {label}
      </div>

      {/* Input */}
      <div className="p-1.5">
        <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full text-center text-xs bg-white border-none outline-none rounded" />
      </div>
    </div>
  );
}
