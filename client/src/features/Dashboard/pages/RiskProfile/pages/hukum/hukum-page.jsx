// src/features/Dashboard/pages/RiskProfile/pages/Hukum/components/HukumInherent.jsx
...
import { calculatePeringkat, calculatePeringkatFromText, isNumericRiskLevels } from './utils/riskcalculator';
// ==== Komponen & Utils ====
// import DataTable from './components/hukum/datatable-hukum';
import DataTable from './components/hukum/datatable-hukum';
import { RiskField, YearInput, QuarterSelect } from './components/hukum/input-hukum';
import { computeWeighted, makeEmptyRow } from './utils/calc';
import { exportHukumToExcel } from './utils/exportExcel';
import ToastNotification from './components/kpmr-hukum/ToastNotification';
// import { computeWeighted, makeEmptyRow } from './utils/calc';
// import { exportHukumToExcel } from './utils/exportExcel';
// import ToastNotification from './components/kpmr-hukum/ToastNotification';
...
// ==== Hooks & Services ====
// import { useHukum } from './hooks/hukum/hukum.hook';
import { useHukum } from './hook/hukum/hukum.hook';
...
export default function HukumInherent({ viewYear, viewQuarter, onViewYearChange, onViewQuarterChange, query, onQueryChange }) {
  ...
  async function HUKUM_addSection() {
    if (!HUKUM_sectionForm.no || !HUKUM_sectionForm.sectionLabel) {
    ...
    try {
      ...
      await logCreate('HUKUM', `Tambah section: ${newSection.parameter} (No: ${newSection.no})`, {
  ...
  async function HUKUM_saveSection() {
    ...
    try {
      await updateSection(HUKUM_sectionForm.id, sectionData);
      setIsAddingNewSection(false);
      setIsEditingSection(false);
      showToast(`Section "${sectionData.parameter}" berhasil diupdate`, 'success');
      await logUpdate('HUKUM', `Update section: ${sectionData.parameter} (No: ${sectionData.no})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: HUKUM_sectionForm.id, no: sectionData.no, parameter: sectionData.parameter, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  async function HUKUM_removeSection(id) {
    ...
    try {
      ...
      await logDelete('HUKUM', `Hapus section: ${sectionToDelete.parameter} (No: ${sectionToDelete.no})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isNumericRiskLevels(riskLevels)) {
        ...
        newPeringkat = calculatePeringkatFromText(HUKUM_form.hasilText || '', riskLevels);
      }
  ...
  }, [HUKUM_form?.mode, HUKUM_form?.hasilText, HUKUM_form?.low, HUKUM_form?.lowToModerate, HUKUM_form?.moderate, HUKUM_form?.moderateToHigh, HUKUM_form?.high]);
  ...
  const HUKUM_totalWeighted = useMemo(() => HUKUM_filtered.reduce((sum, r) => sum + (Number(r.weighted || 0) || 0), 0), [HUKUM_filtered]);
  ...
  const buildBaseRow = () => {
    const peringkatNum = HUKUM_form.peringkat === '' || HUKUM_form.peringkat == null ? 0 : Number(HUKUM_form.peringkat);
    ...
    return {
      ...
      numeratorLabel: HUKUM_form.mode === 'NILAI_TUNGGAL' || HUKUM_form.mode === 'TEKS' ? '' : HUKUM_form.numeratorLabel,
      numeratorValue: HUKUM_form.mode === 'NILAI_TUNGGAL' || HUKUM_form.mode === 'TEKS' ? '' : HUKUM_form.numeratorValue,
    };
  };

  const HUKUM_addRow = async () => {
    if (!HUKUM_sectionForm.id) {
    ...
    const section = currentPeriodSections.find((s) => s.id === HUKUM_sectionForm.id);

    if (!section) return;

    const weightedAuto = computeWeightedLocal(baseRow.bobotSection, baseRow.bobotIndikator, baseRow.peringkat);
    const backendData = transformToBackend(baseRow, viewYear, viewQuarter, HUKUM_sectionForm.id, section);
    ...
    try {
      ...
      await logCreate('HUKUM', `Tambah indikator: ${backendData.indikator} (Section: ${section.parameter})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: HUKUM_sectionForm.id, subNo: backendData.subNo, indikator: backendData.indikator, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const HUKUM_saveEdit = async () => {
    ...
    const updateData = {
      year: viewYear,
      quarter: viewQuarter,
      sectionId: HUKUM_sectionForm.id,
      subNo: baseRow.subNo,
    ...
    try {
      await updateIndikator(HUKUM_editingRow.id, updateData);
      HUKUM_resetForm();
      setShowHukumForm(false);
      showToast('Indikator berhasil diupdate!', 'success');
      await logUpdate('HUKUM', `Update indikator: ${updateData.indikator} (ID: ${HUKUM_editingRow.id})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', indicatorId: HUKUM_editingRow.id, subNo: updateData.subNo, indikator: updateData.indikator, peringkat: updateData.peringkat, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const HUKUM_removeRow = async (row) => {
    if (!row) return;
    ...
    try {
      ...
      if (HUKUM_editingRow?.id === row.id) HUKUM_resetForm();
      showToast(`Indikator "${row.indikator}" berhasil dihapus`, 'success');
      await logDelete('HUKUM', `Hapus indikator: ${row.indikator} (Sub No: ${row.subNo})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  const HUKUM_exportExcel = async () => {
    try {
      exportHukumToExcel(HUKUM_filtered, viewYear, viewQuarter);
      showToast(`Data Hukum tahun ${viewYear} triwulan ${viewQuarter} berhasil diexport`, 'success');
      await logExport('HUKUM', `Export data Hukum tahun ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', year: viewYear, quarter: viewQuarter, dataCount: HUKUM_filtered.length }
      });
    } catch (err) {
      showToast('Gagal mengexport data', 'error');
      await logExport('HUKUM', `Gagal export Hukum ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: false,
      ...
      <header className="px-4 py-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex items-end gap-2">
            <button onClick={() => setShowHukumForm(true)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 font-semibold">
              + Tambah Data
            </button>

            <button onClick={HUKUM_exportExcel} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
          ...
          <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-white font-semibold text-lg sm:text-xl">Form Section – Hukum</h2>
              <button onClick={() => setShowHukumForm(false)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-white/20 hover:bg-white/30 text-white font-bold" title="Tutup Form">
                ✕
              </button>
            </div>
            ...
            <div className="flex items-end gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setHUKUM_sectionForm((f) => ({ ...f, no: e.target.value }))}
                />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setHUKUM_sectionForm((f) => ({ ...f, bobotSection: e.target.value }))}
                />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setHUKUM_sectionForm((f) => ({ ...f, sectionLabel: e.target.value }))}
              ...
              <div className="flex gap-2 self-end">
                {!isAddingNewSection && !isEditingSection && (
                  <>
                    ...
                    <button className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center" onClick={() => HUKUM_removeSection(HUKUM_sectionForm.id)} title="Hapus Section">
                      <Trash2 size={20} />
                    </button>
                  </>
                )}
                ...
                {(isAddingNewSection || isEditingSection) && (
                  <>
                    <button
                      ...
                      onClick={isAddingNewSection ? HUKUM_addSection : HUKUM_saveSection}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setIsEditingSection(false);
                        if (HUKUM_sectionForm.id) {
                          HUKUM_selectSection(HUKUM_sectionForm.id);
                        }
                      }}
                      title="Batal"
                    >
            ...
            <div className="relative">
              ...
              <select
                ...
                value={isAddingNewSection ? 'ADD_NEW_SECTION' : HUKUM_sectionForm.id || ''}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === 'ADD_NEW_SECTION') {
                    setIsAddingNewSection(true);
                    setIsEditingSection(true);
                    setHUKUM_sectionForm({ id: 0, no: '', bobotSection: 0, sectionLabel: '' });
                    return;
                  }
                  if (val) {
                    setIsAddingNewSection(false);
                    setIsEditingSection(false);
                    HUKUM_selectSection(Number(val));
                  }
                }}
              >
          ...
          <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-2">
                ...
                <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={HUKUM_form.subNo} onChange={(e) => setHUKUM_form((f) => ({ ...f, subNo: e.target.value }))} placeholder="1.1" />
              </div>
              <div className="col-span-7">
                ...
                <input
                  ...
                  onChange={(e) => setHUKUM_form((f) => ({ ...f, indikator: e.target.value }))}
              ...
              <div className="col-span-3">
                ...
                <input
                  ...
                  onChange={(e) => setHUKUM_form((f) => ({ ...f, bobotIndikator: e.target.value }))}
                  placeholder="50"
                />
              </div>
            </div>
                ...
                {HUKUM_form.mode === 'TEKS' ? (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) => setHUKUM_form((f) => ({ ...f, hasilText: e.target.value }))}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        type="text"
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                        placeholder={HUKUM_form.mode === 'RASIO' ? 'Contoh default: pemb / peny — atau rumus custom (pemb, peny)' : 'Contoh default: peny — atau rumus custom (peny / 1000)'}
                        value={HUKUM_form.formula || ''}
                        onChange={(e) => setHUKUM_form((f) => ({ ...f, formula: e.target.value }))}
                      />
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={HUKUM_form.isPercent || false}
                          onChange={(e) => setHUKUM_form((f) => ({ ...f, isPercent: e.target.checked }))}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setHUKUM_form((f) => ({ ...f, numeratorLabel: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setHUKUM_form((f) => ({ ...f, numeratorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setHUKUM_form((f) => ({ ...f, denominatorLabel: e.target.value }))}
                    placeholder={HUKUM_form.mode === 'RASIO' ? 'Total Asset (Jutaan)' : 'Jumlah kejadian, jumlah kasus, dll.'}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setHUKUM_form((f) => ({ ...f, denominatorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      if (!val || val.endsWith(',') || val.endsWith('.') || val === '-') return;
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-6 space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setHUKUM_form((f) => ({ ...f, sumberRisiko: e.target.value }))}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setHUKUM_form((f) => ({ ...f, dampak: e.target.value }))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="number" className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-100 font-bold text-center" value={HUKUM_form.peringkat ?? ''} readOnly />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                        value={(() => {
                          if (HUKUM_form.mode === 'TEKS') return HUKUM_form.hasilText || '';
                          ...
                          if (HUKUM_form.mode === 'NILAI_TUNGGAL') return fmtNumber(raw);
                  ...
                  <div>
                    ...
                    <textarea rows={3} className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white" value={HUKUM_form.keterangan} onChange={(e) => setHUKUM_form((f) => ({ ...f, keterangan: e.target.value }))} />
                  </div>
                </div>
                ...
                <div className="col-span-6 grid grid-cols-1 gap-3 items-stretch">
                  <RiskField
                    ...
                    onChange={(v) => setHUKUM_form((f) => ({ ...f, low: v }))}
                    color="#4A5A2C"
                    textColor="#ffffff"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setHUKUM_form((f) => ({ ...f, lowToModerate: v }))}
                    color="#A6D86C"
                    textColor="#0B2545"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setHUKUM_form((f) => ({ ...f, moderate: v }))}
                    color="#FFFF00"
                    textColor="#4B3A00"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setHUKUM_form((f) => ({ ...f, moderateToHigh: v }))}
                    color="#FAD2A7"
                    textColor="#5A2E00"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setHUKUM_form((f) => ({ ...f, high: v }))}
                    color="#E57373"
                    textColor="#FFFFFF"
            ...
            <div className="mt-6 flex justify-end">
              {!HUKUM_editingRow ? (
                <button onClick={HUKUM_addRow} className="px-8 py-3 rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold">
                  + Tambah
                </button>
              ) : (
                <div className="flex gap-3">
                  <button onClick={HUKUM_saveEdit} className="px-8 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">
                    Simpan
                  </button>
                  <button onClick={HUKUM_resetForm} className="px-8 py-3 rounded-lg border-2 border-gray-300 hover:bg-gray-50 font-semibold">
                    Batal
                  </button>
                </div>
              )}
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
          <div className="relative h-[350px]">
            <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
              <DataTable rows={HUKUM_filtered} totalWeighted={HUKUM_totalWeighted} viewYear={viewYear} viewQuarter={viewQuarter} startEdit={HUKUM_startEdit} removeRow={HUKUM_removeRow} />
            </div>
          </div>
        </div>
      </section>
