// src/features/Dashboard/pages/RiskProfile/pages/Stratejik/hooks/KPMR/kpmr-stratejik.hook.ts

import { useState, useEffect, useCallback } from 'react';
import {
  kpmrStratejikApiService,
  KPMRStratejikAspect,
  KPMRStratejikQuestion,
  KPMRStratejikDefinition,
  KPMRStratejikScore,
  KPMRStratejikFullDataResponse,
  CreateKPMRStratejikAspectData,
  UpdateKPMRStratejikAspectData,
  CreateKPMRStratejikQuestionData,
  UpdateKPMRStratejikQuestionData,
  CreateKPMRStratejikDefinitionData,
  UpdateKPMRStratejikDefinitionData,
  CreateKPMRStratejikScoreData,
  UpdateKPMRStratejikScoreData,
  Period,
  DeleteResponse,
  transformFullDataToGroups,
} from '../../service/kpmr-stratejik/kpmr-stratejik.service';

// ===================== INTERFACES =====================

export interface KPMRStratejikFilters {
  year?: number;
  quarter?: string;
  aspekNo?: string;
  query?: string;
...
export interface KPMRStratejikGroup {
  aspekNo: string;
  aspekTitle: string;
  aspekBobot: number;
  sections: Array<{
...
export interface UseKpmrStratejikReturn {
  // ========== STATE ==========
  aspects: KPMRStratejikAspect[];
  questions: KPMRStratejikQuestion[];
  definitions: KPMRStratejikDefinition[];
  scores: KPMRStratejikScore[];
  fullData: KPMRStratejikFullDataResponse | null;
  groups: KPMRStratejikGroup[];
  periods: Period[];
  years: number[];
  viewYear: number;
  viewQuarter: string;
  ...
  search: (year?: number, query?: string, aspekNo?: string) => Promise<KPMRStratejikDefinition[]>;

  // ========== ASPECT CRUD ==========
  createAspect: (data: CreateKPMRStratejikAspectData) => Promise<KPMRStratejikAspect>;
  updateAspect: (id: number, data: UpdateKPMRStratejikAspectData) => Promise<KPMRStratejikAspect>;
  deleteAspect: (id: number) => Promise<DeleteResponse>;

  // ========== QUESTION CRUD ==========
  createQuestion: (data: CreateKPMRStratejikQuestionData) => Promise<KPMRStratejikQuestion>;
  updateQuestion: (id: number, data: UpdateKPMRStratejikQuestionData) => Promise<KPMRStratejikQuestion>;
  deleteQuestion: (id: number) => Promise<DeleteResponse>;

  // ========== DEFINITION CRUD ==========
  createOrUpdateDefinition: (data: CreateKPMRStratejikDefinitionData) => Promise<KPMRStratejikDefinition>;
  updateDefinition: (id: number, data: UpdateKPMRStratejikDefinitionData) => Promise<KPMRStratejikDefinition>;
  deleteDefinition: (definitionId: number, year: number) => Promise<DeleteResponse>;

  // ========== SCORE CRUD ==========
  createOrUpdateScore: (data: CreateKPMRStratejikScoreData) => Promise<KPMRStratejikScore>;
  updateScore: (id: number, data: UpdateKPMRStratejikScoreData) => Promise<KPMRStratejikScore>;
...
interface UseKpmrStratejikOptions {
  initialYear?: number;
...
export const useKpmrStratejik = (options?: UseKpmrStratejikOptions): UseKpmrStratejikReturn => {
  ...
  const [aspects, setAspects] = useState<KPMRStratejikAspect[]>([]);
  const [questions, setQuestions] = useState<KPMRStratejikQuestion[]>([]);
  const [definitions, setDefinitions] = useState<KPMRStratejikDefinition[]>([]);
  const [scores, setScores] = useState<KPMRStratejikScore[]>([]);
  const [fullData, setFullData] = useState<KPMRStratejikFullDataResponse | null>(null);
  const [groups, setGroups] = useState<KPMRStratejikGroup[]>([]);
  ...
  const fetchAspects = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrStratejikApiService.getAllAspects(year));
        setAspects(data);
      } catch (err) {
  ...
  const fetchQuestions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrStratejikApiService.getAllQuestions(year));
        setQuestions(data);
      } catch (err) {
  ...
  const fetchDefinitions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        ...
        const data = await withLoading(() => kpmrStratejikApiService.getDefinitionsByYear(targetYear));
        setDefinitions(data);
      } catch (err) {
  ...
  const fetchScores = useCallback(
    async (year?: number, quarter?: string): Promise<void> => {
      try {
        const targetYear = year || viewYear;
        const data = await withLoading(() => kpmrStratejikApiService.getScoresByPeriod(targetYear, quarter));
        setScores(data);
      } catch (err) {
  ...
  const fetchFullData = useCallback(async (year: number): Promise<void> => {
    ...
    try {
      setLoading(true);
      const data = await kpmrStratejikApiService.getFullData(year);
      setFullData(data);
  ...
  const fetchPeriods = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrStratejikApiService.getPeriods());
      setPeriods(data);
    } catch (err) {
  ...
  const fetchYears = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrStratejikApiService.getAvailableYears());
      setYears(data);
    } catch (err) {
  ...
  const search = useCallback(
    async (year?: number, searchQuery?: string, aspekNo?: string): Promise<KPMRStratejikDefinition[]> => {
      try {
        return await withLoading(() => kpmrStratejikApiService.searchKPMR(year, searchQuery || query, aspekNo));
      } catch (err) {
  ...
  const createAspect = useCallback(
    async (data: CreateKPMRStratejikAspectData): Promise<KPMRStratejikAspect> => {
      try {
        const newAspect = await withLoading(() => kpmrStratejikApiService.createAspect(data));
  ...
  const updateAspect = useCallback(
    async (id: number, data: UpdateKPMRStratejikAspectData): Promise<KPMRStratejikAspect> => {
      try {
        const updatedAspect = await withLoading(() => kpmrStratejikApiService.updateAspect(id, data));
  ...
  const deleteAspect = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrStratejikApiService.deleteAspect(id));
        if (result.success) {
  ...
  const createQuestion = useCallback(
    async (data: CreateKPMRStratejikQuestionData): Promise<KPMRStratejikQuestion> => {
      try {
        const newQuestion = await withLoading(() => kpmrStratejikApiService.createQuestion(data));
  ...
  const updateQuestion = useCallback(
    async (id: number, data: UpdateKPMRStratejikQuestionData): Promise<KPMRStratejikQuestion> => {
      try {
        const updatedQuestion = await withLoading(() => kpmrStratejikApiService.updateQuestion(id, data));
  ...
  const deleteQuestion = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        ...
        const result = await withLoading(() => kpmrStratejikApiService.deleteQuestion(id));
  ...
  const createOrUpdateDefinition = useCallback(
    async (data: CreateKPMRStratejikDefinitionData): Promise<KPMRStratejikDefinition> => {
      try {
        const definition = await withLoading(() => kpmrStratejikApiService.createOrUpdateDefinition(data));
  ...
  const updateDefinition = useCallback(
    async (id: number, data: UpdateKPMRStratejikDefinitionData): Promise<KPMRStratejikDefinition> => {
      try {
        const cleanData: UpdateKPMRStratejikDefinitionData = {};
        ...
        const updatedDefinition = await withLoading(() => kpmrStratejikApiService.updateDefinition(id, cleanData));
  ...
  const deleteDefinition = useCallback(
    async (definitionId: number, year: number): Promise<DeleteResponse> => {
      try {
        ...
        const response = await withLoading(() => kpmrStratejikApiService.deleteDefinitionPermanent(definitionId, year));
  ...
  const createOrUpdateScore = useCallback(
    async (data: CreateKPMRStratejikScoreData): Promise<KPMRStratejikScore> => {
      try {
        const score = await withLoading(() => kpmrStratejikApiService.createOrUpdateScore(data));
  ...
  const updateScore = useCallback(
    async (id: number, data: UpdateKPMRStratejikScoreData): Promise<KPMRStratejikScore> => {
      try {
        const updatedScore = await withLoading(() => kpmrStratejikApiService.updateScore(id, data));
  ...
  const deleteScore = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrStratejikApiService.deleteScore(id));
  ...
  const deleteScoreByTarget = useCallback(
    async (definitionId: number, year: number, quarter: string): Promise<DeleteResponse> => {
      try {
        const response = await withLoading(() => kpmrStratejikApiService.deleteScoreByTarget(definitionId, year, quarter));
        await fetchScores(year);
        await fetchFullData(year);
        return response;
      } catch (err) {
