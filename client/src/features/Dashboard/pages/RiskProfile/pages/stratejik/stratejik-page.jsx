...
import { RiskField, YearInput, QuarterSelect } from './components/stratejik/input-stratejik';
import { getCurrentQuarter, getCurrentYear } from './utils/time';
// import { computeWeighted, makeEmptyRow } from './utils/stratejik/calc';
// import { exportStratejikToExcel } from './utils/stratejik/exportExcel';
// import ToastNotification from './components/kpmr-stratejik/ToastNotification';
import { exportStratejikToExcel } from './utils/exportExcel';
import { computeWeighted, makeEmptyRow } from './utils/calc';
import ToastNotification from './components/kpmr-stratejik/ToastNotification';

// ==== Hooks & Services ====
// import { useStratejik } from './hooks/stratejik/new-stratejik.hook';
...
// ===================== Stratejik: fallback empty row =====================
const stFallbackEmpty = (year, quarter) => ({
  year,
  quarter,
  no: '',
  subNo: '',
...
const computeStratejikHasil = (row) => {
...
export default function StratejikInherent({ viewYear, viewQuarter, onViewYearChange, onViewQuarterChange, query, onQueryChange }) {
  const [showStratejikForm, setShowStratejikForm] = useState(false);
  ...
  // ----- SECTION LIST -----
  const [STRATEJIK_sectionForm, setSTRATEJIK_sectionForm] = useState({
    id: 0,
    no: '',
    bobotSection: 0,
    sectionLabel: '',
  ...
  function STRATEJIK_selectSection(id) {
    const s = currentPeriodSections.find((x) => x.id === id);
    if (s) {
      setSTRATEJIK_sectionForm({
        id: s.id,
        no: s.no,
  ...
  async function STRATEJIK_addSection() {
    if (!STRATEJIK_sectionForm.no || !STRATEJIK_sectionForm.sectionLabel) {
      showToast('No Section dan Section Label harus diisi!', 'error');
      return;
    }

    const sectionData = {
      no: STRATEJIK_sectionForm.no,
      bobotSection: Number(STRATEJIK_sectionForm.bobotSection || 0),
      parameter: STRATEJIK_sectionForm.sectionLabel,
      year: viewYear,
      quarter: viewQuarter,
      isActive: true,
    };
    ...
    try {
      ...
      await logCreate('STRATEJIK', `Tambah section: ${newSection.parameter} (No: ${newSection.no})`, { userId: currentUser.id, isSuccess: true, metadata: { sectionId: newSection.id, no: newSection.no, parameter: newSection.parameter, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  async function STRATEJIK_saveSection() {
    if (!STRATEJIK_sectionForm.id) return;
    ...
    try {
      await updateSection(STRATEJIK_sectionForm.id, sectionData);
      setIsAddingNewSection(false);
      setIsEditingSection(false);
      showToast(`Section "${sectionData.parameter}" berhasil diupdate`, 'success');
      await logUpdate('STRATEJIK', `Update section: ${sectionData.parameter} (No: ${sectionData.no})`, { userId: currentUser.id, isSuccess: true, metadata: { sectionId: STRATEJIK_sectionForm.id, no: sectionData.no, parameter: sectionData.parameter, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  async function STRATEJIK_removeSection(id) {
    const sectionToDelete = currentPeriodSections.find((s) => s.id === id);

    if (!sectionToDelete) {
      console.warn('[STRATEJIK_removeSection] Section not found:', id);
    ...
    try {
      ...
      await logDelete('STRATEJIK', `Hapus section: ${sectionToDelete.parameter} (No: ${sectionToDelete.no})`, { userId: currentUser.id, isSuccess: true, metadata: { sectionId: id, no: sectionToDelete.no, parameter: sectionToDelete.parameter, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  const stMakeRow = () =>
    typeof makeEmptyRow === 'function'
      ...
      : stFallbackEmpty(viewYear, viewQuarter);

  const [STRATEJIK_form, setSTRATEJIK_form] = useState(stMakeRow());
  const [STRATEJIK_editingRow, setSTRATEJIK_editingRow] = useState(null);
  ...
  useEffect(() => {
    if (STRATEJIK_form?.mode === 'TEKS') return;
    if (!STRATEJIK_form || !STRATEJIK_sectionForm) return;

    try {
      const baseRow = {
        ...STRATEJIK_form,
        no: STRATEJIK_sectionForm.no || '',
        sectionLabel: STRATEJIK_sectionForm.sectionLabel || '',
        bobotSection: STRATEJIK_sectionForm.bobotSection || 0,
      };

      const hasilNum = computeStratejikHasil(baseRow);

      if (hasilNum !== '' && hasilNum != null) {
        const newPeringkat = calculatePeringkat(
          hasilNum,
          {
            low: STRATEJIK_form.low || '',
            lowToModerate: STRATEJIK_form.lowToModerate || '',
            moderate: STRATEJIK_form.moderate || '',
            moderateToHigh: STRATEJIK_form.moderateToHigh || '',
      ...
      console.warn('[STRATEJIK] Peringkat calculation failed:', err);
    }
  }, [
    STRATEJIK_form?.low,
    STRATEJIK_form?.lowToModerate,
    STRATEJIK_form?.moderate,
    STRATEJIK_form?.moderateToHigh,
    STRATEJIK_form?.numeratorValue,
    STRATEJIK_form?.denominatorValue,
    STRATEJIK_form?.formula,
    STRATEJIK_form?.isPercent,
    STRATEJIK_form?.mode,
    STRATEJIK_sectionForm?.bobotSection,
  ]);

  // Auto-calculate untuk TEKS mode
  useEffect(() => {
    if (STRATEJIK_form?.mode !== 'TEKS') return;
    if (!STRATEJIK_form) return;
    ...
    try {
      ...
      console.warn('[STRATEJIK] TEKS peringkat calculation failed:', err);
    }
  }, [STRATEJIK_form?.mode, STRATEJIK_form?.hasilText, STRATEJIK_form?.low, STRATEJIK_form?.lowToModerate, STRATEJIK_form?.moderate, STRATEJIK_form?.moderateToHigh, STRATEJIK_form?.high]);
  ...
  const STRATEJIK_filtered = useMemo(() => {
  ...
  const STRATEJIK_totalWeighted = useMemo(() => STRATEJIK_filtered.reduce((sum, r) => sum + (Number(r.weighted || 0) || 0), 0), [STRATEJIK_filtered]);

  const STRATEJIK_resetForm = () => {
    setSTRATEJIK_form(stMakeRow());
    setSTRATEJIK_editingRow(null);
    setRawNumeratorInput('');
    setRawDenominatorInput('');
  };

  const buildBaseRow = () => {
    const peringkatNum = STRATEJIK_form.peringkat === '' || STRATEJIK_form.peringkat == null ? 0 : Number(STRATEJIK_form.peringkat);

    return {
      ...STRATEJIK_form,
      no: STRATEJIK_sectionForm.no,
      sectionLabel: STRATEJIK_sectionForm.sectionLabel,
      bobotSection: STRATEJIK_sectionForm.bobotSection,
      peringkat: peringkatNum,
      numeratorLabel: STRATEJIK_form.mode === 'NILAI_TUNGGAL' || STRATEJIK_form.mode === 'TEKS' ? '' : STRATEJIK_form.numeratorLabel,
      numeratorValue: STRATEJIK_form.mode === 'NILAI_TUNGGAL' || STRATEJIK_form.mode === 'TEKS' ? '' : STRATEJIK_form.numeratorValue,
    };
  };

  const STRATEJIK_addRow = async () => {
    if (!STRATEJIK_sectionForm.id) {
    ...
    const section = currentPeriodSections.find((s) => s.id === STRATEJIK_sectionForm.id);

    if (!section) return;

    const weightedAuto = computeWeightedLocal(baseRow.bobotSection, baseRow.bobotIndikator, baseRow.peringkat);
    const backendData = transformToBackend(baseRow, viewYear, viewQuarter, STRATEJIK_sectionForm.id, section);

    try {
      await createIndikator(backendData);
      STRATEJIK_resetForm();
      setShowStratejikForm(false);
      showToast('Indikator berhasil ditambahkan!', 'success');
      await logCreate('STRATEJIK', `Tambah indikator: ${backendData.indikator} (Section: ${section.parameter})`, { userId: currentUser.id, isSuccess: true, metadata: { sectionId: STRATEJIK_sectionForm.id, subNo: backendData.subNo, indikator: backendData.indikator, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  const STRATEJIK_startEdit = (row) => {
    if (!row) return;
    setShowStratejikForm(true);

    setSTRATEJIK_editingRow(row);
  ...
  const STRATEJIK_saveEdit = async () => {
    ...
    const updateData = {
      year: viewYear,
      quarter: viewQuarter,
      sectionId: STRATEJIK_sectionForm.id,
      subNo: baseRow.subNo,
    ...
    try {
      await updateIndikator(STRATEJIK_editingRow.id, updateData);
      STRATEJIK_resetForm();
      setShowStratejikForm(false);
      showToast('Indikator berhasil diupdate!', 'success');
      await logUpdate('STRATEJIK', `Update indikator: ${updateData.indikator} (ID: ${STRATEJIK_editingRow.id})`, { userId: currentUser.id, isSuccess: true, metadata: { indicatorId: STRATEJIK_editingRow.id, subNo: updateData.subNo, indikator: updateData.indikator, peringkat: updateData.peringkat, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  const STRATEJIK_removeRow = async (row) => {
    if (!row) return;
    ...
    try {
      ...
      if (STRATEJIK_editingRow?.id === row.id) STRATEJIK_resetForm();
      showToast(`Indikator "${row.indikator}" berhasil dihapus`, 'success');
      await logDelete('STRATEJIK', `Hapus indikator: ${row.indikator} (Sub No: ${row.subNo})`, { userId: currentUser.id, isSuccess: true, metadata: { indicatorId: row.id, subNo: row.subNo, indikator: row.indikator, year: viewYear, quarter: viewQuarter } });
    } catch (err) {
  ...
  const STRATEJIK_exportExcel = async () => {
    try {
      exportStratejikToExcel(STRATEJIK_filtered, viewYear, viewQuarter);
      showToast(`Data Stratejik tahun ${viewYear} triwulan ${viewQuarter} berhasil diexport`, 'success');
      await logExport('STRATEJIK', `Export data Stratejik ${viewYear} TW${viewQuarter}`, { userId: currentUser.id, isSuccess: true, metadata: { year: viewYear, quarter: viewQuarter, dataCount: STRATEJIK_filtered.length } });
    } catch (err) {
      showToast('Gagal mengexport data', 'error');
      await logExport('STRATEJIK', `Gagal export Stratejik ${viewYear} TW${viewQuarter}`, { userId: currentUser.id, isSuccess: false, metadata: { error: err.message } });
    }
  };
  ...
  return (
    <>
      ...
      <header className="px-4 py-4 flex items-center justify-between gap-3">
        <h2 className="text-xl sm:text-2xl font-semibold">Form – Stratejik</h2>
        <div className="flex items-end gap-4">
            ...
            <div className="flex flex-col gap-1">
              <QuarterSelect
                ...
                onChange={(v) => {
                  ...
                  setSTRATEJIK_form((f) => ({ ...f, quarter: v }));
                }}
              />
            </div>
          </div>
          ...
          <div className="flex items-end gap-2">
            ...
            <button onClick={STRATEJIK_exportExcel} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
          ...
          <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-white font-semibold text-lg sm:text-xl">Form Section – Stratejik</h2>
              <button onClick={() => setShowStratejikForm(false)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-white/20 hover:bg-white/30 text-white font-bold" title="Tutup Form">
                ✕
              </button>
            </div>
            ...
            <div className="flex items-end gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={STRATEJIK_sectionForm.no}
                  onChange={(e) => setSTRATEJIK_sectionForm((f) => ({ ...f, no: e.target.value }))}
                />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  style={!isAddingNewSection && !isEditingSection ? { backgroundColor: '#f3f4f6', cursor: 'not-allowed' } : {}}
                  disabled={!isAddingNewSection && !isEditingSection}
                  value={STRATEJIK_sectionForm.bobotSection ?? ''}
                  onChange={(e) => setSTRATEJIK_sectionForm((f) => ({ ...f, bobotSection: e.target.value }))}
                />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <input
                  ...
                  style={!isAddingNewSection && !isEditingSection ? { backgroundColor: '#f3f4f6', cursor: 'not-allowed' } : {}}
                  disabled={!isAddingNewSection && !isEditingSection}
                  value={STRATEJIK_sectionForm.sectionLabel}
                  onChange={(e) => setSTRATEJIK_sectionForm((f) => ({ ...f, sectionLabel: e.target.value }))}
              ...
              <div className="flex gap-2 self-end">
                {!isAddingNewSection && !isEditingSection && (
                  <>
                    ...
                    <button className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center" onClick={() => STRATEJIK_removeSection(STRATEJIK_sectionForm.id)} title="Hapus Section">
                      <Trash2 size={20} />
                    </button>
                  </>
                )}
                ...
                {(isAddingNewSection || isEditingSection) && (
                  <>
                    <button
                      ...
                      onClick={isAddingNewSection ? STRATEJIK_addSection : STRATEJIK_saveSection}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setIsEditingSection(false);
                        if (STRATEJIK_sectionForm.id) {
                          STRATEJIK_selectSection(STRATEJIK_sectionForm.id);
                        }
                      }}
                      title="Batal"
                    >
            ...
            <div className="relative">
              ...
              <select
                ...
                value={isAddingNewSection ? 'ADD_NEW_SECTION' : STRATEJIK_sectionForm.id || ''}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === 'ADD_NEW_SECTION') {
                    setIsAddingNewSection(true);
                    setIsEditingSection(true);
                    setSTRATEJIK_sectionForm({ id: 0, no: '', bobotSection: 0, sectionLabel: '' });
                    return;
                  }
                  if (val) {
                    setIsAddingNewSection(false);
                    setIsEditingSection(false);
                    STRATEJIK_selectSection(Number(val));
                  }
                }}
              >
          ...
          <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-2">
                ...
                <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={STRATEJIK_form.subNo} onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, subNo: e.target.value }))} placeholder="1.1" />
              </div>
              <div className="col-span-7">
                ...
                <input
                  ...
                  value={STRATEJIK_form.indikator}
                  onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, indikator: e.target.value }))}
                  placeholder="Nama indikator stratejik…"
                />
              </div>
              <div className="col-span-3">
                ...
                <input
                  ...
                  value={STRATEJIK_form.bobotIndikator ?? ''}
                  onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, bobotIndikator: e.target.value }))}
                  placeholder="50"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-4">
                ...
                <select
                  ...
                  value={STRATEJIK_form.mode || 'RASIO'}
                  onChange={(e) => {
              ...
              <div className="col-span-8">
                {STRATEJIK_form.mode === 'TEKS' ? (
                  <>
                    ...
                    <input
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                      value={STRATEJIK_form.hasilText || ''}
                      onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, hasilText: e.target.value }))}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        type="text"
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                        placeholder={STRATEJIK_form.mode === 'RASIO' ? 'Contoh default: pemb / peny — atau rumus custom (pemb, peny)' : 'Contoh default: peny — atau rumus custom (peny / 1000)'}
                        value={STRATEJIK_form.formula || ''}
                        onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, formula: e.target.value }))}
                      />
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={STRATEJIK_form.isPercent || false}
                          onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, isPercent: e.target.checked }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setSTRATEJIK_form((f) => ({ ...f, numeratorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      ...
                      if (formatted && formatted !== val) {
                        ...
                        setSTRATEJIK_form((f) => ({ ...f, numeratorValue: formatted }));
                      }
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setSTRATEJIK_form((f) => ({ ...f, denominatorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      if (!val || val.endsWith(',') || val.endsWith('.') || val === '-') return;
                      ...
                      if (formatted && formatted !== val) {
                        ...
                        setSTRATEJIK_form((f) => ({ ...f, denominatorValue: formatted }));
                      }
                    }}
                    placeholder="Contoh: -1000, 1.000, 1.000,50, 10%"
                  />
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-6 space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      value={STRATEJIK_form.sumberRisiko}
                      onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, sumberRisiko: e.target.value }))}
                  ...
                  <div>
                    ...
                    <textarea
                      rows={4}
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm resize-none bg-white"
                      value={STRATEJIK_form.dampak}
                      onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, dampak: e.target.value }))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="number" className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-100 font-bold text-center" value={STRATEJIK_form.peringkat ?? ''} readOnly />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        value={(() => {
                          const peringkatNum = STRATEJIK_form.peringkat === '' || STRATEJIK_form.peringkat == null ? 0 : Number(STRATEJIK_form.peringkat);
                          const bobotSecNum = Number(STRATEJIK_sectionForm.bobotSection || 0);
                          const bobotIndNum = Number(STRATEJIK_form.bobotIndikator || 0);
                    ...
                    <div>
                      ...
                      <input
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                        value={(() => {
                          if (STRATEJIK_form.mode === 'TEKS') return STRATEJIK_form.hasilText || '';
                          const raw = computeStratejikHasil(buildBaseRow());
                          if (raw === '' || raw == null) return '';
                          if (STRATEJIK_form.isPercent) {
                            const pct = Number(raw) * 100;
                            if (!isFinite(pct) || isNaN(pct)) return '';
                            return `${pct.toFixed(2)}%`;
                          }
                          if (STRATEJIK_form.mode === 'NILAI_TUNGGAL') return fmtNumber(raw);
                  ...
                  <div>
                    ...
                    <textarea rows={3} className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white" value={STRATEJIK_form.keterangan} onChange={(e) => setSTRATEJIK_form((f) => ({ ...f, keterangan: e.target.value }))} />
                  </div>
                </div>
                ...
                <div className="col-span-6 grid grid-cols-1 gap-3 items-stretch">
                  <RiskField
                    className="w-full"
                    label="Low"
                    value={STRATEJIK_form.low}
                    onChange={(v) => setSTRATEJIK_form((f) => ({ ...f, low: v }))}
                    color="#4A5A2C"
                    textColor="#ffffff"
                    placeholder={STRATEJIK_form.mode === 'TEKS' ? 'Contoh: Baik, Aman, Tes' : 'x ≤ 1%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Low to Moderate"
                    value={STRATEJIK_form.lowToModerate}
                    onChange={(v) => setSTRATEJIK_form((f) => ({ ...f, lowToModerate: v }))}
                    color="#A6D86C"
                    textColor="#0B2545"
                    placeholder={STRATEJIK_form.mode === 'TEKS' ? 'Contoh: Cukup Baik' : '1% < x ≤ 2%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Moderate"
                    value={STRATEJIK_form.moderate}
                    onChange={(v) => setSTRATEJIK_form((f) => ({ ...f, moderate: v }))}
                    color="#FFFF00"
                    textColor="#4B3A00"
                    placeholder={STRATEJIK_form.mode === 'TEKS' ? 'Contoh: Sedang, Normal' : '2% < x ≤ 3%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Moderate to High"
                    value={STRATEJIK_form.moderateToHigh}
                    onChange={(v) => setSTRATEJIK_form((f) => ({ ...f, moderateToHigh: v }))}
                    color="#FAD2A7"
                    textColor="#5A2E00"
                    placeholder={STRATEJIK_form.mode === 'TEKS' ? 'Contoh: Cukup Tinggi' : '3% < x ≤ 4%'}
                  />
                  <RiskField
                    className="w-full"
                    label="High"
                    value={STRATEJIK_form.high}
                    onChange={(v) => setSTRATEJIK_form((f) => ({ ...f, high: v }))}
                    color="#E57373"
                    textColor="#FFFFFF"
                    placeholder={STRATEJIK_form.mode === 'TEKS' ? 'Contoh: Buruk, Berbahaya, Tinggi' : 'x > 4%'}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end">
              {!STRATEJIK_editingRow ? (
                <button onClick={STRATEJIK_addRow} className="px-8 py-3 rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold">
                  + Tambah
                </button>
              ) : (
                <div className="flex gap-3">
                  <button onClick={STRATEJIK_saveEdit} className="px-8 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">
                    Simpan
                  </button>
                  <button onClick={STRATEJIK_resetForm} className="px-8 py-3 rounded-lg border-2 border-gray-300 hover:bg-gray-50 font-semibold">
                    Batal
                  </button>
                </div>
              )}
