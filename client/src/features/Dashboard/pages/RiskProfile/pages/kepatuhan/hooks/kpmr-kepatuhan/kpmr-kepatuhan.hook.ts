// src/features/Dashboard/pages/RiskProfile/pages/Kepatuhan/hooks/KPMR/kpmr-kepatuhan.hook.ts

import { useState, useEffect, useCallback } from 'react';
import {
  kpmrKepatuhanApiService,
  KPMRKepatuhanAspect,
  KPMRKepatuhanQuestion,
  KPMRKepatuhanDefinition,
  KPMRKepatuhanScore,
  KPMRKepatuhanFullDataResponse,
  CreateKPMRKepatuhanAspectData,
  UpdateKPMRKepatuhanAspectData,
  CreateKPMRKepatuhanQuestionData,
  UpdateKPMRKepatuhanQuestionData,
  CreateKPMRKepatuhanDefinitionData,
  UpdateKPMRKepatuhanDefinitionData,
  CreateKPMRKepatuhanScoreData,
  UpdateKPMRKepatuhanScoreData,
  Period,
  DeleteResponse,
  transformFullDataToGroups,
} from '../../services/kpmr-kepatuhan/kpmr-kepatuhan.service';

// ===================== INTERFACES =====================

export interface KPMRKepatuhanFilters {
  year?: number;
  quarter?: string;
  aspekNo?: string;
  query?: string;
...
export interface KPMRKepatuhanGroup {
  aspekNo: string;
  aspekTitle: string;
  aspekBobot: number;
  sections: Array<{
...
export interface UseKpmrKepatuhanReturn {
  // ========== STATE ==========
  aspects: KPMRKepatuhanAspect[];
  questions: KPMRKepatuhanQuestion[];
  definitions: KPMRKepatuhanDefinition[];
  scores: KPMRKepatuhanScore[];
  fullData: KPMRKepatuhanFullDataResponse | null;
  groups: KPMRKepatuhanGroup[];
  periods: Period[];
  years: number[];
  viewYear: number;
  viewQuarter: string;
  ...
  search: (year?: number, query?: string, aspekNo?: string) => Promise<KPMRKepatuhanDefinition[]>;

  // ========== ASPECT CRUD ==========
  createAspect: (data: CreateKPMRKepatuhanAspectData) => Promise<KPMRKepatuhanAspect>;
  updateAspect: (id: number, data: UpdateKPMRKepatuhanAspectData) => Promise<KPMRKepatuhanAspect>;
  deleteAspect: (id: number) => Promise<DeleteResponse>;

  // ========== QUESTION CRUD ==========
  createQuestion: (data: CreateKPMRKepatuhanQuestionData) => Promise<KPMRKepatuhanQuestion>;
  updateQuestion: (id: number, data: UpdateKPMRKepatuhanQuestionData) => Promise<KPMRKepatuhanQuestion>;
  deleteQuestion: (id: number) => Promise<DeleteResponse>;

  // ========== DEFINITION CRUD ==========
  createOrUpdateDefinition: (data: CreateKPMRKepatuhanDefinitionData) => Promise<KPMRKepatuhanDefinition>;
  updateDefinition: (id: number, data: UpdateKPMRKepatuhanDefinitionData) => Promise<KPMRKepatuhanDefinition>;
  deleteDefinition: (definitionId: number, year: number) => Promise<DeleteResponse>;

  // ========== SCORE CRUD ==========
  createOrUpdateScore: (data: CreateKPMRKepatuhanScoreData) => Promise<KPMRKepatuhanScore>;
  updateScore: (id: number, data: UpdateKPMRKepatuhanScoreData) => Promise<KPMRKepatuhanScore>;
...
interface UseKpmrKepatuhanOptions {
  initialYear?: number;
...
export const useKpmrKepatuhan = (options?: UseKpmrKepatuhanOptions): UseKpmrKepatuhanReturn => {
  ...
  const [aspects, setAspects] = useState<KPMRKepatuhanAspect[]>([]);
  const [questions, setQuestions] = useState<KPMRKepatuhanQuestion[]>([]);
  const [definitions, setDefinitions] = useState<KPMRKepatuhanDefinition[]>([]);
  const [scores, setScores] = useState<KPMRKepatuhanScore[]>([]);
  const [fullData, setFullData] = useState<KPMRKepatuhanFullDataResponse | null>(null);
  const [groups, setGroups] = useState<KPMRKepatuhanGroup[]>([]);
  ...
  const fetchAspects = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrKepatuhanApiService.getAllAspects(year));
        setAspects(data);
      } catch (err) {
  ...
  const fetchQuestions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrKepatuhanApiService.getAllQuestions(year));
        setQuestions(data);
      } catch (err) {
  ...
  const fetchDefinitions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        ...
        const data = await withLoading(() => kpmrKepatuhanApiService.getDefinitionsByYear(targetYear));
        setDefinitions(data);
      } catch (err) {
  ...
  const fetchScores = useCallback(
    async (year?: number, quarter?: string): Promise<void> => {
      try {
        const targetYear = year || viewYear;
        const data = await withLoading(() => kpmrKepatuhanApiService.getScoresByPeriod(targetYear, quarter));
        setScores(data);
      } catch (err) {
  ...
  const fetchFullData = useCallback(async (year: number): Promise<void> => {
    ...
    try {
      setLoading(true);
      const data = await kpmrKepatuhanApiService.getFullData(year);
      setFullData(data);
  ...
  const fetchPeriods = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrKepatuhanApiService.getPeriods());
      setPeriods(data);
    } catch (err) {
  ...
  const fetchYears = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrKepatuhanApiService.getAvailableYears());
      setYears(data);
    } catch (err) {
  ...
  const search = useCallback(
    async (year?: number, searchQuery?: string, aspekNo?: string): Promise<KPMRKepatuhanDefinition[]> => {
      try {
        return await withLoading(() => kpmrKepatuhanApiService.searchKPMR(year, searchQuery || query, aspekNo));
      } catch (err) {
  ...
  const createAspect = useCallback(
    async (data: CreateKPMRKepatuhanAspectData): Promise<KPMRKepatuhanAspect> => {
      try {
        const newAspect = await withLoading(() => kpmrKepatuhanApiService.createAspect(data));
  ...
  const updateAspect = useCallback(
    async (id: number, data: UpdateKPMRKepatuhanAspectData): Promise<KPMRKepatuhanAspect> => {
      try {
        const updatedAspect = await withLoading(() => kpmrKepatuhanApiService.updateAspect(id, data));
  ...
  const deleteAspect = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrKepatuhanApiService.deleteAspect(id));
        if (result.success) {
  ...
  const createQuestion = useCallback(
    async (data: CreateKPMRKepatuhanQuestionData): Promise<KPMRKepatuhanQuestion> => {
      try {
        const newQuestion = await withLoading(() => kpmrKepatuhanApiService.createQuestion(data));
  ...
  const updateQuestion = useCallback(
    async (id: number, data: UpdateKPMRKepatuhanQuestionData): Promise<KPMRKepatuhanQuestion> => {
      try {
        const updatedQuestion = await withLoading(() => kpmrKepatuhanApiService.updateQuestion(id, data));
  ...
  const deleteQuestion = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        ...
        const result = await withLoading(() => kpmrKepatuhanApiService.deleteQuestion(id));
  ...
  const createOrUpdateDefinition = useCallback(
    async (data: CreateKPMRKepatuhanDefinitionData): Promise<KPMRKepatuhanDefinition> => {
      try {
        const definition = await withLoading(() => kpmrKepatuhanApiService.createOrUpdateDefinition(data));
  ...
  const updateDefinition = useCallback(
    async (id: number, data: UpdateKPMRKepatuhanDefinitionData): Promise<KPMRKepatuhanDefinition> => {
      try {
        const cleanData: UpdateKPMRKepatuhanDefinitionData = {};
        ...
        const updatedDefinition = await withLoading(() => kpmrKepatuhanApiService.updateDefinition(id, cleanData));
  ...
  const deleteDefinition = useCallback(
    async (definitionId: number, year: number): Promise<DeleteResponse> => {
      try {
        ...
        const response = await withLoading(() => kpmrKepatuhanApiService.deleteDefinitionPermanent(definitionId, year));
  ...
  const createOrUpdateScore = useCallback(
    async (data: CreateKPMRKepatuhanScoreData): Promise<KPMRKepatuhanScore> => {
      try {
        const score = await withLoading(() => kpmrKepatuhanApiService.createOrUpdateScore(data));
  ...
  const updateScore = useCallback(
    async (id: number, data: UpdateKPMRKepatuhanScoreData): Promise<KPMRKepatuhanScore> => {
      try {
        const updatedScore = await withLoading(() => kpmrKepatuhanApiService.updateScore(id, data));
  ...
  const deleteScore = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrKepatuhanApiService.deleteScore(id));
  ...
  const deleteScoreByTarget = useCallback(
    async (definitionId: number, year: number, quarter: string): Promise<DeleteResponse> => {
      try {
        const response = await withLoading(() => kpmrKepatuhanApiService.deleteScoreByTarget(definitionId, year, quarter));
        await fetchScores(year);
        await fetchFullData(year);
        return response;
      } catch (err) {
