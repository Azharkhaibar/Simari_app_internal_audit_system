...
interface UseKepatuhanReturn {
  ...
  sectionsWithIndicators: Array<KepatuhanSection & { indicators: KepatuhanIndikator[]; totalWeighted: number; indicatorCount: number }>;
  periods: Period[];
  viewYear: number;
  viewQuarter: Quarter;
  query: string;
  ...
  // ========== DATA OPERATIONS ==========
  getSections: (isActive?: boolean) => Promise<KepatuhanSection[]>;
  getAllIndikators: () => Promise<KepatuhanIndikator[]>;
  getIndikatorsByPeriod: (year: number, quarter: Quarter) => Promise<KepatuhanIndikator[]>;
  getSectionsWithIndicatorsByPeriod: (year: number, quarter: Quarter) => Promise<Array<KepatuhanSection & { indicators: KepatuhanIndikator[]; totalWeighted: number; indicatorCount: number }>>;
  getPeriods: () => Promise<Period[]>;
  searchIndikators: (query?: string, year?: number, quarter?: Quarter) => Promise<KepatuhanIndikator[]>;
  getAllSections: (isActive?: boolean) => Promise<KepatuhanSection[]>;
...
export const useKepatuhan = (options?: UseKepatuhanOptions): UseKepatuhanReturn => {
  ...
  const [sectionsWithIndicators, setSectionsWithIndicators] = useState<Array<KepatuhanSection & { indicators: KepatuhanIndikator[]; totalWeighted: number; indicatorCount: number }>>([]);
  ...
  // ========== SECTION OPERATIONS ==========
  const getSections = useCallback(async (isActive?: boolean): Promise<KepatuhanSection[]> => {
  ...
  const getAllSections = useCallback(async (isActive?: boolean): Promise<KepatuhanSection[]> => {
    return withLoading(async () => {
      const data = await kepatuhanApiService.getAllSections(isActive);
      return data;
    });
  ...
  const getSectionById = useCallback(async (id: number): Promise<KepatuhanSection> => {
    return withLoading(async () => {
  ...
  const createSection = useCallback(async (data: CreateKepatuhanSectionData): Promise<KepatuhanSection> => {
    return withLoading(async () => {
  ...
  const updateSection = useCallback(async (id: number, data: UpdateKepatuhanSectionData): Promise<KepatuhanSection> => {
    return withLoading(async () => {
  ...
  const deleteSection = useCallback(async (id: number): Promise<DeleteResponse> => {
    return withLoading(async () => {
      const result = await kepatuhanApiService.deleteSection(id);
  ...
  const getAllIndikators = useCallback(async (): Promise<KepatuhanIndikator[]> => {
    return withLoading(async () => {
  ...
  const getIndikatorsByPeriod = useCallback(async (year: number, quarter: Quarter): Promise<KepatuhanIndikator[]> => {
    return withLoading(async () => {
  ...
  const getSectionsWithIndicatorsByPeriod = useCallback(
    async (year: number, quarter: Quarter): Promise<any> => {
      return withLoading(async () => {
        ...
        const response = await kepatuhanApiService.getSectionsWithIndicatorsByPeriod(targetYear, targetQuarter);
  ...
  const searchIndikators = useCallback(async (searchQuery?: string, year?: number, quarter?: Quarter): Promise<KepatuhanIndikator[]> => {
    return withLoading(async () => {
  ...
  const getIndikatorById = useCallback(async (id: number): Promise<KepatuhanIndikator> => {
    return withLoading(async () => {
  ...
  const updateIndikator = useCallback(
    async (id: number, data: UpdateKepatuhanData): Promise<KepatuhanIndikator> => {
      return withLoading(async () => {
  ...
  const deleteIndikator = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      return withLoading(async () => {
        const result = await kepatuhanApiService.deleteIndikator(id);
  ...
  const calculateTotalWeighted = useCallback(async () => {
    try {
      const total = await kepatuhanApiService.getTotalWeightedByPeriod(viewYear, viewQuarter);
  ...
  const getPeriodsWithCounts = useCallback(async (): Promise<(Period & { indicatorCount: number })[]> => {
    return withLoading(async () => {
      const data = await kepatuhanApiService.getPeriodsWithCounts();
      return data;
    });
  }, []);
  ...
  const getIndikatorCount = useCallback(async (year: number, quarter: Quarter): Promise<number> => {
    return withLoading(async () => {
      const count = await kepatuhanApiService.getIndikatorCount(year, quarter);
      return count;
    });
  }, []);
