// src/features/Dashboard/pages/RiskProfile/pages/Kepatuhan/components/KepatuhanInherent.jsx
import React, { useState, useMemo, useEffect, useRef } from 'react';
import { Download, Trash2, Edit3, Search, Plus, ChevronDown } from 'lucide-react';
// import { calculatePeringkat, calculatePeringkatFromText, isNumericRiskLevels } from './utils/kepatuhan/riskcalculator';

import { calculatePeringkat, calculatePeringkatFromText, isNumericRiskLevels } from './utils/riskcalculator';
// ==== Komponen & Utils ====
import DataTable from './components/kepatuhan/new-datatable';
import { RiskField, YearInput, QuarterSelect } from './components/kepatuhan/input-kepatuhan';
import { getCurrentQuarter, getCurrentYear } from './utils/time';
// import { computeWeighted, makeEmptyRow } from './utils/kepatuhan/calc';
// import { exportKepatuhanToExcel } from './utils/kepatuhan/exportExcel';
// import ToastNotification from './components/kpmr-kepatuhan/ToastNotification';
import { computeWeighted, makeEmptyRow } from './utils/calc';
import { exportKepatuhanToExcel } from './utils/exportExcel';
import ToastNotification from './components/kpmr-kepatuhan/ToastNotification';

// ==== Hooks & Services ====
// import { useKepatuhan } from './hooks/kepatuhan/new-kepatuhan.hook';
import { useKepatuhan } from './hooks/kepatuhan/kepatuhan.hook';
...
const kpFallbackEmpty = (year, quarter) => ({
  year,
  quarter,
  no: '',
  subNo: '',
...
export default function KepatuhanInherent({ viewYear, viewQuarter, onViewYearChange, onViewQuarterChange, query, onQueryChange }) {
  ...
  async function KEPATUHAN_addSection() {
    if (!KEPATUHAN_sectionForm.no || !KEPATUHAN_sectionForm.sectionLabel) {
    ...
    try {
      ...
      await logCreate('KEPATUHAN', `Tambah section: ${newSection.parameter} (No: ${newSection.no})`, {
  ...
  async function KEPATUHAN_saveSection() {
    ...
    try {
      await updateSection(KEPATUHAN_sectionForm.id, sectionData);
      setIsAddingNewSection(false);
      setIsEditingSection(false);
      showToast(`Section "${sectionData.parameter}" berhasil diupdate`, 'success');
      await logUpdate('KEPATUHAN', `Update section: ${sectionData.parameter} (No: ${sectionData.no})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: KEPATUHAN_sectionForm.id, no: sectionData.no, parameter: sectionData.parameter, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  async function KEPATUHAN_removeSection(id) {
    ...
    try {
      ...
      await logDelete('KEPATUHAN', `Hapus section: ${sectionToDelete.parameter} (No: ${sectionToDelete.no})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  const kpMakeRow = () =>
    typeof makeEmptyRow === 'function'
      ...
      : kpFallbackEmpty(viewYear, viewQuarter);

  const [KEPATUHAN_form, setKEPATUHAN_form] = useState(kpMakeRow());
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[KEPATUHAN] Peringkat calculation failed:', err);
    }
  }, [
    KEPATUHAN_form?.low,
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isNumericRiskLevels(riskLevels)) {
        ...
        newPeringkat = calculatePeringkatFromText(KEPATUHAN_form.hasilText || '', riskLevels);
      }
  ...
  }, [KEPATUHAN_form?.mode, KEPATUHAN_form?.hasilText, KEPATUHAN_form?.low, KEPATUHAN_form?.lowToModerate, KEPATUHAN_form?.moderate, KEPATUHAN_form?.moderateToHigh, KEPATUHAN_form?.high]);
  ...
  const KEPATUHAN_totalWeighted = useMemo(() => KEPATUHAN_filtered.reduce((sum, r) => sum + (Number(r.weighted || 0) || 0), 0), [KEPATUHAN_filtered]);

  const KEPATUHAN_resetForm = () => {
    setKEPATUHAN_form(kpMakeRow());
  ...
  const buildBaseRow = () => {
    const peringkatNum = KEPATUHAN_form.peringkat === '' || KEPATUHAN_form.peringkat == null ? 0 : Number(KEPATUHAN_form.peringkat);
    ...
    return {
      ...
      numeratorLabel: KEPATUHAN_form.mode === 'NILAI_TUNGGAL' || KEPATUHAN_form.mode === 'TEKS' ? '' : KEPATUHAN_form.numeratorLabel,
      numeratorValue: KEPATUHAN_form.mode === 'NILAI_TUNGGAL' || KEPATUHAN_form.mode === 'TEKS' ? '' : KEPATUHAN_form.numeratorValue,
    };
  };

  const KEPATUHAN_addRow = async () => {
    if (!KEPATUHAN_sectionForm.id) {
    ...
    const section = currentPeriodSections.find((s) => s.id === KEPATUHAN_sectionForm.id);

    if (!section) return;

    const weightedAuto = computeWeightedLocal(baseRow.bobotSection, baseRow.bobotIndikator, baseRow.peringkat);
    const backendData = transformToBackend(baseRow, viewYear, viewQuarter, KEPATUHAN_sectionForm.id, section);
    ...
    try {
      ...
      await logCreate('KEPATUHAN', `Tambah indikator: ${backendData.indikator} (Section: ${section.parameter})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: KEPATUHAN_sectionForm.id, subNo: backendData.subNo, indikator: backendData.indikator, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const KEPATUHAN_saveEdit = async () => {
    ...
    const updateData = {
      year: viewYear,
      quarter: viewQuarter,
      sectionId: KEPATUHAN_sectionForm.id,
      subNo: baseRow.subNo,
    ...
    try {
      await updateIndikator(KEPATUHAN_editingRow.id, updateData);
      KEPATUHAN_resetForm();
      setShowKepatuhanForm(false);
      showToast('Indikator berhasil diupdate!', 'success');
      await logUpdate('KEPATUHAN', `Update indikator: ${updateData.indikator} (ID: ${KEPATUHAN_editingRow.id})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', indicatorId: KEPATUHAN_editingRow.id, subNo: updateData.subNo, indikator: updateData.indikator, peringkat: updateData.peringkat, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const KEPATUHAN_removeRow = async (row) => {
    if (!row) return;
    ...
    try {
      ...
      if (KEPATUHAN_editingRow?.id === row.id) KEPATUHAN_resetForm();
      showToast(`Indikator "${row.indikator}" berhasil dihapus`, 'success');
      await logDelete('KEPATUHAN', `Hapus indikator: ${row.indikator} (Sub No: ${row.subNo})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  const KEPATUHAN_exportExcel = async () => {
    try {
      exportKepatuhanToExcel(KEPATUHAN_filtered, viewYear, viewQuarter);
      showToast(`Data Kepatuhan tahun ${viewYear} triwulan ${viewQuarter} berhasil diexport`, 'success');
      await logExport('KEPATUHAN', `Export data Kepatuhan ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', year: viewYear, quarter: viewQuarter, dataCount: KEPATUHAN_filtered.length }
      });
    } catch (err) {
      showToast('Gagal mengexport data', 'error');
      await logExport('KEPATUHAN', `Gagal export Kepatuhan ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: false,
      ...
      <header className="px-4 py-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex items-end gap-2">
            <button onClick={() => setShowKepatuhanForm(true)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 font-semibold">
              + Tambah Data
            </button>

            <button onClick={KEPATUHAN_exportExcel} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
          ...
          <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-white font-semibold text-lg sm:text-xl">Form Section – Kepatuhan</h2>
              <button onClick={() => setShowKepatuhanForm(false)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-white/20 hover:bg-white/30 text-white font-bold" title="Tutup Form">
                ✕
              </button>
            </div>
            ...
            <div className="flex items-end gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setKEPATUHAN_sectionForm((f) => ({ ...f, no: e.target.value }))}
                />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setKEPATUHAN_sectionForm((f) => ({ ...f, bobotSection: e.target.value }))}
                />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) => setKEPATUHAN_sectionForm((f) => ({ ...f, sectionLabel: e.target.value }))}
              ...
              <div className="flex gap-2 self-end">
                {!isAddingNewSection && !isEditingSection && (
                  <>
                    ...
                    <button className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center" onClick={() => KEPATUHAN_removeSection(KEPATUHAN_sectionForm.id)} title="Hapus Section">
                      <Trash2 size={20} />
                    </button>
                  </>
                )}
                ...
                {(isAddingNewSection || isEditingSection) && (
                  <>
                    <button
                      ...
                      onClick={isAddingNewSection ? KEPATUHAN_addSection : KEPATUHAN_saveSection}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setIsEditingSection(false);
                        if (KEPATUHAN_sectionForm.id) {
                          KEPATUHAN_selectSection(KEPATUHAN_sectionForm.id);
                        }
                      }}
                      title="Batal"
                    >
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (val === 'ADD_NEW_SECTION') {
                    ...
                    setKEPATUHAN_sectionForm({ id: 0, no: '', bobotSection: 0, sectionLabel: '' });
                    return;
                  }
                  if (val) {
                    setIsAddingNewSection(false);
                    setIsEditingSection(false);
                    KEPATUHAN_selectSection(Number(val));
                  }
                }}
              >
          ...
          <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-2">
                ...
                <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={KEPATUHAN_form.subNo} onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, subNo: e.target.value }))} placeholder="1.1" />
              </div>
              <div className="col-span-7">
                ...
                <input
                  ...
                  onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, indikator: e.target.value }))}
              ...
              <div className="col-span-3">
                ...
                <input
                  ...
                  onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, bobotIndikator: e.target.value }))}
                  placeholder="50"
                />
              </div>
            </div>
                ...
                {KEPATUHAN_form.mode === 'TEKS' ? (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, hasilText: e.target.value }))}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        type="text"
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                        placeholder={KEPATUHAN_form.mode === 'RASIO' ? 'Contoh default: pemb / peny — atau rumus custom (pemb, peny)' : 'Contoh default: peny — atau rumus custom (peny / 1000)'}
                        value={KEPATUHAN_form.formula || ''}
                        onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, formula: e.target.value }))}
                      />
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={KEPATUHAN_form.isPercent || false}
                          onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, isPercent: e.target.checked }))}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, numeratorLabel: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setKEPATUHAN_form((f) => ({ ...f, numeratorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, denominatorLabel: e.target.value }))}
                    placeholder={KEPATUHAN_form.mode === 'RASIO' ? 'Total Asset (Jutaan)' : 'Jumlah kejadian, jumlah kasus, dll.'}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setKEPATUHAN_form((f) => ({ ...f, denominatorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      if (!val || val.endsWith(',') || val.endsWith('.') || val === '-') return;
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-6 space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, sumberRisiko: e.target.value }))}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, dampak: e.target.value }))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="number" className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-100 font-bold text-center" value={KEPATUHAN_form.peringkat ?? ''} readOnly />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                        value={(() => {
                          if (KEPATUHAN_form.mode === 'TEKS') return KEPATUHAN_form.hasilText || '';
                          ...
                          if (KEPATUHAN_form.mode === 'NILAI_TUNGGAL') return fmtNumber(raw);
                  ...
                  <div>
                    ...
                    <textarea rows={3} className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white" value={KEPATUHAN_form.keterangan} onChange={(e) => setKEPATUHAN_form((f) => ({ ...f, keterangan: e.target.value }))} />
                  </div>
                </div>
                ...
                <div className="col-span-6 grid grid-cols-1 gap-3 items-stretch">
                  <RiskField
                    ...
                    onChange={(v) => setKEPATUHAN_form((f) => ({ ...f, low: v }))}
                    color="#4A5A2C"
                    textColor="#ffffff"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setKEPATUHAN_form((f) => ({ ...f, lowToModerate: v }))}
                    color="#A6D86C"
                    textColor="#0B2545"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setKEPATUHAN_form((f) => ({ ...f, moderate: v }))}
                    color="#FFFF00"
                    textColor="#4B3A00"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setKEPATUHAN_form((f) => ({ ...f, moderateToHigh: v }))}
                    color="#FAD2A7"
                    textColor="#5A2E00"
                  ...
                  <RiskField
                    ...
                    onChange={(v) => setKEPATUHAN_form((f) => ({ ...f, high: v }))}
                    color="#E57373"
                    textColor="#FFFFFF"
            ...
            <div className="mt-6 flex justify-end">
              {!KEPATUHAN_editingRow ? (
                <button onClick={KEPATUHAN_addRow} className="px-8 py-3 rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold">
                  + Tambah
                </button>
              ) : (
                <div className="flex gap-3">
                  <button onClick={KEPATUHAN_saveEdit} className="px-8 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">
                    Simpan
                  </button>
                  <button onClick={KEPATUHAN_resetForm} className="px-8 py-3 rounded-lg border-2 border-gray-300 hover:bg-gray-50 font-semibold">
                    Batal
                  </button>
                </div>
              )}
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
          <div className="relative h-[350px]">
            <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
              <DataTable rows={KEPATUHAN_filtered} totalWeighted={KEPATUHAN_totalWeighted} viewYear={viewYear} viewQuarter={viewQuarter} startEdit={KEPATUHAN_startEdit} removeRow={KEPATUHAN_removeRow} />
            </div>
          </div>
        </div>
      </section>
