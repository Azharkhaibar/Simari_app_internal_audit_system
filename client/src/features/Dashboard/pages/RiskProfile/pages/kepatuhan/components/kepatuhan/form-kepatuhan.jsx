...
import { computeWeighted } from '../../utils/kepatuhan/calc';
...
export default function FormKepatuhan({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Kepatuhan', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('KEPATUHAN', `Menambahkan data kepatuhan baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/kepatuhan',
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
            indikator: form.indikator,
            kepatuhan_data: serviceData,
          },
        });
      ...
      onReset();
      alert('Data kepatuhan berhasil ditambahkan!');
    } catch (error) {
      console.error('Error adding kepatuhan:', error);

      if (logCreate && typeof logCreate === 'function') {
        await logCreate('KEPATUHAN', `Gagal menambahkan data kepatuhan - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/kepatuhan',
          isSuccess: false,
          metadata: {
            error: error.message,
      ...
      alert('Gagal menambahkan data kepatuhan: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('KEPATUHAN', `Memperbarui data kepatuhan - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/kepatuhan/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
      ...
      alert('Data kepatuhan berhasil diperbarui!');
    } catch (error) {
      console.error('Error saving kepatuhan:', error);

      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('KEPATUHAN', `Gagal memperbarui data kepatuhan - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/kepatuhan/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: {
            error: error.message,
          },
      ...
      alert('Gagal memperbarui data kepatuhan: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data kepatuhan dan analisis risiko secara komprehensif</p>
            </div>
          </div>
