...
import { computeWeighted } from '../../utils/operasional/calc';
...
export default function FormOperasional({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Operasional', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('OPERASIONAL', `Menambahkan data operasional baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/operasional',
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
            indikator: form.indikator,
            operasional_data: serviceData,
          },
        });
      ...
      console.error('Error adding operasional:', error);

      if (logCreate && typeof logCreate === 'function') {
        await logCreate('OPERASIONAL', `Gagal menambahkan data operasional - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/operasional',
          isSuccess: false,
          metadata: {
            error: error.message,
      ...
      alert('Gagal menambahkan data operasional: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('OPERASIONAL', `Memperbarui data operasional - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/operasional/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
      ...
      console.error('Error saving operasional:', error);

      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('OPERASIONAL', `Gagal memperbarui data operasional - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/operasional/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: {
            error: error.message,
          },
      ...
      alert('Gagal memperbarui data operasional: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data operasional dan analisis risiko secara komprehensif</p>
            </div>
          </div>
