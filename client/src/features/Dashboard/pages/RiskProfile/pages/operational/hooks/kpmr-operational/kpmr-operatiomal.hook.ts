// src/features/Dashboard/pages/RiskProfile/pages/Operasional/hooks/KPMR/kpmr-operasional.hook.ts

import { useState, useEffect, useCallback } from 'react';
import {
  kpmrOperasionalApiService,
  KPMROperasionalAspect,
  KPMROperasionalQuestion,
  KPMROperasionalDefinition,
  KPMROperasionalScore,
  KPMROperasionalFullDataResponse,
  CreateKPMROperasionalAspectData,
  UpdateKPMROperasionalAspectData,
  CreateKPMROperasionalQuestionData,
  UpdateKPMROperasionalQuestionData,
  CreateKPMROperasionalDefinitionData,
  UpdateKPMROperasionalDefinitionData,
  CreateKPMROperasionalScoreData,
  UpdateKPMROperasionalScoreData,
  Period,
  DeleteResponse,
  transformFullDataToGroups,
} from '../../services/kpmr-operational/kpmr-operational.service';

// ===================== INTERFACES =====================

export interface KPMROperasionalFilters {
  year?: number;
  quarter?: string;
  aspekNo?: string;
  query?: string;
...
export interface KPMROperasionalGroup {
  aspekNo: string;
  aspekTitle: string;
  aspekBobot: number;
  sections: Array<{
...
export interface UseKpmrOperasionalReturn {
  // ========== STATE ==========
  aspects: KPMROperasionalAspect[];
  questions: KPMROperasionalQuestion[];
  definitions: KPMROperasionalDefinition[];
  scores: KPMROperasionalScore[];
  fullData: KPMROperasionalFullDataResponse | null;
  groups: KPMROperasionalGroup[];
  periods: Period[];
  years: number[];
  viewYear: number;
  viewQuarter: string;
  ...
  search: (year?: number, query?: string, aspekNo?: string) => Promise<KPMROperasionalDefinition[]>;

  // ========== ASPECT CRUD ==========
  createAspect: (data: CreateKPMROperasionalAspectData) => Promise<KPMROperasionalAspect>;
  updateAspect: (id: number, data: UpdateKPMROperasionalAspectData) => Promise<KPMROperasionalAspect>;
  deleteAspect: (id: number) => Promise<DeleteResponse>;

  // ========== QUESTION CRUD ==========
  createQuestion: (data: CreateKPMROperasionalQuestionData) => Promise<KPMROperasionalQuestion>;
  updateQuestion: (id: number, data: UpdateKPMROperasionalQuestionData) => Promise<KPMROperasionalQuestion>;
  deleteQuestion: (id: number) => Promise<DeleteResponse>;

  // ========== DEFINITION CRUD ==========
  createOrUpdateDefinition: (data: CreateKPMROperasionalDefinitionData) => Promise<KPMROperasionalDefinition>;
  updateDefinition: (id: number, data: UpdateKPMROperasionalDefinitionData) => Promise<KPMROperasionalDefinition>;
  deleteDefinition: (definitionId: number, year: number) => Promise<DeleteResponse>;

  // ========== SCORE CRUD ==========
  createOrUpdateScore: (data: CreateKPMROperasionalScoreData) => Promise<KPMROperasionalScore>;
  updateScore: (id: number, data: UpdateKPMROperasionalScoreData) => Promise<KPMROperasionalScore>;
...
interface UseKpmrOperasionalOptions {
  initialYear?: number;
...
export const useKpmrOperasional = (options?: UseKpmrOperasionalOptions): UseKpmrOperasionalReturn => {
  ...
  const [aspects, setAspects] = useState<KPMROperasionalAspect[]>([]);
  const [questions, setQuestions] = useState<KPMROperasionalQuestion[]>([]);
  const [definitions, setDefinitions] = useState<KPMROperasionalDefinition[]>([]);
  const [scores, setScores] = useState<KPMROperasionalScore[]>([]);
  const [fullData, setFullData] = useState<KPMROperasionalFullDataResponse | null>(null);
  const [groups, setGroups] = useState<KPMROperasionalGroup[]>([]);
  ...
  const fetchAspects = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrOperasionalApiService.getAllAspects(year));
        setAspects(data);
      } catch (err) {
  ...
  const fetchQuestions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        const data = await withLoading(() => kpmrOperasionalApiService.getAllQuestions(year));
        setQuestions(data);
      } catch (err) {
  ...
  const fetchDefinitions = useCallback(
    async (year?: number): Promise<void> => {
      try {
        ...
        const data = await withLoading(() => kpmrOperasionalApiService.getDefinitionsByYear(targetYear));
        setDefinitions(data);
      } catch (err) {
  ...
  const fetchScores = useCallback(
    async (year?: number, quarter?: string): Promise<void> => {
      try {
        const targetYear = year || viewYear;
        const data = await withLoading(() => kpmrOperasionalApiService.getScoresByPeriod(targetYear, quarter));
        setScores(data);
      } catch (err) {
  ...
  const fetchFullData = useCallback(async (year: number): Promise<void> => {
    ...
    try {
      setLoading(true);
      const data = await kpmrOperasionalApiService.getFullData(year);
      setFullData(data);
  ...
  const fetchPeriods = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrOperasionalApiService.getPeriods());
      setPeriods(data);
    } catch (err) {
  ...
  const fetchYears = useCallback(async (): Promise<void> => {
    try {
      const data = await withLoading(() => kpmrOperasionalApiService.getAvailableYears());
      setYears(data);
    } catch (err) {
  ...
  const search = useCallback(
    async (year?: number, searchQuery?: string, aspekNo?: string): Promise<KPMROperasionalDefinition[]> => {
      try {
        return await withLoading(() => kpmrOperasionalApiService.searchKPMR(year, searchQuery || query, aspekNo));
      } catch (err) {
  ...
  const createAspect = useCallback(
    async (data: CreateKPMROperasionalAspectData): Promise<KPMROperasionalAspect> => {
      try {
        const newAspect = await withLoading(() => kpmrOperasionalApiService.createAspect(data));
  ...
  const updateAspect = useCallback(
    async (id: number, data: UpdateKPMROperasionalAspectData): Promise<KPMROperasionalAspect> => {
      try {
        const updatedAspect = await withLoading(() => kpmrOperasionalApiService.updateAspect(id, data));
  ...
  const deleteAspect = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrOperasionalApiService.deleteAspect(id));
        if (result.success) {
  ...
  const createQuestion = useCallback(
    async (data: CreateKPMROperasionalQuestionData): Promise<KPMROperasionalQuestion> => {
      try {
        const newQuestion = await withLoading(() => kpmrOperasionalApiService.createQuestion(data));
  ...
  const updateQuestion = useCallback(
    async (id: number, data: UpdateKPMROperasionalQuestionData): Promise<KPMROperasionalQuestion> => {
      try {
        const updatedQuestion = await withLoading(() => kpmrOperasionalApiService.updateQuestion(id, data));
  ...
  const deleteQuestion = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        ...
        const result = await withLoading(() => kpmrOperasionalApiService.deleteQuestion(id));
  ...
  const createOrUpdateDefinition = useCallback(
    async (data: CreateKPMROperasionalDefinitionData): Promise<KPMROperasionalDefinition> => {
      try {
        const definition = await withLoading(() => kpmrOperasionalApiService.createOrUpdateDefinition(data));
  ...
  const updateDefinition = useCallback(
    async (id: number, data: UpdateKPMROperasionalDefinitionData): Promise<KPMROperasionalDefinition> => {
      try {
        const cleanData: UpdateKPMROperasionalDefinitionData = {};
        ...
        const updatedDefinition = await withLoading(() => kpmrOperasionalApiService.updateDefinition(id, cleanData));
  ...
  const deleteDefinition = useCallback(
    async (definitionId: number, year: number): Promise<DeleteResponse> => {
      try {
        ...
        const response = await withLoading(() => kpmrOperasionalApiService.deleteDefinitionPermanent(definitionId, year));
  ...
  const createOrUpdateScore = useCallback(
    async (data: CreateKPMROperasionalScoreData): Promise<KPMROperasionalScore> => {
      try {
        const score = await withLoading(() => kpmrOperasionalApiService.createOrUpdateScore(data));
  ...
  const updateScore = useCallback(
    async (id: number, data: UpdateKPMROperasionalScoreData): Promise<KPMROperasionalScore> => {
      try {
        const updatedScore = await withLoading(() => kpmrOperasionalApiService.updateScore(id, data));
  ...
  const deleteScore = useCallback(
    async (id: number): Promise<DeleteResponse> => {
      try {
        const result = await withLoading(() => kpmrOperasionalApiService.deleteScore(id));
  ...
  const deleteScoreByTarget = useCallback(
    async (definitionId: number, year: number, quarter: string): Promise<DeleteResponse> => {
      try {
        const response = await withLoading(() => kpmrOperasionalApiService.deleteScoreByTarget(definitionId, year, quarter));
        await fetchScores(year);
        await fetchFullData(year);
        return response;
      } catch (err) {
