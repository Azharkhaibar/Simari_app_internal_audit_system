// src/features/Dashboard/pages/RiskProfile/pages/Repository/risk-profile-repository-holding.component.jsx
...
import { useRiskProfileRepository } from '../hooks/riskprofilerepository.hook';

// ==================== CONSTANTS ====================
const MODULE_ORDER = ['INVESTASI', 'PASAR', 'LIKUIDITAS', 'OPERASIONAL', 'HUKUM', 'STRATEGIK', 'KEPATUHAN', 'REPUTASI'];
const HOLDING_MODULES = ['INVESTASI', 'PASAR', 'LIKUIDITAS', 'OPERASIONAL', 'HUKUM', 'STRATEGIK', 'KEPATUHAN', 'REPUTASI'];
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
      ...
      <p className="text-gray-600 max-w-sm mx-auto mb-6">
        {hasFilters ? 'Tidak ada data Holding yang sesuai dengan filter atau pencarian Anda.' : `Tidak ada data Holding tersedia untuk periode ${year} ${quarter}.`}
      </p>
      <div className="flex justify-center gap-3">
        ...
        <button onClick={() => window.location.reload()} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Refresh Halaman</button>
      </div>
    </div>
  );
};
...
const FilterModal = ({ isOpen, onClose, selectedModules, onToggleModule, onSelectAll, onClearAll }) => {
  ...
  const availableModules = HOLDING_MODULES;
      ...
      <div className="bg-white rounded-xl w-full max-w-md shadow-xl">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Filter Modul Holding</h3>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <button onClick={onClose} className="w-full px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Terapkan Filter</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="mb-8 animate-fadeIn">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <button onClick={onToggleExpand} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${isExpanded ? 'bg-blue-100 text-blue-600 hover:bg-blue-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onToggleExpand} className={`p-2 rounded-lg transition-colors flex items-center gap-2 ${isExpanded ? 'bg-blue-100 text-blue-600 hover:bg-blue-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              ...
              <table className="min-w-[1450px] text-sm border border-gray-300 border-collapse">
                <thead>
                  <tr className="bg-[#1e40af] text-white">
                    ...
                    <th className="border border-black px-3 py-2 bg-[#1e40af]" rowSpan={2} style={{ width: 100 }}>Hasil</th>
                    <th className="border border-black px-3 py-2 bg-[#1e40af]" rowSpan={2} style={{ width: 70 }}>Peringkat</th>
                    <th className="border border-black px-3 py-2 bg-[#1e40af] text-white" rowSpan={2} style={{ width: 90 }}>Weighted</th>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className="border border-gray-400 text-white font-semibold text-center bg-[#1e3a5f]" colSpan={2}>Summary</td>
...
// ==================== MAIN COMPONENT ====================
export const RiskProfileRepositoryHolding = () => {
  ...
  const { data, loading, error, fetchRepositoryData, setYearFilter, setQuarterFilter, setModuleTypesFilter, setSearchFilter, resetFilters, setPageSize } = useRiskProfileRepository({
    initialFilters: { year: new Date().getFullYear(), quarter: 'Q1', moduleTypes: HOLDING_MODULES },
  ...
  const groupedByModule = useMemo(() => {
    const grouped = {};
    const modulesToShow = selectedModules.length > 0 ? selectedModules.filter(m => HOLDING_MODULES.includes(m)) : [...HOLDING_MODULES];
  ...
  const totalWeighted = useMemo(() => HOLDING_MODULES.reduce((t, m) => t + (groupedByModule[m] || []).reduce((s, i) => s + (Number(i.weighted) || 0), 0), 0), [groupedByModule]);
  const modulesWithData = useMemo(() => HOLDING_MODULES.filter(m => groupedByModule[m] && groupedByModule[m].length > 0), [groupedByModule]);
  const totalIndicators = useMemo(() => HOLDING_MODULES.reduce((a, m) => a + (groupedByModule[m]?.length || 0), 0), [groupedByModule]);
  ...
  const handleModuleToggle = useCallback((m) => {
    ...
    setSelectedModules(next); setModuleTypesFilter(next.length > 0 ? next : HOLDING_MODULES); setExpandedModule(null);
  }, [selectedModules, setModuleTypesFilter]);
  const handleSelectAll = useCallback(() => { setSelectedModules([...HOLDING_MODULES]); setModuleTypesFilter([...HOLDING_MODULES]); }, [setModuleTypesFilter]);
  const handleClearAll = useCallback(() => { setSelectedModules([]); setModuleTypesFilter(HOLDING_MODULES); }, [setModuleTypesFilter]);
  ...
  const handleRefresh = useCallback(async () => { try { setPageSize(0); await fetchRepositoryData(); } catch (e) { console.error(e); } }, [fetchRepositoryData, setPageSize]);
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl shadow-lg text-white p-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/20 rounded-xl"><FileText className="w-6 h-6" /></div>
            <div><h1 className="text-2xl font-bold">Risk Profile Repository</h1><p className="text-blue-100 text-sm">Holding - Data modul Investasi, Pasar, Likuiditas, Operasional, Hukum, Strategik, Kepatuhan, dan Reputasi</p></div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mt-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowFilterModal(true)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 bg-blue-500/80 text-white font-semibold hover:bg-blue-600 h-[46px] min-w-[120px] justify-center">
      ...
      {loading && <div className="p-3 bg-blue-100 border border-blue-400 text-blue-700 rounded flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Memuat data repository...</div>}

      <div className="p-4 border border-gray-200 rounded-lg bg-gray-50">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3"><Filter size={16} className="text-gray-600" /><span className="text-sm font-medium text-gray-700">Filter Modul Holding:</span></div>
          <button onClick={() => setShowFilterModal(true)} className="text-sm text-blue-600 hover:text-blue-800">Buka Filter Modal</button>
        </div>
        <div className="flex flex-wrap gap-2">
          {HOLDING_MODULES.map(key => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white border rounded-xl p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Total Modul</p><p className="text-2xl font-bold mt-1">{modulesWithData.length}</p></div><div className="text-blue-600 bg-blue-50 p-2 rounded-lg">📊</div></div></div>
      ...
      {expandedModule && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
          <div className="flex items-center gap-2"><Maximize2 size={16} className="text-blue-600" /><span className="text-blue-800 font-medium">Preview: <span className="font-bold" style={{ color: MODULE_COLORS[expandedModule] }}>{MODULE_NAMES[expandedModule]}</span></span></div>
          <button onClick={() => setExpandedModule(null)} className="text-blue-600 hover:text-blue-800 text-sm font-medium">Close Preview</button>
        </div>
      )}
...
export default RiskProfileRepositoryHolding;