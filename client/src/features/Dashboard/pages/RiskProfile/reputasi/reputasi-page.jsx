// src/features/Dashboard/pages/RiskProfile/pages/Reputasi/components/ReputasiInherent.jsx
import React, { useState, useMemo, useEffect, useRef } from 'react';
import { Download, Trash2, Edit3, Search, Plus, ChevronDown } from 'lucide-react';
// import { calculatePeringkat, calculatePeringkatFromText, isNumericRiskLevels } from './utils/reputasi/riskcalculator';

import { calculatePeringkat, calculatePeringkatFromText, isNumericRiskLevels } from './utils/riskcalculator';
// ==== Komponen & Utils ====
import DataTable from './components/reputasi/datatable-reputasi';
import { RiskField, YearInput, QuarterSelect } from './components/reputasi/input-reputasi';
import { getCurrentQuarter, getCurrentYear } from './utils/time';
// import { computeWeighted, makeEmptyRow } from './utils/reputasi/calc';
// import { exportReputasiToExcel } from './utils/reputasi/exportExcel';
// import ToastNotification from './components/kpmr-reputasi/ToastNotification';
import { computeWeighted, makeEmptyRow } from './utils/calc';
import { exportReputasiToExcel } from './utils/exportExcel';
import ToastNotification from './components/kpmr-reputasi/ToastNotification';

// ==== Hooks & Services ====
// import { useReputasi } from './hooks/reputasi/new-reputasi.hook';
import { useReputasi } from './hooks/reputasi/reputasi.hook';
...
// ===================== Reputasi: fallback empty row =====================
const rpFallbackEmpty = (year, quarter) => ({
  year,
  quarter,
  no: '',
  subNo: '',
...
const computeReputasiHasil = (row) => {
...
export default function ReputasiInherent({ viewYear, viewQuarter, onViewYearChange, onViewQuarterChange, query, onQueryChange }) {
  const [showReputasiForm, setShowReputasiForm] = useState(false);
  ...
  // ----- SECTION LIST -----
  const [REPUTASI_sectionForm, setREPUTASI_sectionForm] = useState({
    id: 0,
    no: '',
    bobotSection: 0,
    sectionLabel: '',
  ...
  function REPUTASI_selectSection(id) {
    const s = currentPeriodSections.find((x) => x.id === id);
    if (s) {
      setREPUTASI_sectionForm({
        id: s.id,
        no: s.no,
  ...
  async function REPUTASI_addSection() {
    if (!REPUTASI_sectionForm.no || !REPUTASI_sectionForm.sectionLabel) {
      showToast('No Section dan Section Label harus diisi!', 'error');
      return;
    }

    const sectionData = {
      no: REPUTASI_sectionForm.no,
      bobotSection: Number(REPUTASI_sectionForm.bobotSection || 0),
      parameter: REPUTASI_sectionForm.sectionLabel,
      year: viewYear,
      quarter: viewQuarter,
      isActive: true,
    };
    ...
    try {
      ...
      await logCreate('REPUTASI', `Tambah section: ${newSection.parameter} (No: ${newSection.no})`, {
  ...
  async function REPUTASI_saveSection() {
    if (!REPUTASI_sectionForm.id) return;
    ...
    try {
      await updateSection(REPUTASI_sectionForm.id, sectionData);
      setIsAddingNewSection(false);
      setIsEditingSection(false);
      showToast(`Section "${sectionData.parameter}" berhasil diupdate`, 'success');
      await logUpdate('REPUTASI', `Update section: ${sectionData.parameter} (No: ${sectionData.no})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: REPUTASI_sectionForm.id, no: sectionData.no, parameter: sectionData.parameter, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  async function REPUTASI_removeSection(id) {
    const sectionToDelete = currentPeriodSections.find((s) => s.id === id);

    if (!sectionToDelete) {
      console.warn('[REPUTASI_removeSection] Section not found:', id);
    ...
    try {
      ...
      await logDelete('REPUTASI', `Hapus section: ${sectionToDelete.parameter} (No: ${sectionToDelete.no})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  const rpMakeRow = () =>
    typeof makeEmptyRow === 'function'
      ...
      : rpFallbackEmpty(viewYear, viewQuarter);

  const [REPUTASI_form, setREPUTASI_form] = useState(rpMakeRow());
  const [REPUTASI_editingRow, setREPUTASI_editingRow] = useState(null);
  ...
  useEffect(() => {
    if (REPUTASI_form?.mode === 'TEKS') return;
    if (!REPUTASI_form || !REPUTASI_sectionForm) return;

    try {
      const baseRow = {
        ...REPUTASI_form,
        no: REPUTASI_sectionForm.no || '',
        sectionLabel: REPUTASI_sectionForm.sectionLabel || '',
        bobotSection: REPUTASI_sectionForm.bobotSection || 0,
      };

      const hasilNum = computeReputasiHasil(baseRow);

      if (hasilNum !== '' && hasilNum != null) {
        const newPeringkat = calculatePeringkat(
          hasilNum,
          {
            low: REPUTASI_form.low || '',
            lowToModerate: REPUTASI_form.lowToModerate || '',
            moderate: REPUTASI_form.moderate || '',
            moderateToHigh: REPUTASI_form.moderateToHigh || '',
            high: REPUTASI_form.high || '',
          },
          REPUTASI_form.isPercent || false,
        );
        if (REPUTASI_form.peringkat !== newPeringkat) {
          setREPUTASI_form((prev) => ({ ...prev, peringkat: newPeringkat }));
        }
      }
    } catch (err) {
      console.warn('[REPUTASI] Peringkat calculation failed:', err);
    }
  }, [
    REPUTASI_form?.low,
    REPUTASI_form?.lowToModerate,
    REPUTASI_form?.moderate,
    REPUTASI_form?.moderateToHigh,
    REPUTASI_form?.numeratorValue,
    REPUTASI_form?.denominatorValue,
    REPUTASI_form?.formula,
    REPUTASI_form?.isPercent,
    REPUTASI_form?.mode,
    REPUTASI_sectionForm?.bobotSection,
  ]);

  // Auto-calculate untuk TEKS mode
  useEffect(() => {
    if (REPUTASI_form?.mode !== 'TEKS') return;
    if (!REPUTASI_form) return;
    ...
    try {
      ...
      if (isNumericRiskLevels(riskLevels)) {
        const hasilNum = parseFloat(REPUTASI_form.hasilText);
        if (!isNaN(hasilNum)) {
          newPeringkat = calculatePeringkat(hasilNum, riskLevels, true);
        }
      } else {
        newPeringkat = calculatePeringkatFromText(REPUTASI_form.hasilText || '', riskLevels);
      }
      ...
      console.warn('[REPUTASI] TEKS peringkat calculation failed:', err);
    }
  }, [REPUTASI_form?.mode, REPUTASI_form?.hasilText, REPUTASI_form?.low, REPUTASI_form?.lowToModerate, REPUTASI_form?.moderate, REPUTASI_form?.moderateToHigh, REPUTASI_form?.high]);
  ...
  const REPUTASI_filtered = useMemo(() => {
  ...
  const REPUTASI_totalWeighted = useMemo(() => REPUTASI_filtered.reduce((sum, r) => sum + (Number(r.weighted || 0) || 0), 0), [REPUTASI_filtered]);

  const REPUTASI_resetForm = () => {
    setREPUTASI_form(rpMakeRow());
    setREPUTASI_editingRow(null);
    setRawNumeratorInput('');
    setRawDenominatorInput('');
  };

  const buildBaseRow = () => {
    const peringkatNum = REPUTASI_form.peringkat === '' || REPUTASI_form.peringkat == null ? 0 : Number(REPUTASI_form.peringkat);

    return {
      ...REPUTASI_form,
      no: REPUTASI_sectionForm.no,
      sectionLabel: REPUTASI_sectionForm.sectionLabel,
      bobotSection: REPUTASI_sectionForm.bobotSection,
      peringkat: peringkatNum,
      numeratorLabel: REPUTASI_form.mode === 'NILAI_TUNGGAL' || REPUTASI_form.mode === 'TEKS' ? '' : REPUTASI_form.numeratorLabel,
      numeratorValue: REPUTASI_form.mode === 'NILAI_TUNGGAL' || REPUTASI_form.mode === 'TEKS' ? '' : REPUTASI_form.numeratorValue,
    };
  };

  const REPUTASI_addRow = async () => {
    if (!REPUTASI_sectionForm.id) {
      showToast('Pilih section terlebih dahulu!', 'error');
      return;
    }

    const baseRow = buildBaseRow();
    const rawHasil = computeReputasiHasil(baseRow);
    const section = currentPeriodSections.find((s) => s.id === REPUTASI_sectionForm.id);

    if (!section) return;

    const weightedAuto = computeWeightedLocal(baseRow.bobotSection, baseRow.bobotIndikator, baseRow.peringkat);
    const backendData = transformToBackend(baseRow, viewYear, viewQuarter, REPUTASI_sectionForm.id, section);

    try {
      await createIndikator(backendData);
      REPUTASI_resetForm();
      setShowReputasiForm(false);
      showToast('Indikator berhasil ditambahkan!', 'success');
      await logCreate('REPUTASI', `Tambah indikator: ${backendData.indikator} (Section: ${section.parameter})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', sectionId: REPUTASI_sectionForm.id, subNo: backendData.subNo, indikator: backendData.indikator, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const REPUTASI_startEdit = (row) => {
    if (!row) return;
    setShowReputasiForm(true);

    setREPUTASI_editingRow(row);
  ...
  const REPUTASI_saveEdit = async () => {
    if (!REPUTASI_editingRow) return;
    ...
    const updateData = {
      year: viewYear,
      quarter: viewQuarter,
      sectionId: REPUTASI_sectionForm.id,
      subNo: baseRow.subNo,
    ...
    try {
      await updateIndikator(REPUTASI_editingRow.id, updateData);
      REPUTASI_resetForm();
      setShowReputasiForm(false);
      showToast('Indikator berhasil diupdate!', 'success');
      await logUpdate('REPUTASI', `Update indikator: ${updateData.indikator} (ID: ${REPUTASI_editingRow.id})`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', indicatorId: REPUTASI_editingRow.id, subNo: updateData.subNo, indikator: updateData.indikator, peringkat: updateData.peringkat, year: viewYear, quarter: viewQuarter }
      });
    } catch (err) {
  ...
  const REPUTASI_removeRow = async (row) => {
    if (!row) return;
    ...
    try {
      ...
      if (REPUTASI_editingRow?.id === row.id) REPUTASI_resetForm();
      showToast(`Indikator "${row.indikator}" berhasil dihapus`, 'success');
      await logDelete('REPUTASI', `Hapus indikator: ${row.indikator} (Sub No: ${row.subNo})`, {
        userId: currentUser.id,
        isSuccess: true,
  ...
  const REPUTASI_exportExcel = async () => {
    try {
      exportReputasiToExcel(REPUTASI_filtered, viewYear, viewQuarter);
      showToast(`Data Reputasi tahun ${viewYear} triwulan ${viewQuarter} berhasil diexport`, 'success');
      await logExport('REPUTASI', `Export data Reputasi ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: true,
        metadata: { type: 'inherent', year: viewYear, quarter: viewQuarter, dataCount: REPUTASI_filtered.length }
      });
    } catch (err) {
      showToast('Gagal mengexport data', 'error');
      await logExport('REPUTASI', `Gagal export Reputasi ${viewYear} TW${viewQuarter}`, {
        userId: currentUser.id,
        isSuccess: false,
      ...
      <header className="px-4 py-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-end gap-4">
            ...
            <div className="flex flex-col gap-1">
              <QuarterSelect
                ...
                onChange={(v) => {
                  ...
                  setREPUTASI_form((f) => ({ ...f, quarter: v }));
                }}
              />
            </div>
          </div>
          ...
          <div className="flex items-end gap-2">
            <button onClick={() => setShowReputasiForm(true)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 font-semibold">
              + Tambah Data
            </button>

            <button onClick={REPUTASI_exportExcel} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
          ...
          <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-white font-semibold text-lg sm:text-xl">Form Section – Reputasi</h2>
              <button onClick={() => setShowReputasiForm(false)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-white/20 hover:bg-white/30 text-white font-bold" title="Tutup Form">
                ✕
              </button>
            </div>
            ...
            <div className="flex items-end gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={REPUTASI_sectionForm.no}
                  onChange={(e) => setREPUTASI_sectionForm((f) => ({ ...f, no: e.target.value }))}
                />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  style={!isAddingNewSection && !isEditingSection ? { backgroundColor: '#f3f4f6', cursor: 'not-allowed' } : {}}
                  disabled={!isAddingNewSection && !isEditingSection}
                  value={REPUTASI_sectionForm.bobotSection ?? ''}
                  onChange={(e) => setREPUTASI_sectionForm((f) => ({ ...f, bobotSection: e.target.value }))}
                />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <input
                  ...
                  style={!isAddingNewSection && !isEditingSection ? { backgroundColor: '#f3f4f6', cursor: 'not-allowed' } : {}}
                  disabled={!isAddingNewSection && !isEditingSection}
                  value={REPUTASI_sectionForm.sectionLabel}
                  onChange={(e) => setREPUTASI_sectionForm((f) => ({ ...f, sectionLabel: e.target.value }))}
              ...
              <div className="flex gap-2 self-end">
                {!isAddingNewSection && !isEditingSection && (
                  <>
                    ...
                    <button className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center" onClick={() => REPUTASI_removeSection(REPUTASI_sectionForm.id)} title="Hapus Section">
                      <Trash2 size={20} />
                    </button>
                  </>
                )}
                ...
                {(isAddingNewSection || isEditingSection) && (
                  <>
                    <button
                      ...
                      onClick={isAddingNewSection ? REPUTASI_addSection : REPUTASI_saveSection}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setIsEditingSection(false);
                        if (REPUTASI_sectionForm.id) {
                          REPUTASI_selectSection(REPUTASI_sectionForm.id);
                        }
                      }}
                      title="Batal"
                    >
            ...
            <div className="relative">
              ...
              <select
                ...
                value={isAddingNewSection ? 'ADD_NEW_SECTION' : REPUTASI_sectionForm.id || ''}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === 'ADD_NEW_SECTION') {
                    setIsAddingNewSection(true);
                    setIsEditingSection(true);
                    setREPUTASI_sectionForm({ id: 0, no: '', bobotSection: 0, sectionLabel: '' });
                    return;
                  }
                  if (val) {
                    setIsAddingNewSection(false);
                    setIsEditingSection(false);
                    REPUTASI_selectSection(Number(val));
                  }
                }}
              >
          ...
          <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-2">
                ...
                <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={REPUTASI_form.subNo} onChange={(e) => setREPUTASI_form((f) => ({ ...f, subNo: e.target.value }))} placeholder="1.1" />
              </div>
              <div className="col-span-7">
                ...
                <input
                  ...
                  value={REPUTASI_form.indikator}
                  onChange={(e) => setREPUTASI_form((f) => ({ ...f, indikator: e.target.value }))}
                  placeholder="Nama indikator reputasi…"
                />
              </div>
              <div className="col-span-3">
                ...
                <input
                  ...
                  value={REPUTASI_form.bobotIndikator ?? ''}
                  onChange={(e) => setREPUTASI_form((f) => ({ ...f, bobotIndikator: e.target.value }))}
                  placeholder="50"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-4">
                ...
                <select
                  ...
                  value={REPUTASI_form.mode || 'RASIO'}
                  onChange={(e) => {
              ...
              <div className="col-span-8">
                {REPUTASI_form.mode === 'TEKS' ? (
                  <>
                    <label className="text-sm font-medium mb-2 block text-white">Hasil (Teks) – akan muncul di kolom "Hasil"</label>
                    <input
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                      value={REPUTASI_form.hasilText || ''}
                      onChange={(e) => setREPUTASI_form((f) => ({ ...f, hasilText: e.target.value }))}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        type="text"
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                        placeholder={REPUTASI_form.mode === 'RASIO' ? 'Contoh default: pemb / peny — atau rumus custom (pemb, peny)' : 'Contoh default: peny — atau rumus custom (peny / 1000)'}
                        value={REPUTASI_form.formula || ''}
                        onChange={(e) => setREPUTASI_form((f) => ({ ...f, formula: e.target.value }))}
                      />
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={REPUTASI_form.isPercent || false}
                          onChange={(e) => setREPUTASI_form((f) => ({ ...f, isPercent: e.target.checked }))}
            ...
            {REPUTASI_form.mode === 'RASIO' && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="text-sm font-medium mb-2 block text-white">Faktor Pembilang</label>
                  <input
                    className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                    value={REPUTASI_form.numeratorLabel}
                    onChange={(e) => setREPUTASI_form((f) => ({ ...f, numeratorLabel: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setREPUTASI_form((f) => ({ ...f, numeratorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      ...
                      if (formatted && formatted !== val) {
                        ...
                        setREPUTASI_form((f) => ({ ...f, numeratorValue: formatted }));
                      }
                    }}
            ...
            {REPUTASI_form.mode !== 'TEKS' && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                    value={REPUTASI_form.denominatorLabel}
                    onChange={(e) => setREPUTASI_form((f) => ({ ...f, denominatorLabel: e.target.value }))}
                    placeholder={REPUTASI_form.mode === 'RASIO' ? 'Total Asset (Jutaan)' : 'Jumlah kejadian, jumlah kasus, dll.'}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setREPUTASI_form((f) => ({ ...f, denominatorValue: val }));
                    }}
                    onBlur={(e) => {
                      const val = e.target.value;
                      if (!val || val.endsWith(',') || val.endsWith('.') || val === '-') return;
                      ...
                      if (formatted && formatted !== val) {
                        ...
                        setREPUTASI_form((f) => ({ ...f, denominatorValue: formatted }));
                      }
                    }}
                    placeholder="Contoh: -1000, 1.000, 1.000,50, 10%"
                  />
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-6 space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      value={REPUTASI_form.sumberRisiko}
                      onChange={(e) => setREPUTASI_form((f) => ({ ...f, sumberRisiko: e.target.value }))}
                  ...
                  <div>
                    ...
                    <textarea
                      rows={4}
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm resize-none bg-white"
                      value={REPUTASI_form.dampak}
                      onChange={(e) => setREPUTASI_form((f) => ({ ...f, dampak: e.target.value }))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="number" className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-100 font-bold text-center" value={REPUTASI_form.peringkat ?? ''} readOnly />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        value={(() => {
                          const peringkatNum = REPUTASI_form.peringkat === '' || REPUTASI_form.peringkat == null ? 0 : Number(REPUTASI_form.peringkat);
                          const bobotSecNum = Number(REPUTASI_sectionForm.bobotSection || 0);
                          const bobotIndNum = Number(REPUTASI_form.bobotIndikator || 0);
                    ...
                    <div>
                      ...
                      <input
                        className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                        value={(() => {
                          if (REPUTASI_form.mode === 'TEKS') return REPUTASI_form.hasilText || '';
                          const raw = computeReputasiHasil(buildBaseRow());
                          if (raw === '' || raw == null) return '';
                          if (REPUTASI_form.isPercent) {
                            const pct = Number(raw) * 100;
                            if (!isFinite(pct) || isNaN(pct)) return '';
                            return `${pct.toFixed(2)}%`;
                          }
                          if (REPUTASI_form.mode === 'NILAI_TUNGGAL') return fmtNumber(raw);
                  ...
                  <div>
                    ...
                    <textarea
                      rows={3}
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                      value={REPUTASI_form.keterangan}
                      onChange={(e) => setREPUTASI_form((f) => ({ ...f, keterangan: e.target.value }))}
                    />
                  </div>
                </div>
                ...
                <div className="col-span-6 grid grid-cols-1 gap-3 items-stretch">
                  <RiskField
                    className="w-full"
                    label="Low"
                    value={REPUTASI_form.low}
                    onChange={(v) => setREPUTASI_form((f) => ({ ...f, low: v }))}
                    color="#4A5A2C"
                    textColor="#ffffff"
                    placeholder={REPUTASI_form.mode === 'TEKS' ? 'Contoh: Baik, Aman, Tes' : 'x ≤ 1%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Low to Moderate"
                    value={REPUTASI_form.lowToModerate}
                    onChange={(v) => setREPUTASI_form((f) => ({ ...f, lowToModerate: v }))}
                    color="#A6D86C"
                    textColor="#0B2545"
                    placeholder={REPUTASI_form.mode === 'TEKS' ? 'Contoh: Cukup Baik' : '1% < x ≤ 2%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Moderate"
                    value={REPUTASI_form.moderate}
                    onChange={(v) => setREPUTASI_form((f) => ({ ...f, moderate: v }))}
                    color="#FFFF00"
                    textColor="#4B3A00"
                    placeholder={REPUTASI_form.mode === 'TEKS' ? 'Contoh: Sedang, Normal' : '2% < x ≤ 3%'}
                  />
                  <RiskField
                    className="w-full"
                    label="Moderate to High"
                    value={REPUTASI_form.moderateToHigh}
                    onChange={(v) => setREPUTASI_form((f) => ({ ...f, moderateToHigh: v }))}
                    color="#FAD2A7"
                    textColor="#5A2E00"
                    placeholder={REPUTASI_form.mode === 'TEKS' ? 'Contoh: Cukup Tinggi' : '3% < x ≤ 4%'}
                  />
                  <RiskField
                    className="w-full"
                    label="High"
                    value={REPUTASI_form.high}
                    onChange={(v) => setREPUTASI_form((f) => ({ ...f, high: v }))}
                    color="#E57373"
                    textColor="#FFFFFF"
                    placeholder={REPUTASI_form.mode === 'TEKS' ? 'Contoh: Buruk, Berbahaya, Tinggi' : 'x > 4%'}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end">
              {!REPUTASI_editingRow ? (
                <button onClick={REPUTASI_addRow} className="px-8 py-3 rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold">
                  + Tambah
                </button>
              ) : (
                <div className="flex gap-3">
                  <button onClick={REPUTASI_saveEdit} className="px-8 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">
                    Simpan
                  </button>
                  <button onClick={REPUTASI_resetForm} className="px-8 py-3 rounded-lg border-2 border-gray-300 hover:bg-gray-50 font-semibold">
                    Batal
                  </button>
                </div>
              )}
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
          <div className="relative h-[350px]">
            <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
              <DataTable rows={REPUTASI_filtered} totalWeighted={REPUTASI_totalWeighted} viewYear={viewYear} viewQuarter={viewQuarter} startEdit={REPUTASI_startEdit} removeRow={REPUTASI_removeRow} />
            </div>
          </div>
        </div>
      </section>
