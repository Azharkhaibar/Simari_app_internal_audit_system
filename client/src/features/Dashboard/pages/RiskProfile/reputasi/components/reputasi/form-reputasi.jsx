...
import { computeWeighted } from '../../utils/reputasi/calc';
...
export default function FormReputasi({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Reputasi', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('REPUTASI', `Menambahkan data reputasi baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/reputasi',
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
            indikator: form.indikator,
            reputasi_data: serviceData,
          },
        });
      ...
      onReset();
      alert('Data reputasi berhasil ditambahkan!');
    } catch (error) {
      console.error('Error adding reputasi:', error);

      if (logCreate && typeof logCreate === 'function') {
        await logCreate('REPUTASI', `Gagal menambahkan data reputasi - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/reputasi',
          isSuccess: false,
          metadata: {
            error: error.message,
      ...
      alert('Gagal menambahkan data reputasi: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('REPUTASI', `Memperbarui data reputasi - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/reputasi/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
      ...
      alert('Data reputasi berhasil diperbarui!');
    } catch (error) {
      console.error('Error saving reputasi:', error);

      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('REPUTASI', `Gagal memperbarui data reputasi - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/reputasi/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: {
            error: error.message,
          },
      ...
      alert('Gagal memperbarui data reputasi: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data reputasi dan analisis risiko secara komprehensif</p>
            </div>
          </div>
