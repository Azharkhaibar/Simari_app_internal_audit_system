// investasi-kpmr.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import { useToast } from '../../components/use-toast';
import kpmrInvestasiApiService, {
  FrontendKpmrResponse,
  FrontendAspekResponse,
  FrontendPertanyaanResponse,
  CreateKpmrAspekInvestasiDto,
  CreateKpmrPertanyaanInvestasiDto,
  UpdateKpmrAspekInvestasiDto,
  UpdateKpmrPertanyaanInvestasiDto,
  UpdateSkorDto,
  CreateKpmrInvestasiOjkDto,
} from '../../services/kpmr/investasi-kpmr.service';

interface UseKpmrInvestasiReturn {
  // State
  ...
  // Aspek operations
  addAspek: (kpmrId: number | string, aspekData: CreateKpmrAspekInvestasiDto) => Promise<FrontendAspekResponse>;
  updateAspek: (id: number | string, aspekData: UpdateKpmrAspekInvestasiDto) => Promise<FrontendAspekResponse>;
  deleteAspek: (id: number | string) => Promise<void>;

  // Pertanyaan operations
  addPertanyaan: (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanInvestasiDto) => Promise<FrontendPertanyaanResponse>;
  updatePertanyaan: (id: number | string, pertanyaanData: UpdateKpmrPertanyaanInvestasiDto) => Promise<FrontendPertanyaanResponse>;
...
export function useKpmrInvestasi(): UseKpmrInvestasiReturn {
  ...
  const cleanId = useCallback((id: string | number): number => {
    return kpmrInvestasiApiService.cleanId(id);
  }, []);
  ...
  const loadKpmrByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      ...
      try {
        const data = await kpmrInvestasiApiService.getKpmrByYearQuarter(targetYear, targetQuarter, true);

        if (!mountedRef.current) return;

        safeSet(setKpmr, data);
        safeSet(setCurrentKpmrId, data.id);
        const frontendRows = kpmrInvestasiApiService.convertToFrontendFormat(data);
  ...
  const createKpmr = useCallback(
    async (year: number, quarter: number): Promise<FrontendKpmrResponse | null> => {
      ...
      try {
        ...
        const payload: CreateKpmrInvestasiOjkDto = {
          year: yearNum,
        ...
        const data = await kpmrInvestasiApiService.createKpmr(payload);
  ...
  const refreshKpmrData = useCallback(async (): Promise<FrontendAspekResponse[]> => {
    ...
    try {
      ...
      const freshData = await kpmrInvestasiApiService.getKpmrWithRelations(cleanIdNum);

      if (!mountedRef.current) return [];

      safeSet(setKpmr, freshData);
      const frontendRows = kpmrInvestasiApiService.convertToFrontendFormat(freshData);
      safeSet(setRows, frontendRows);
  ...
  const addAspek = useCallback(
    async (kpmrId: number | string, aspekData: CreateKpmrAspekInvestasiDto): Promise<FrontendAspekResponse> => {
      ...
      try {
        ...
        const payload: CreateKpmrAspekInvestasiDto = {
        ...
        const newAspek = await kpmrInvestasiApiService.createAspek(cleanKpmrId, payload);
  ...
  const updateAspek = useCallback(
    async (id: number | string, aspekData: UpdateKpmrAspekInvestasiDto): Promise<FrontendAspekResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedAspek = await kpmrInvestasiApiService.updateAspek(cleanIdNum, aspekData);
  ...
  const deleteAspek = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrInvestasiApiService.deleteAspek(cleanIdNum);
  ...
  const addPertanyaan = useCallback(
    async (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanInvestasiDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const newPertanyaan = await kpmrInvestasiApiService.createPertanyaan(cleanAspekId, payload);
  ...
  const updatePertanyaan = useCallback(
    async (id: number | string, pertanyaanData: UpdateKpmrPertanyaanInvestasiDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrInvestasiApiService.updatePertanyaan(cleanIdNum, pertanyaanData);
  ...
  const deletePertanyaan = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrInvestasiApiService.deletePertanyaan(cleanIdNum);
  ...
  const updateSkor = useCallback(
    async (id: number | string, quarter: string, skor: number): Promise<FrontendPertanyaanResponse> => {
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrInvestasiApiService.updateSkor(cleanIdNum, updateSkorDto);
...
export default useKpmrInvestasi;