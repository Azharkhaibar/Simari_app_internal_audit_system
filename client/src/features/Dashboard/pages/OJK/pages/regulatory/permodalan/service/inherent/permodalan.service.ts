...
export interface PermodalanProdukOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
  parameters?: PermodalanParameterEntity[];
  summary?: {
...
export interface PermodalanParameterEntity {
  id: number;
  nomor?: string;
  judul: string;
  bobot: number;
  ...
  nilaiList?: PermodalanNilaiEntity[];
  orderIndex: number;
  permodalanProdukOjkId: number;
  createdAt: Date;
  updatedAt: Date;
}

export interface PermodalanNilaiEntity {
  id: number;
  nomor?: string;
  judul: {
    type?: string;
...
export interface CreatePermodalanProdukInherentDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdatePermodalanProdukInherentDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class PermodalanProdukService {
  private baseUrl = '/permodalan';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: PermodalanProdukOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
  ...
  async ensureDataExists(year: number, quarter: number): Promise<PermodalanProdukOjkEntity> {
  ...
  async loadOrCreateData(year: number, quarter: number): Promise<PermodalanProdukOjkEntity> {
  ...
  public formatToFrontend(entity: PermodalanProdukOjkEntity | null): any[] {
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[PermodalanProdukService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<PermodalanProdukOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_permodalan_produk.get<PermodalanProdukOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<PermodalanProdukOjkEntity | null> {
  ...
  async getAll(): Promise<PermodalanProdukOjkEntity[]> {
    const url = this.baseUrl;
    this.logApiCall('GET', url);
  ...
  async getById(id: number): Promise<PermodalanProdukOjkEntity> {
  ...
  async create(createDto: CreatePermodalanProdukInherentDto): Promise<PermodalanProdukOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_permodalan_produk.post<PermodalanProdukOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdatePermodalanProdukInherentDto): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_permodalan_produk.put<PermodalanProdukOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_permodalan_produk.put<PermodalanProdukOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdatePermodalanProdukInherentDto['summary']): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_permodalan_produk.put<PermodalanProdukOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async addParameter(inherentId: number, dto: CreateParameterDto): Promise<PermodalanProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_permodalan_produk.post<PermodalanProdukOjkEntity>(url, payload);
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateParameterDto): Promise<PermodalanProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_permodalan_produk.put<PermodalanProdukOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(inherentId: number, parameterId: number): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_permodalan_produk.post<PermodalanProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(inherentId: number, parameterId: number): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_permodalan_produk.delete<PermodalanProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateNilaiDto): Promise<PermodalanProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<PermodalanProdukOjkEntity> {
  ...
  async copyNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<PermodalanProdukOjkEntity> {
  ...
  async removeNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async importFromExcel(importData: any): Promise<PermodalanProdukOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_permodalan_produk.post<PermodalanProdukOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: PermodalanProdukOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<PermodalanProdukOjkEntity> {
    ...
    try {
      ...
      const createDto: CreatePermodalanProdukInherentDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
      ...
      const fallbackData: PermodalanProdukOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: PermodalanProdukOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_permodalan_produk.get(this.baseUrl, { timeout: 5000 });
...
export const permodalanProdukService = new PermodalanProdukService();
export default permodalanProdukService;
