// src/ojk/investasi-produk/investasi-produk-ojk/investasi-produk-inherent.jsx
...
export default function InvestasiProdukInherentWrapper() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useInvestasiProdukIntegration(year, quarter);
    ...
    () => ({
      addParameter: async (createParamDto) => {
        try {
          ...
          await logCreate('INVESTASI_OJK', `Tambah Parameter: "${createParamDto.judul}" (Bobot: ${createParamDto.bobot}%)`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, createParamDto }
          });
          return { success: true };
        } catch (err) {
          log.error('Error adding parameter:', err);
          await logCreate('INVESTASI_OJK', `Gagal Tambah Parameter: "${createParamDto.judul}"`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, createParamDto, error: err.message }
          });
          return {
            success: false,
      ...
      updateParameter: async (parameterId, updateParamDto) => {
        try {
          ...
          await logUpdate('INVESTASI_OJK', `Update Parameter ID: ${parameterId} - "${updateParamDto.judul}"`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, updateParamDto }
          });
          return { success: true };
        } catch (err) {
          log.error('Error updating parameter:', err);
          await logUpdate('INVESTASI_OJK', `Gagal Update Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, updateParamDto, error: err.message }
          });
          return {
            success: false,
      ...
      deleteParameter: async (parameterId) => {
        try {
          ...
          await logDelete('INVESTASI_OJK', `Hapus Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId }
          });
          return { success: true };
        } catch (err) {
          log.error('Error deleting parameter:', err);
          await logDelete('INVESTASI_OJK', `Gagal Hapus Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, error: err.message }
          });
          return {
            success: false,
      ...
      addNilai: async (parameterId, createNilaiDto) => {
        try {
          ...
          await logCreate('INVESTASI_OJK', `Tambah Nilai pada Parameter ID: ${parameterId} - "${createNilaiDto.judul?.text || ''}"`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, createNilaiDto }
          });
          return { success: true };
        } catch (err) {
          log.error('Error adding nilai:', err);
          await logCreate('INVESTASI_OJK', `Gagal Tambah Nilai pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, createNilaiDto, error: err.message }
          });
          return {
            success: false,
      ...
      updateNilai: async (parameterId, nilaiId, updateNilaiDto) => {
        try {
          ...
          await logUpdate('INVESTASI_OJK', `Update Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId, updateNilaiDto }
          });
          return { success: true };
        } catch (err) {
          log.error('Error updating nilai:', err);
          await logUpdate('INVESTASI_OJK', `Gagal Update Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId, updateNilaiDto, error: err.message }
          });
          return {
            success: false,
      ...
      deleteNilai: async (parameterId, nilaiId) => {
        try {
          ...
          await logDelete('INVESTASI_OJK', `Hapus Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId }
          });
          return { success: true };
        } catch (err) {
          log.error('Error deleting nilai:', err);
          await logDelete('INVESTASI_OJK', `Gagal Hapus Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId, error: err.message }
          });
          return {
            success: false,
      ...
      copyParameter: async (parameterId) => {
        try {
          ...
          await logCreate('INVESTASI_OJK', `Salin Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId }
          });
          ...
          await logCreate('INVESTASI_OJK', `Gagal Salin Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, error: err.message }
          });
      ...
      copyNilai: async (parameterId, nilaiId) => {
        try {
          ...
          await logCreate('INVESTASI_OJK', `Salin Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId }
          });
          ...
          await logCreate('INVESTASI_OJK', `Gagal Salin Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId, error: err.message }
          });
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Investasi Produk - Inherent Risk</h1>
      ...
      {/* Komponen utama */}
      <InvestasiProdukInherent rows={rows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={isLoading} isLocked={currentInherentData?.isLocked} />
    </div>
  );
}

// Komponen utama - PERBAIKAN: Tambah prop isInitialLoading
function InvestasiProdukInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  const handleAddNewParameter = useCallback(async () => {
    ...
    try {
      ...
      if (kategori.model === 'open_end') {
        if (!kategori.jenis) {
          throw new Error('Jenis produk investasi wajib dipilih untuk model "open_end"');
        }
            ...
            <div className="w-full flex gap-4 my-3 items-start">
              ...
              {parameter.kategori.model === 'open_end' && (
                <div className="w-[50%] flex flex-col">
                  <label className="font-semibold text-sm ml-1 mb-1 text-slate-200">Jenis Produk Investasi</label>
                  <select
                    className="bg-white text-slate-800 text-sm rounded px-2 py-1 border border-slate-300"
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Investasi Produk - Inherent</h1>
