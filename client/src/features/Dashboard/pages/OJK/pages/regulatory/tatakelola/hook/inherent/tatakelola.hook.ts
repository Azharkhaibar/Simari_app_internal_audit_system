// src/ojk/tatakelola-produk/hook/inherent/tatakelola.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import tatakelolaProdukService, { CreateNilaiDto, CreateParameterDto, TatakelolaProdukOjkEntity, UpdateNilaiDto, UpdateParameterDto } from '../../service/inherent/tatakelola.service';

export const useTatakelolaProdukIntegration = (initialYear?: number, initialQuarter?: number) => {
  ...
  const [currentInherentData, setCurrentInherentData] = useState<TatakelolaProdukOjkEntity | null>(null);
  ...
  const cacheRef = useRef<
    Map<
      string,
      {
        ...
        entity: TatakelolaProdukOjkEntity | null;
        timestamp: number;
      }
    >
  >(new Map());
  ...
  const getOrCreateData = useCallback(async (targetYear: number, targetQuarter: number, forceReload = false) => {
    ...
    try {
      ...
      const result = await tatakelolaProdukService.findOrCreate(targetYear, targetQuarter);
      ...
      const formattedRows = tatakelolaProdukService.formatToFrontend(result.data);
  ...
  const updateCache = useCallback(
    (y: number, q: number, data: { rows: any[]; inherentId: number | null; entity: TatakelolaProdukOjkEntity | null }) => {
  ...
  const formatParameterJudul = useCallback(
    (judul: any): string => {
      ...
      return tatakelolaProdukService.formatParameterJudul(judul);
    },
    [validateParameterJudul],
  );

  const formatNilaiJudul = useCallback((judul: any): CreateNilaiDto['judul'] => {
    return tatakelolaProdukService.formatNilaiJudul(judul);
  }, []);
  ...
  const handleAddParameter = useCallback(
    async (dto: Partial<CreateParameterDto>) => {
      ...
      try {
        ...
        await tatakelolaProdukService.loadOrCreateData(context.year, context.quarter);

        // Tambahkan parameter
        await tatakelolaProdukService.addParameter(context.inherentId, finalPayload);
    ...
    async (parameterId: string, dto: UpdateParameterDto) => {
      ...
      try {
        ...
        if (dto.judul !== undefined) {
          ...
          payload.judul = tatakelolaProdukService.formatParameterJudul(dto.judul);
        }
        ...
        await tatakelolaProdukService.updateParameter(context.inherentId, parameterIdNum, payload);
  ...
  const handleCopyParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await tatakelolaProdukService.copyParameter(context.inherentId, parameterIdNum);
  ...
  const handleDeleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await tatakelolaProdukService.removeParameter(context.inherentId, parameterIdNum);
  ...
  const handleAddNilai = useCallback(
    async (parameterId: string, dto: CreateNilaiDto) => {
      ...
      try {
        ...
        // Tambahkan nilai
        await tatakelolaProdukService.addNilai(context.inherentId, parameterIdNum, payload);
  ...
  const handleUpdateNilai = useCallback(
    async (parameterId: string, nilaiId: string, dto: UpdateNilaiDto) => {
      ...
      try {
        ...
        await tatakelolaProdukService.updateNilai(context.inherentId, parameterIdNum, nilaiIdNum, payload);
  ...
  const handleCopyNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await tatakelolaProdukService.copyNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleDeleteNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await tatakelolaProdukService.removeNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleReorderParameters = useCallback(
    async (parameterIds: string[]) => {
      ...
      try {
        ...
        await tatakelolaProdukService.reorderParameters(context.inherentId, parameterIdsNum);
  ...
  const handleReorderNilai = useCallback(
    async (parameterId: string, nilaiIds: string[]) => {
      ...
      try {
        ...
        await tatakelolaProdukService.reorderNilai(context.inherentId, parameterIdNum, nilaiIdsNum);
...
export default useTatakelolaProdukIntegration;