// src/ojk/strategis-produk/hook/inherent/strategis.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import strategisProdukService, { CreateNilaiDto, CreateParameterDto, StrategisProdukOjkEntity, UpdateNilaiDto, UpdateParameterDto } from '../../services/inherent/strategis.service';

export const useStrategisProdukIntegration = (initialYear?: number, initialQuarter?: number) => {
  ...
  const [currentInherentData, setCurrentInherentData] = useState<StrategisProdukOjkEntity | null>(null);
  ...
  const cacheRef = useRef<
    Map<
      string,
      {
        ...
        entity: StrategisProdukOjkEntity | null;
        timestamp: number;
      }
    >
  >(new Map());
  ...
  const getOrCreateData = useCallback(async (targetYear: number, targetQuarter: number, forceReload = false) => {
    ...
    try {
      ...
      const result = await strategisProdukService.findOrCreate(targetYear, targetQuarter);
      ...
      const formattedRows = strategisProdukService.formatToFrontend(result.data);
  ...
  const updateCache = useCallback(
    (y: number, q: number, data: { rows: any[]; inherentId: number | null; entity: StrategisProdukOjkEntity | null }) => {
  ...
  const formatParameterJudul = useCallback(
    (judul: any): string => {
      ...
      return strategisProdukService.formatParameterJudul(judul);
    },
    [validateParameterJudul],
  );

  const formatNilaiJudul = useCallback((judul: any): CreateNilaiDto['judul'] => {
    return strategisProdukService.formatNilaiJudul(judul);
  }, []);
  ...
  const handleAddParameter = useCallback(
    async (dto: Partial<CreateParameterDto>) => {
      ...
      try {
        ...
        await strategisProdukService.loadOrCreateData(context.year, context.quarter);

        // Tambahkan parameter
        await strategisProdukService.addParameter(context.inherentId, finalPayload);
    ...
    async (parameterId: string, dto: UpdateParameterDto) => {
      ...
      try {
        ...
        if (dto.judul !== undefined) {
          ...
          payload.judul = strategisProdukService.formatParameterJudul(dto.judul);
        }
        ...
        await strategisProdukService.updateParameter(context.inherentId, parameterIdNum, payload);
  ...
  const handleCopyParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await strategisProdukService.copyParameter(context.inherentId, parameterIdNum);
  ...
  const handleDeleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await strategisProdukService.removeParameter(context.inherentId, parameterIdNum);
  ...
  const handleAddNilai = useCallback(
    async (parameterId: string, dto: CreateNilaiDto) => {
      ...
      try {
        ...
        // Tambahkan nilai
        await strategisProdukService.addNilai(context.inherentId, parameterIdNum, payload);
  ...
  const handleUpdateNilai = useCallback(
    async (parameterId: string, nilaiId: string, dto: UpdateNilaiDto) => {
      ...
      try {
        ...
        await strategisProdukService.updateNilai(context.inherentId, parameterIdNum, nilaiIdNum, payload);
  ...
  const handleCopyNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await strategisProdukService.copyNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleDeleteNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await strategisProdukService.removeNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleReorderParameters = useCallback(
    async (parameterIds: string[]) => {
      ...
      try {
        ...
        await strategisProdukService.reorderParameters(context.inherentId, parameterIdsNum);
  ...
  const handleReorderNilai = useCallback(
    async (parameterId: string, nilaiIds: string[]) => {
      ...
      try {
        ...
        await strategisProdukService.reorderNilai(context.inherentId, parameterIdNum, nilaiIdsNum);
...
export default useStrategisProdukIntegration;
