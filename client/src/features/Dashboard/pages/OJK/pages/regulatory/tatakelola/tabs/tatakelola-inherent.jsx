// src/ojk/tatakelola-produk/tatakelola-produk-ojk/tatakelola-produk-inherent.jsx
...
import useTatakelolaProdukIntegration from '../hook/inherent/tatakelola.hook';
...
export default function TatakelolaProdukInherentWrapper() {
  const { user } = useAuth();
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useTatakelolaProdukIntegration(year, quarter);
    ...
    () => ({
      addParameter: async (createParamDto) => {
        try {
          ...
          // AUDIT LOG: CREATE Parameter Tata Kelola Produk Inherent
          await logCreate('TATAKELOLA_OJK', `Menambah parameter baru - ${createParamDto.judul} (Bobot: ${createParamDto.bobot}%)`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterData: createParamDto }
          });
      ...
      updateParameter: async (parameterId, updateParamDto) => {
        try {
          ...
          // AUDIT LOG: UPDATE Parameter Tata Kelola Produk Inherent
          await logUpdate('TATAKELOLA_OJK', `Mengupdate parameter ID: ${parameterId} - ${updateParamDto.judul}`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, updateData: updateParamDto }
          });
      ...
      deleteParameter: async (parameterId) => {
        try {
          ...
          // AUDIT LOG: DELETE Parameter Tata Kelola Produk Inherent
          await logDelete('TATAKELOLA_OJK', `Menghapus parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      addNilai: async (parameterId, createNilaiDto) => {
        try {
          ...
          // AUDIT LOG: CREATE Nilai Tata Kelola Produk Inherent
          await logCreate('TATAKELOLA_OJK', `Menambah nilai baru pada parameter ID: ${parameterId} - ${createNilaiDto.judul?.text}`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiData: createNilaiDto }
          });
      ...
      updateNilai: async (parameterId, nilaiId, updateNilaiDto) => {
        try {
          ...
          // AUDIT LOG: UPDATE Nilai Tata Kelola Produk Inherent
          await logUpdate('TATAKELOLA_OJK', `Mengupdate nilai ID: ${nilaiId} pada parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      deleteNilai: async (parameterId, nilaiId) => {
        try {
          ...
          // AUDIT LOG: DELETE Nilai Tata Kelola Produk Inherent
          await logDelete('TATAKELOLA_OJK', `Menghapus nilai ID: ${nilaiId} dari parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId }
          });
      ...
      copyParameter: async (parameterId) => {
        try {
          ...
          // AUDIT LOG: COPY Parameter Tata Kelola Produk Inherent
          await logCreate('TATAKELOLA_OJK', `Menyalin parameter ID: ${parameterId} menjadi parameter baru`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, sourceParameterId: parameterId }
          });
      ...
      copyNilai: async (parameterId, nilaiId) => {
        try {
          ...
          // AUDIT LOG: COPY Nilai Tata Kelola Produk Inherent
          await logCreate('TATAKELOLA_OJK', `Menyalin nilai ID: ${nilaiId} pada parameter ID: ${parameterId} menjadi nilai baru`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, sourceNilaiId: nilaiId }
          });
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Tata Kelola Produk - Inherent Risk</h1>
      ...
      {/* Komponen utama */}
      <TatakelolaProdukInherent rows={rows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={isLoading} isLocked={currentInherentData?.isLocked} />
    </div>
  );
}

// Komponen utama - PERBAIKAN: Tambah prop isInitialLoading
function TatakelolaProdukInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Tata Kelola Produk - Inherent</h1>
