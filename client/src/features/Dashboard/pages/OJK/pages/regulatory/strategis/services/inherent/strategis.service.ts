...
export interface StrategisProdukOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
  parameters?: StrategisParameterEntity[];
  summary?: {
...
export interface StrategisParameterEntity {
  id: number;
  nomor?: string;
  judul: string;
  bobot: number;
  ...
  nilaiList?: StrategisNilaiEntity[];
  orderIndex: number;
  strategisProdukOjkId: number;
  createdAt: Date;
  updatedAt: Date;
}

export interface StrategisNilaiEntity {
  id: number;
  nomor?: string;
  judul: {
    type?: string;
...
export interface CreateStrategisProdukInherentDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateStrategisProdukInherentDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class StrategisProdukService {
  private baseUrl = '/strategis';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: StrategisProdukOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
  ...
  async ensureDataExists(year: number, quarter: number): Promise<StrategisProdukOjkEntity> {
  ...
  async loadOrCreateData(year: number, quarter: number): Promise<StrategisProdukOjkEntity> {
  ...
  public formatToFrontend(entity: StrategisProdukOjkEntity | null): any[] {
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[StrategisProdukService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<StrategisProdukOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_strategis_produk.get<StrategisProdukOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<StrategisProdukOjkEntity | null> {
    ...
    try {
      const response = await api_strategis_produk.get<any>(url, { params });
  ...
  async getAll(): Promise<StrategisProdukOjkEntity[]> {
    const url = this.baseUrl;
    this.logApiCall('GET', url);
  ...
  async getById(id: number): Promise<StrategisProdukOjkEntity> {
  ...
  async create(createDto: CreateStrategisProdukInherentDto): Promise<StrategisProdukOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_strategis_produk.post<StrategisProdukOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateStrategisProdukInherentDto): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_strategis_produk.put<StrategisProdukOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_strategis_produk.put<StrategisProdukOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateStrategisProdukInherentDto['summary']): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_strategis_produk.put<StrategisProdukOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async remove(id: number): Promise<{ message: string; id: number }> {
    ...
    try {
      const response = await api_strategis_produk.delete<{ message: string; id: number }>(url);
      return response.data;
    } catch (error: any) {
  ...
  async addParameter(inherentId: number, dto: CreateParameterDto): Promise<StrategisProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_strategis_produk.post<StrategisProdukOjkEntity>(url, payload);
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateParameterDto): Promise<StrategisProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_strategis_produk.put<StrategisProdukOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(inherentId: number, parameterId: number): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_strategis_produk.post<StrategisProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(inherentId: number, parameterId: number): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_strategis_produk.delete<StrategisProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(inherentId: number, parameterIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_strategis_produk.put<{ message: string }>(url, { parameterIds });
      return response.data;
    } catch (error: any) {
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateNilaiDto): Promise<StrategisProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<StrategisProdukOjkEntity> {
  ...
  async copyNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<StrategisProdukOjkEntity> {
  ...
  async removeNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(inherentId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_strategis_produk.put<{ message: string }>(url, { nilaiIds });
      return response.data;
    } catch (error: any) {
  ...
  async importFromExcel(importData: any): Promise<StrategisProdukOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_strategis_produk.post<StrategisProdukOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async getReferences(type?: string): Promise<ReferenceItem[]> {
    ...
    try {
      const response = await api_strategis_produk.get<ReferenceItem[]>(url, { params });
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: StrategisProdukOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<StrategisProdukOjkEntity> {
    ...
    try {
      ...
      const createDto: CreateStrategisProdukInherentDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
      ...
      const fallbackData: StrategisProdukOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: StrategisProdukOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_strategis_produk.get(this.baseUrl, { timeout: 5000 });
...
export const strategisProdukService = new StrategisProdukService();
export default strategisProdukService;
