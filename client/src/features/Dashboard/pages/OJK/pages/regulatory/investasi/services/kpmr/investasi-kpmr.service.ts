...
// services/kpmr/investasi-produk-kpmr.service.ts
...
// ========== CREATE DTOs ==========
export interface CreateKpmrInvestasiOjkDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
  ...
  notes?: string;
  summary?: any;
  aspekList?: CreateKpmrAspekInvestasiDto[];
}

export interface CreateKpmrAspekInvestasiDto {
  nomor?: string;
  judul: string;
  bobot: number;
  deskripsi?: string;
  kpmrOjkId?: number;
  orderIndex?: number;
  pertanyaanList?: CreateKpmrPertanyaanInvestasiDto[];
}

export interface CreateKpmrPertanyaanInvestasiDto {
  nomor?: string;
  pertanyaan: string;
  skor?: KpmrSkorDto;
...
// ========== UPDATE DTOs ==========
export interface UpdateKpmrAspekInvestasiDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  deskripsi?: string;
...
export interface UpdateKpmrPertanyaanInvestasiDto {
  nomor?: string;
  pertanyaan?: string;
  skor?: KpmrSkorDto;
  indicator?: KpmrIndicatorDto;
...
// ========== SERVICE CLASS ==========
class KpmrInvestasiApiService {
  private readonly BASE_PATH = '/kpmr-investasi';
  ...
  private handleApiError(error: any, defaultMessage: string): never {
    console.error(`❌ [KPMR Investasi Service] Error:`, error);
  ...
  async getKpmrByYearQuarter(year: number, quarter: number, withRelations: boolean = true): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_investasi_produk.get(`${url}${params}`);
  ...
  async getKpmrWithRelations(id: number | string): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_investasi_produk.get(`${this.BASE_PATH}/${cleanId}?withRelations=true`);
      return response.data;
    } catch (error: any) {
  ...
  async createAspek(kpmrId: number | string, data: CreateKpmrAspekInvestasiDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_investasi_produk.post(`${this.BASE_PATH}/${cleanKpmrId}/aspek`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updateAspek(id: number | string, data: UpdateKpmrAspekInvestasiDto): Promise<FrontendAspekResponse> {
    try {
      const cleanId = this.cleanId(id);
      const response = await api_investasi_produk.patch(`${this.BASE_PATH}/aspek/${cleanId}`, data);
      return response.data;
    } catch (error: any) {
  ...
  async deleteAspek(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_investasi_produk.delete(`${this.BASE_PATH}/aspek/${cleanId}`);
    } catch (error: any) {
  ...
  async createPertanyaan(aspekId: number | string, data: CreateKpmrPertanyaanInvestasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanAspekId = this.cleanId(aspekId);

      const payload: CreateKpmrPertanyaanInvestasiDto = {
        nomor: data.nomor || '',
      ...
      const response = await api_investasi_produk.post(`${this.BASE_PATH}/aspek/${cleanAspekId}/pertanyaan`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updatePertanyaan(id: number | string, data: UpdateKpmrPertanyaanInvestasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanId = this.cleanId(id);
      ...
      console.log('[KPMR Investasi Service] Update pertanyaan payload:', JSON.stringify(payload, null, 2));

      const response = await api_investasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async deletePertanyaan(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_investasi_produk.delete(`${this.BASE_PATH}/pertanyaan/${cleanId}`);
    } catch (error: any) {
  ...
  async createKpmr(data: CreateKpmrInvestasiOjkDto): Promise<FrontendKpmrResponse> {
    try {
      console.log(`🆕 [KPMR Investasi Service] Creating KPMR:`, data);
      ...
      console.log('📦 [KPMR Investasi Service] Sending payload:', payload);

      const response = await api_investasi_produk.post(this.BASE_PATH, payload);

      if (!response || !response.data) {
        throw new Error('Response dari server tidak valid');
      }

      console.log('✅ [KPMR Investasi Service] Response:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('❌ [KPMR Investasi Service] Create error:', {
  ...
  async updateSkor(id: number | string, data: UpdateSkorDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_investasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}/skor`, data);
      return response.data;
    } catch (error: any) {
...
export const kpmrInvestasiApiService = new KpmrInvestasiApiService();
export default kpmrInvestasiApiService;