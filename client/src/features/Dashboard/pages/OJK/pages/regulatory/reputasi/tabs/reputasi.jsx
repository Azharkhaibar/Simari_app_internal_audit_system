// src/ojk/reputasi-produk/reputasi-produk-ojk/reputasi.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import ReputasiProdukInherentWrapper from './reputasi-inherent';
import ReputasiProdukKpmrPage from './reputasi-kpmr';
import { useHeaderStore } from '../../../../store/header';
import { exportInherent } from '../../../../utils/export/export-inherent';
import { exportKpmr } from '../../../../utils/export/export-kpmr';
import useReputasiProdukIntegration from '../hooks/inherent/reputasi.hook';
...
export default function ReputasiOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useReputasiProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrReputasi();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [ReputasiOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [ReputasiOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [ReputasiOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [ReputasiOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [ReputasiOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [ReputasiOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [ReputasiOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [ReputasiOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [ReputasiOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const handleKpmrRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [ReputasiOJK] Error refreshing KPMR:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    return <ReputasiProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
