// src/ojk/permodalan-produk/permodalan-produk-ojk/permodalan-produk-inherent.jsx
...
import usePermodalanProdukIntegration from '../hook/inherent/permodalan.hook';
...
export default function PermodalanProdukInherentWrapper() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = usePermodalanProdukIntegration(year, quarter);
    ...
    () => ({
      addParameter: async (createParamDto) => {
        try {
          ...
          await logCreate('PERMODALAN_OJK', `Tambah Parameter: "${createParamDto.judul}" (Bobot: ${createParamDto.bobot}%)`, {
          ...
          await logCreate('PERMODALAN_OJK', `Gagal Tambah Parameter: "${createParamDto.judul}"`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      updateParameter: async (parameterId, updateParamDto) => {
        try {
          ...
          await logUpdate('PERMODALAN_OJK', `Update Parameter ID: ${parameterId} - "${updateParamDto.judul}"`, {
            userId: currentUser.id,
            isSuccess: true,
          ...
          await logUpdate('PERMODALAN_OJK', `Gagal Update Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      deleteParameter: async (parameterId) => {
        try {
          ...
          await logDelete('PERMODALAN_OJK', `Hapus Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
          ...
          await logDelete('PERMODALAN_OJK', `Gagal Hapus Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      addNilai: async (parameterId, createNilaiDto) => {
        try {
          ...
          await logCreate('PERMODALAN_OJK', `Tambah Nilai Baru pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
          ...
          await logCreate('PERMODALAN_OJK', `Gagal Tambah Nilai Baru pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      updateNilai: async (parameterId, nilaiId, updateNilaiDto) => {
        try {
          ...
          await logUpdate('PERMODALAN_OJK', `Update Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
          ...
          await logUpdate('PERMODALAN_OJK', `Gagal Update Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      deleteNilai: async (parameterId, nilaiId) => {
        try {
          ...
          await logDelete('PERMODALAN_OJK', `Hapus Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
          ...
          await logDelete('PERMODALAN_OJK', `Gagal Hapus Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
      ...
      copyParameter: async (parameterId) => {
        try {
          ...
          await logCreate('PERMODALAN_OJK', `Salin Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId }
          });
          ...
          await logCreate('PERMODALAN_OJK', `Gagal Salin Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, error: err.message }
          });
      ...
      copyNilai: async (parameterId, nilaiId) => {
        try {
          ...
          await logCreate('PERMODALAN_OJK', `Salin Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId }
          });
          ...
          await logCreate('PERMODALAN_OJK', `Gagal Salin Nilai ID: ${nilaiId} pada Parameter ID: ${parameterId}`, {
            userId: currentUser.id,
            isSuccess: false,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId, error: err.message }
          });
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Permodalan Produk - Inherent Risk</h1>
      ...
      {/* Komponen utama */}
      <PermodalanProdukInherent rows={rows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={isLoading} isLocked={currentInherentData?.isLocked} />
    </div>
  );
}

// Komponen utama - PERBAIKAN: Tambah prop isInitialLoading
function PermodalanProdukInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Permodalan Produk - Inherent</h1>
