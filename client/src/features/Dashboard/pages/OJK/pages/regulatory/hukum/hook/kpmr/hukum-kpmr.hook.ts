...
import kpmrHukumApiService, {
  ...
  CreateKpmrAspekHukumDto,
  CreateKpmrPertanyaanHukumDto,
  UpdateKpmrAspekHukumDto,
  UpdateKpmrPertanyaanHukumDto,
  UpdateSkorDto,
  CreateKpmrHukumOjkDto,
} from '../../service/kpmr/hukum-kpmr.service';

interface UseKpmrHukumReturn {
  // State
  ...
  // Aspek operations
  addAspek: (kpmrId: number | string, aspekData: CreateKpmrAspekHukumDto) => Promise<FrontendAspekResponse>;
  updateAspek: (id: number | string, aspekData: UpdateKpmrAspekHukumDto) => Promise<FrontendAspekResponse>;
  deleteAspek: (id: number | string) => Promise<void>;

  // Pertanyaan operations
  addPertanyaan: (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanHukumDto) => Promise<FrontendPertanyaanResponse>;
  updatePertanyaan: (id: number | string, pertanyaanData: UpdateKpmrPertanyaanHukumDto) => Promise<FrontendPertanyaanResponse>;
...
export function useKpmrHukum(): UseKpmrHukumReturn {
  ...
  const cleanId = useCallback((id: string | number): number => {
    return kpmrHukumApiService.cleanId(id);
  }, []);
  ...
  const loadKpmrByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      ...
      try {
        const data = await kpmrHukumApiService.getKpmrByYearQuarter(targetYear, targetQuarter, true);

        if (!mountedRef.current) return;

        safeSet(setKpmr, data);
        safeSet(setCurrentKpmrId, data.id);
        const frontendRows = kpmrHukumApiService.convertToFrontendFormat(data);
  ...
  const createKpmr = useCallback(
    async (year: number, quarter: number): Promise<FrontendKpmrResponse | null> => {
      ...
      try {
        ...
        const payload: CreateKpmrHukumOjkDto = {
          year: yearNum,
        ...
        const data = await kpmrHukumApiService.createKpmr(payload);
  ...
  const refreshKpmrData = useCallback(async (): Promise<FrontendAspekResponse[]> => {
    ...
    try {
      ...
      const freshData = await kpmrHukumApiService.getKpmrWithRelations(cleanIdNum);

      if (!mountedRef.current) return [];

      safeSet(setKpmr, freshData);
      const frontendRows = kpmrHukumApiService.convertToFrontendFormat(freshData);
      safeSet(setRows, frontendRows);
  ...
  const addAspek = useCallback(
    async (kpmrId: number | string, aspekData: CreateKpmrAspekHukumDto): Promise<FrontendAspekResponse> => {
      ...
      try {
        ...
        const payload: CreateKpmrAspekHukumDto = {
        ...
        const newAspek = await kpmrHukumApiService.createAspek(cleanKpmrId, payload);
  ...
  const updateAspek = useCallback(
    async (id: number | string, aspekData: UpdateKpmrAspekHukumDto): Promise<FrontendAspekResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedAspek = await kpmrHukumApiService.updateAspek(cleanIdNum, aspekData);
  ...
  const deleteAspek = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrHukumApiService.deleteAspek(cleanIdNum);
  ...
  const addPertanyaan = useCallback(
    async (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanHukumDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const payload: CreateKpmrPertanyaanHukumDto = {
        ...
        const newPertanyaan = await kpmrHukumApiService.createPertanyaan(cleanAspekId, payload);
  ...
  const updatePertanyaan = useCallback(
    async (id: number | string, pertanyaanData: UpdateKpmrPertanyaanHukumDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrHukumApiService.updatePertanyaan(cleanIdNum, pertanyaanData);
  ...
  const deletePertanyaan = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrHukumApiService.deletePertanyaan(cleanIdNum);
  ...
  const updateSkor = useCallback(
    async (id: number | string, quarter: string, skor: number): Promise<FrontendPertanyaanResponse> => {
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrHukumApiService.updateSkor(cleanIdNum, updateSkorDto);
