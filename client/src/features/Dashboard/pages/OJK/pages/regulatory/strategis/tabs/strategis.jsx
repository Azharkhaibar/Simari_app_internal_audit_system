// src/ojk/strategis-produk/strategis-produk-ojk/strategis.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import StrategisProdukInherentWrapper from './strategis-inherent';
import StrategisProdukKpmrPage from './strategis-kpmr';
...
export default function StrategisOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useStrategisProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrStrategis();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [StrategisOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [StrategisOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [StrategisOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [StrategisOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [StrategisOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [StrategisOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [StrategisOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [StrategisOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [StrategisOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const handleKpmrRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [StrategisOJK] Error refreshing KPMR:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    return <StrategisProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Strategis Produk" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && (
          <StrategisProdukInherentWrapper
            rows={inherentRows}
            setRows={setInherentRows}
            search={search}
            active
