// src/ojk/permodalan-produk/permodalan-produk-ojk/permodalan.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import PermodalanProdukInherentWrapper from './permodalan-inherent';
import PermodalanProdukKpmrPage from './permodalan-kpmr';
...
export default function PermodalanOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = usePermodalanProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrPermodalan();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [PermodalanOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [PermodalanOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [PermodalanOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [PermodalanOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [PermodalanOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [PermodalanOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [PermodalanOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [PermodalanOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [PermodalanOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const handleKpmrRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [PermodalanOJK] Error refreshing KPMR:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    return <PermodalanProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Permodalan Produk" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && (
          <PermodalanProdukInherentWrapper
            rows={inherentRows}
            setRows={setInherentRows}
            search={search}
            active
