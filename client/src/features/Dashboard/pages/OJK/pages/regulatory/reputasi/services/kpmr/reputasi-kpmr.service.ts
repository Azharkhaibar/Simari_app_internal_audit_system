...
// services/kpmr/reputasi-produk-kpmr.service.ts
...
// ========== CREATE DTOs ==========
export interface CreateKpmrReputasiOjkDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
  ...
  notes?: string;
  summary?: any;
  aspekList?: CreateKpmrAspekReputasiDto[];
}

export interface CreateKpmrAspekReputasiDto {
  nomor?: string;
  judul: string;
  bobot: number;
  deskripsi?: string;
  kpmrOjkId?: number;
  orderIndex?: number;
  pertanyaanList?: CreateKpmrPertanyaanReputasiDto[];
}

export interface CreateKpmrPertanyaanReputasiDto {
  nomor?: string;
  pertanyaan: string;
  skor?: KpmrSkorDto;
...
// ========== UPDATE DTOs ==========
export interface UpdateKpmrAspekReputasiDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  deskripsi?: string;
...
export interface UpdateKpmrPertanyaanReputasiDto {
  nomor?: string;
  pertanyaan?: string;
  skor?: KpmrSkorDto;
  indicator?: KpmrIndicatorDto;
...
class KpmrReputasiApiService {
  private readonly BASE_PATH = '/kpmr-reputasi';
  ...
  private handleApiError(error: any, defaultMessage: string): never {
    console.error(`❌ [KPMR Reputasi Service] Error:`, error);
  ...
  async getKpmrByYearQuarter(year: number, quarter: number, withRelations: boolean = true): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_reputasi_produk.get(`${url}${params}`);
  ...
  async getKpmrWithRelations(id: number | string): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_reputasi_produk.get(`${this.BASE_PATH}/${cleanId}?withRelations=true`);
      return response.data;
    } catch (error: any) {
  ...
  async createAspek(kpmrId: number | string, data: CreateKpmrAspekReputasiDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_reputasi_produk.post(`${this.BASE_PATH}/${cleanKpmrId}/aspek`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updateAspek(id: number | string, data: UpdateKpmrAspekReputasiDto): Promise<FrontendAspekResponse> {
    try {
      const cleanId = this.cleanId(id);
      const response = await api_reputasi_produk.patch(`${this.BASE_PATH}/aspek/${cleanId}`, data);
      return response.data;
    } catch (error: any) {
  ...
  async deleteAspek(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_reputasi_produk.delete(`${this.BASE_PATH}/aspek/${cleanId}`);
    } catch (error: any) {
  ...
  async createPertanyaan(aspekId: number | string, data: CreateKpmrPertanyaanReputasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_reputasi_produk.post(`${this.BASE_PATH}/aspek/${cleanAspekId}/pertanyaan`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updatePertanyaan(id: number | string, data: UpdateKpmrPertanyaanReputasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanId = this.cleanId(id);
      ...
      console.log('[KPMR Reputasi Service] Update pertanyaan payload:', JSON.stringify(payload, null, 2));

      const response = await api_reputasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async deletePertanyaan(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_reputasi_produk.delete(`${this.BASE_PATH}/pertanyaan/${cleanId}`);
    } catch (error: any) {
  ...
  async createKpmr(data: CreateKpmrReputasiOjkDto): Promise<FrontendKpmrResponse> {
    try {
      console.log(`🆕 [KPMR Reputasi Service] Creating KPMR:`, data);
      ...
      console.log('📦 [KPMR Reputasi Service] Sending payload:', payload);

      const response = await api_reputasi_produk.post(this.BASE_PATH, payload);

      if (!response || !response.data) {
        throw new Error('Response dari server tidak valid');
      }

      console.log('✅ [KPMR Reputasi Service] Response:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('❌ [KPMR Reputasi Service] Create error:', {
  ...
  async updateSkor(id: number | string, data: UpdateSkorDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_reputasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}/skor`, data);
      return response.data;
    } catch (error: any) {
...
export default kpmrReputasiApiService;