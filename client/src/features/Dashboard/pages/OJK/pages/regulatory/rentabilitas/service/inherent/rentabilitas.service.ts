// services/rentabilitas.service.ts
import api_rentabilitas_produk from '../rentabilitas-api.service';

// =============================================
// TYPES BERDASARKAN BACKEND ENTITY
// =============================================

export interface RentabilitasProdukOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
  parameters?: RentabilitasParameterEntity[];
  summary?: {
...
export interface RentabilitasParameterEntity {
  id: number;
  nomor?: string;
  judul: string;
  bobot: number;
  ...
  nilaiList?: RentabilitasNilaiEntity[];
  orderIndex: number;
...
export interface RentabilitasNilaiEntity {
  id: number;
  nomor?: string;
  judul: {
    type?: string;
...
export interface CreateRentabilitasProdukInherentDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateRentabilitasProdukInherentDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class RentabilitasProdukService {
  private baseUrl = '/rentabilitas';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: RentabilitasProdukOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
  ...
  async ensureDataExists(year: number, quarter: number): Promise<RentabilitasProdukOjkEntity> {
  ...
  async loadOrCreateData(year: number, quarter: number): Promise<RentabilitasProdukOjkEntity> {
  ...
  public formatToFrontend(entity: RentabilitasProdukOjkEntity | null): any[] {
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[RentabilitasProdukService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<RentabilitasProdukOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_rentabilitas_produk.get<RentabilitasProdukOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<RentabilitasProdukOjkEntity | null> {
    ...
    try {
      const response = await api_rentabilitas_produk.get<any>(url, { params });
  ...
  async getAll(): Promise<RentabilitasProdukOjkEntity[]> {
    const url = this.baseUrl;
    this.logApiCall('GET', url);

    try {
      const response = await api_rentabilitas_produk.get<any>(url);
  ...
  async getById(id: number): Promise<RentabilitasProdukOjkEntity> {
  ...
  async create(createDto: CreateRentabilitasProdukInherentDto): Promise<RentabilitasProdukOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasProdukOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateRentabilitasProdukInherentDto): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasProdukOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasProdukOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateRentabilitasProdukInherentDto['summary']): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasProdukOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async remove(id: number): Promise<{ message: string; id: number }> {
    ...
    try {
      const response = await api_rentabilitas_produk.delete<{ message: string; id: number }>(url);
      return response.data;
    } catch (error: any) {
  ...
  async addParameter(inherentId: number, dto: CreateParameterDto): Promise<RentabilitasProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_rentabilitas_produk.post<RentabilitasProdukOjkEntity>(url, payload);
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateParameterDto): Promise<RentabilitasProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_rentabilitas_produk.put<RentabilitasProdukOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(inherentId: number, parameterId: number): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(inherentId: number, parameterId: number): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_rentabilitas_produk.delete<RentabilitasProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(inherentId: number, parameterIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_rentabilitas_produk.put<{ message: string }>(url, { parameterIds });
      return response.data;
    } catch (error: any) {
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateNilaiDto): Promise<RentabilitasProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<RentabilitasProdukOjkEntity> {
  ...
  async copyNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<RentabilitasProdukOjkEntity> {
  ...
  async removeNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(inherentId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_rentabilitas_produk.put<{ message: string }>(url, { nilaiIds });
      return response.data;
    } catch (error: any) {
  ...
  async importFromExcel(importData: any): Promise<RentabilitasProdukOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasProdukOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async getReferences(type?: string): Promise<ReferenceItem[]> {
    ...
    try {
      const response = await api_rentabilitas_produk.get<ReferenceItem[]>(url, { params });
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: RentabilitasProdukOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<RentabilitasProdukOjkEntity> {
    ...
    try {
      ...
      const createDto: CreateRentabilitasProdukInherentDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
      ...
      const fallbackData: RentabilitasProdukOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: RentabilitasProdukOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_rentabilitas_produk.get(this.baseUrl, { timeout: 5000 });
...
export const rentabilitasProdukService = new RentabilitasProdukService();
export default rentabilitasProdukService;
