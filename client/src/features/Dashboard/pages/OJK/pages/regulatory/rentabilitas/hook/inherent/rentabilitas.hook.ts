// src/ojk/rentabilitas-produk/hook/inherent/rentabilitas.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import rentabilitasProdukService, { CreateNilaiDto, CreateParameterDto, RentabilitasProdukOjkEntity, UpdateNilaiDto, UpdateParameterDto } from '../../service/inherent/rentabilitas.service';

export const useRentabilitasProdukIntegration = (initialYear?: number, initialQuarter?: number) => {
  ...
  const [currentInherentData, setCurrentInherentData] = useState<RentabilitasProdukOjkEntity | null>(null);
  ...
  const cacheRef = useRef<
    Map<
      string,
      {
        ...
        entity: RentabilitasProdukOjkEntity | null;
        timestamp: number;
      }
    >
  >(new Map());
  ...
  const getOrCreateData = useCallback(async (targetYear: number, targetQuarter: number, forceReload = false) => {
    ...
    try {
      ...
      const result = await rentabilitasProdukService.findOrCreate(targetYear, targetQuarter);
      ...
      const formattedRows = rentabilitasProdukService.formatToFrontend(result.data);
  ...
  const updateCache = useCallback(
    (y: number, q: number, data: { rows: any[]; inherentId: number | null; entity: RentabilitasProdukOjkEntity | null }) => {
  ...
  const formatParameterJudul = useCallback(
    (judul: any): string => {
      ...
      return rentabilitasProdukService.formatParameterJudul(judul);
    },
    [validateParameterJudul],
  );

  const formatNilaiJudul = useCallback((judul: any): CreateNilaiDto['judul'] => {
    return rentabilitasProdukService.formatNilaiJudul(judul);
  }, []);
  ...
  const handleAddParameter = useCallback(
    async (dto: Partial<CreateParameterDto>) => {
      ...
      try {
        ...
        await rentabilitasProdukService.loadOrCreateData(context.year, context.quarter);

        // Tambahkan parameter
        await rentabilitasProdukService.addParameter(context.inherentId, finalPayload);
    ...
    async (parameterId: string, dto: UpdateParameterDto) => {
      ...
      try {
        ...
        if (dto.judul !== undefined) {
          ...
          payload.judul = rentabilitasProdukService.formatParameterJudul(dto.judul);
        }
        ...
        await rentabilitasProdukService.updateParameter(context.inherentId, parameterIdNum, payload);
  ...
  const handleCopyParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await rentabilitasProdukService.copyParameter(context.inherentId, parameterIdNum);
  ...
  const handleDeleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await rentabilitasProdukService.removeParameter(context.inherentId, parameterIdNum);
  ...
  const handleAddNilai = useCallback(
    async (parameterId: string, dto: CreateNilaiDto) => {
      ...
      try {
        ...
        // Tambahkan nilai
        await rentabilitasProdukService.addNilai(context.inherentId, parameterIdNum, payload);
  ...
  const handleUpdateNilai = useCallback(
    async (parameterId: string, nilaiId: string, dto: UpdateNilaiDto) => {
      ...
      try {
        ...
        await rentabilitasProdukService.updateNilai(context.inherentId, parameterIdNum, nilaiIdNum, payload);
  ...
  const handleCopyNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await rentabilitasProdukService.copyNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleDeleteNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await rentabilitasProdukService.removeNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleReorderParameters = useCallback(
    async (parameterIds: string[]) => {
      ...
      try {
        ...
        await rentabilitasProdukService.reorderParameters(context.inherentId, parameterIdsNum);
  ...
  const handleReorderNilai = useCallback(
    async (parameterId: string, nilaiIds: string[]) => {
      ...
      try {
        ...
        await rentabilitasProdukService.reorderNilai(context.inherentId, parameterIdNum, nilaiIdsNum);
...
export default useRentabilitasProdukIntegration;
