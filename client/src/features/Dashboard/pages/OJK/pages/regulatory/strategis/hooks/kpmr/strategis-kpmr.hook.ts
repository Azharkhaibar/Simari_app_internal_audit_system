// strategis-kpmr.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import { useToast } from '../../components/use-toast';
import kpmrStrategisApiService, {
...
} from '../../services/kpmr/strategis-kpmr.service';

interface UseKpmrStrategisReturn {
  // State
  ...
  // Aspek operations
  addAspek: (kpmrId: number | string, aspekData: CreateKpmrAspekStrategisDto) => Promise<FrontendAspekResponse>;
  updateAspek: (id: number | string, aspekData: UpdateKpmrAspekStrategisDto) => Promise<FrontendAspekResponse>;
  deleteAspek: (id: number | string) => Promise<void>;

  // Pertanyaan operations
  addPertanyaan: (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanStrategisDto) => Promise<FrontendPertanyaanResponse>;
  updatePertanyaan: (id: number | string, pertanyaanData: UpdateKpmrPertanyaanStrategisDto) => Promise<FrontendPertanyaanResponse>;
...
export function useKpmrStrategis(): UseKpmrStrategisReturn {
  ...
  const cleanId = useCallback((id: string | number): number => {
    return kpmrStrategisApiService.cleanId(id);
  }, []);
  ...
  const loadKpmrByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      ...
      try {
        const data = await kpmrStrategisApiService.getKpmrByYearQuarter(targetYear, targetQuarter, true);

        if (!mountedRef.current) return;

        safeSet(setKpmr, data);
        safeSet(setCurrentKpmrId, data.id);
        const frontendRows = kpmrStrategisApiService.convertToFrontendFormat(data);
  ...
  const createKpmr = useCallback(
    async (year: number, quarter: number): Promise<FrontendKpmrResponse | null> => {
      ...
      try {
        ...
        const payload: CreateKpmrStrategisOjkDto = {
          year: yearNum,
        ...
        const data = await kpmrStrategisApiService.createKpmr(payload);
  ...
  const refreshKpmrData = useCallback(async (): Promise<FrontendAspekResponse[]> => {
    ...
    try {
      ...
      const freshData = await kpmrStrategisApiService.getKpmrWithRelations(cleanIdNum);

      if (!mountedRef.current) return [];

      safeSet(setKpmr, freshData);
      const frontendRows = kpmrStrategisApiService.convertToFrontendFormat(freshData);
      safeSet(setRows, frontendRows);
  ...
  const addAspek = useCallback(
    async (kpmrId: number | string, aspekData: CreateKpmrAspekStrategisDto): Promise<FrontendAspekResponse> => {
      ...
      try {
        ...
        const payload: CreateKpmrAspekStrategisDto = {
        ...
        const newAspek = await kpmrStrategisApiService.createAspek(cleanKpmrId, payload);
  ...
  const updateAspek = useCallback(
    async (id: number | string, aspekData: UpdateKpmrAspekStrategisDto): Promise<FrontendAspekResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedAspek = await kpmrStrategisApiService.updateAspek(cleanIdNum, aspekData);
  ...
  const deleteAspek = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrStrategisApiService.deleteAspek(cleanIdNum);
  ...
  const addPertanyaan = useCallback(
    async (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanStrategisDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const newPertanyaan = await kpmrStrategisApiService.createPertanyaan(cleanAspekId, payload);
  ...
  const updatePertanyaan = useCallback(
    async (id: number | string, pertanyaanData: UpdateKpmrPertanyaanStrategisDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrStrategisApiService.updatePertanyaan(cleanIdNum, pertanyaanData);
  ...
  const deletePertanyaan = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrStrategisApiService.deletePertanyaan(cleanIdNum);
  ...
  const updateSkor = useCallback(
    async (id: number | string, quarter: string, skor: number): Promise<FrontendPertanyaanResponse> => {
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrStrategisApiService.updateSkor(cleanIdNum, updateSkorDto);
...
export default useKpmrStrategis;