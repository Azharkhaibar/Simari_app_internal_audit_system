// src/ojk/operasional-produk/operasional-produk-ojk/operasional.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import OperasionalProdukInherentWrapper from './operasional-inherent';
import OperasionalProdukKpmrPage from './operasional-kpmr';
...
export default function OperasionalOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useOperasionalProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrOperasional();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [OperasionalOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [OperasionalOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [OperasionalOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [OperasionalOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [OperasionalOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [OperasionalOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [OperasionalOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [OperasionalOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [OperasionalOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const handleKpmrRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [OperasionalOJK] Error refreshing KPMR:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    return <OperasionalProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Operasional Produk" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && (
          <OperasionalProdukInherentWrapper
            rows={inherentRows}
            setRows={setInherentRows}
            search={search}
            active
