// src/ojk/hukum/hukum-ojk/hukum-inherent.jsx
...
export default function HukumInherentWrapper() {
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      isLoading,
      currentHukumId,
      currentHukumData: currentHukumData ? `ID: ${currentHukumData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, rows, isLoading, currentHukumId, currentHukumData, error]);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Hukum - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentHukumId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentHukumId).substring(0, 8)}...</span>}
            </p>
            {currentHukumData?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(currentHukumData.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {currentHukumData?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{currentHukumData.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <HukumInherent rows={rows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={isLoading} isLocked={currentHukumData?.isLocked} />
    </div>
  );
}

// Komponen utama - PERBAIKAN: Tambah prop isInitialLoading
function HukumInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Hukum - Inherent</h1>
