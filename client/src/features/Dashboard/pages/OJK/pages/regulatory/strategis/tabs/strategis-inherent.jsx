// src/ojk/strategis-produk/strategis-produk-ojk/strategis-produk-inherent.jsx
...
import useStrategisProdukIntegration from '../hooks/inherent/strategis.hook';
...
export default function StrategisProdukInherentWrapper() {
  const { user } = useAuth();
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useStrategisProdukIntegration(year, quarter);
    ...
    () => ({
      addParameter: async (createParamDto) => {
        try {
          ...
          // AUDIT LOG: CREATE Parameter Strategis Produk Inherent
          await logCreate('STRATEGIS_OJK', `Menambah parameter baru - ${createParamDto.judul} (Bobot: ${createParamDto.bobot}%)`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      updateParameter: async (parameterId, updateParamDto) => {
        try {
          ...
          // AUDIT LOG: UPDATE Parameter Strategis Produk Inherent
          await logUpdate('STRATEGIS_OJK', `Mengupdate parameter ID: ${parameterId} - ${updateParamDto.judul}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      deleteParameter: async (parameterId) => {
        try {
          ...
          // AUDIT LOG: DELETE Parameter Strategis Produk Inherent
          await logDelete('STRATEGIS_OJK', `Menghapus parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      addNilai: async (parameterId, createNilaiDto) => {
        try {
          ...
          // AUDIT LOG: CREATE Nilai Strategis Produk Inherent
          await logCreate('STRATEGIS_OJK', `Menambah nilai baru pada parameter ID: ${parameterId} - ${createNilaiDto.judul?.text}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      updateNilai: async (parameterId, nilaiId, updateNilaiDto) => {
        try {
          ...
          // AUDIT LOG: UPDATE Nilai Strategis Produk Inherent
          await logUpdate('STRATEGIS_OJK', `Mengupdate nilai ID: ${nilaiId} pada parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      deleteNilai: async (parameterId, nilaiId) => {
        try {
          ...
          // AUDIT LOG: DELETE Nilai Strategis Produk Inherent
          await logDelete('STRATEGIS_OJK', `Menghapus nilai ID: ${nilaiId} dari parameter ID: ${parameterId}`, {
            userId: user?.id || null,
            isSuccess: true,
            metadata: { type: 'inherent', year, quarter, parameterId, nilaiId }
          });
      ...
      copyParameter: async (parameterId) => {
        try {
          ...
          // AUDIT LOG: COPY Parameter Strategis Produk Inherent
          await logCreate('STRATEGIS_OJK', `Menyalin parameter ID: ${parameterId} menjadi parameter baru`, {
            userId: user?.id || null,
            isSuccess: true,
      ...
      copyNilai: async (parameterId, nilaiId) => {
        try {
          ...
          // AUDIT LOG: COPY Nilai Strategis Produk Inherent
          await logCreate('STRATEGIS_OJK', `Menyalin nilai ID: ${nilaiId} pada parameter ID: ${parameterId} menjadi nilai baru`, {
            userId: user?.id || null,
            isSuccess: true,
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Strategis Produk - Inherent Risk</h1>
      ...
      {/* Komponen utama */}
      <StrategisProdukInherent rows={rows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={isLoading} isLocked={currentInherentData?.isLocked} />
    </div>
  );
}

// Komponen utama - PERBAIKAN: Tambah prop isInitialLoading
function StrategisProdukInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Strategis Produk - Inherent</h1>
