// services/tatakelola.service.ts
import api_tatakelola_produk from '../tatakelola-api.service';

// =============================================
// TYPES BERDASARKAN BACKEND ENTITY
// =============================================

export interface TatakelolaProdukOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
  parameters?: TatakelolaParameterEntity[];
  summary?: {
...
export interface TatakelolaParameterEntity {
  id: number;
  nomor?: string;
  judul: string;
  bobot: number;
  ...
  nilaiList?: TatakelolaNilaiEntity[];
  orderIndex: number;
  tatakelolaProdukOjkId: number;
  createdAt: Date;
  updatedAt: Date;
}

export interface TatakelolaNilaiEntity {
  id: number;
  nomor?: string;
  judul: {
    type?: string;
...
export interface CreateTatakelolaProdukInherentDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateTatakelolaProdukInherentDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class TatakelolaProdukService {
  private baseUrl = '/tatakelola';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: TatakelolaProdukOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
  ...
  async ensureDataExists(year: number, quarter: number): Promise<TatakelolaProdukOjkEntity> {
  ...
  async loadOrCreateData(year: number, quarter: number): Promise<TatakelolaProdukOjkEntity> {
  ...
  public formatToFrontend(entity: TatakelolaProdukOjkEntity | null): any[] {
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[TatakelolaProdukService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<TatakelolaProdukOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_tatakelola_produk.get<TatakelolaProdukOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<TatakelolaProdukOjkEntity | null> {
    ...
    try {
      const response = await api_tatakelola_produk.get<any>(url, { params });
  ...
  async getAll(): Promise<TatakelolaProdukOjkEntity[]> {
    const url = this.baseUrl;
    this.logApiCall('GET', url);

    try {
      const response = await api_tatakelola_produk.get<any>(url);
  ...
  async getById(id: number): Promise<TatakelolaProdukOjkEntity> {
  ...
  async create(createDto: CreateTatakelolaProdukInherentDto): Promise<TatakelolaProdukOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_tatakelola_produk.post<TatakelolaProdukOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateTatakelolaProdukInherentDto): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_tatakelola_produk.put<TatakelolaProdukOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_tatakelola_produk.put<TatakelolaProdukOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateTatakelolaProdukInherentDto['summary']): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_tatakelola_produk.put<TatakelolaProdukOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async remove(id: number): Promise<{ message: string; id: number }> {
    ...
    try {
      const response = await api_tatakelola_produk.delete<{ message: string; id: number }>(url);
      return response.data;
    } catch (error: any) {
  ...
  async addParameter(inherentId: number, dto: CreateParameterDto): Promise<TatakelolaProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_tatakelola_produk.post<TatakelolaProdukOjkEntity>(url, payload);
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateParameterDto): Promise<TatakelolaProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_tatakelola_produk.put<TatakelolaProdukOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(inherentId: number, parameterId: number): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_tatakelola_produk.post<TatakelolaProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(inherentId: number, parameterId: number): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_tatakelola_produk.delete<TatakelolaProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(inherentId: number, parameterIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_tatakelola_produk.put<{ message: string }>(url, { parameterIds });
      return response.data;
    } catch (error: any) {
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateNilaiDto): Promise<TatakelolaProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<TatakelolaProdukOjkEntity> {
  ...
  async copyNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<TatakelolaProdukOjkEntity> {
  ...
  async removeNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(inherentId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_tatakelola_produk.put<{ message: string }>(url, { nilaiIds });
      return response.data;
    } catch (error: any) {
  ...
  async importFromExcel(importData: any): Promise<TatakelolaProdukOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_tatakelola_produk.post<TatakelolaProdukOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async getReferences(type?: string): Promise<ReferenceItem[]> {
    ...
    try {
      const response = await api_tatakelola_produk.get<ReferenceItem[]>(url, { params });
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: TatakelolaProdukOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<TatakelolaProdukOjkEntity> {
    ...
    try {
      ...
      const createDto: CreateTatakelolaProdukInherentDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
      ...
      const fallbackData: TatakelolaProdukOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: TatakelolaProdukOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_tatakelola_produk.get(this.baseUrl, { timeout: 5000 });
...
export const tatakelolaProdukService = new TatakelolaProdukService();
export default tatakelolaProdukService;
