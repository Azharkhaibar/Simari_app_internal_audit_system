// src/ojk/konsentrasi-produk/hook/inherent/konsentrasi-produk.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import konsentrasiProdukService, { CreateNilaiDto, CreateParameterDto, KonsentrasiProdukOjkEntity, UpdateNilaiDto, UpdateParameterDto } from '../../service/inherent/konsentrasi-produk.service';

export const useKonsentrasiProdukIntegration = (initialYear?: number, initialQuarter?: number) => {
  ...
  const [currentInherentData, setCurrentInherentData] = useState<KonsentrasiProdukOjkEntity | null>(null);
  ...
  const cacheRef = useRef<
    Map<
      string,
      {
        ...
        entity: KonsentrasiProdukOjkEntity | null;
        timestamp: number;
      }
    >
  >(new Map());
  ...
  const getOrCreateData = useCallback(async (targetYear: number, targetQuarter: number, forceReload = false) => {
    ...
    try {
      ...
      const result = await konsentrasiProdukService.findOrCreate(targetYear, targetQuarter);
      ...
      const formattedRows = konsentrasiProdukService.formatToFrontend(result.data);
  ...
  const updateCache = useCallback(
    (y: number, q: number, data: { rows: any[]; inherentId: number | null; entity: KonsentrasiProdukOjkEntity | null }) => {
  ...
  const formatParameterJudul = useCallback(
    (judul: any): string => {
      ...
      return konsentrasiProdukService.formatParameterJudul(judul);
    },
    [validateParameterJudul],
  );

  const formatNilaiJudul = useCallback((judul: any): CreateNilaiDto['judul'] => {
    return konsentrasiProdukService.formatNilaiJudul(judul);
  }, []);
  ...
  const handleAddParameter = useCallback(
    async (dto: Partial<CreateParameterDto>) => {
      ...
      try {
        ...
        await konsentrasiProdukService.loadOrCreateData(context.year, context.quarter);

        // Tambahkan parameter
        await konsentrasiProdukService.addParameter(context.inherentId, finalPayload);
    ...
    async (parameterId: string, dto: UpdateParameterDto) => {
      ...
      try {
        ...
        if (dto.judul !== undefined) {
          ...
          payload.judul = konsentrasiProdukService.formatParameterJudul(dto.judul);
        }
        ...
        await konsentrasiProdukService.updateParameter(context.inherentId, parameterIdNum, payload);
  ...
  const handleCopyParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await konsentrasiProdukService.copyParameter(context.inherentId, parameterIdNum);
  ...
  const handleDeleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await konsentrasiProdukService.removeParameter(context.inherentId, parameterIdNum);
  ...
  const handleAddNilai = useCallback(
    async (parameterId: string, dto: CreateNilaiDto) => {
      ...
      try {
        ...
        // Tambahkan nilai
        await konsentrasiProdukService.addNilai(context.inherentId, parameterIdNum, payload);
  ...
  const handleUpdateNilai = useCallback(
    async (parameterId: string, nilaiId: string, dto: UpdateNilaiDto) => {
      ...
      try {
        ...
        await konsentrasiProdukService.updateNilai(context.inherentId, parameterIdNum, nilaiIdNum, payload);
  ...
  const handleCopyNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await konsentrasiProdukService.copyNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleDeleteNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await konsentrasiProdukService.removeNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleReorderParameters = useCallback(
    async (parameterIds: string[]) => {
      ...
      try {
        ...
        await konsentrasiProdukService.reorderParameters(context.inherentId, parameterIdsNum);
  ...
  const handleReorderNilai = useCallback(
    async (parameterId: string, nilaiIds: string[]) => {
      ...
      try {
        ...
        await konsentrasiProdukService.reorderNilai(context.inherentId, parameterIdNum, nilaiIdsNum);
...
export default useKonsentrasiProdukIntegration;