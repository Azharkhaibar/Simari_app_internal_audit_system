...
export interface LikuiditasProdukOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
  parameters?: LikuiditasParameterEntity[];
  summary?: {
...
export interface LikuiditasParameterEntity {
  id: number;
  nomor?: string;
  judul: string;
  bobot: number;
  ...
  nilaiList?: LikuiditasNilaiEntity[];
  orderIndex: number;
...
export interface LikuiditasNilaiEntity {
  id: number;
  nomor?: string;
  judul: {
    type?: string;
...
export interface CreateLikuiditasProdukInherentDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateLikuiditasProdukInherentDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class LikuiditasProdukService {
  private baseUrl = '/likuiditas';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: LikuiditasProdukOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
  ...
  async ensureDataExists(year: number, quarter: number): Promise<LikuiditasProdukOjkEntity> {
  ...
  async loadOrCreateData(year: number, quarter: number): Promise<LikuiditasProdukOjkEntity> {
  ...
  public formatToFrontend(entity: LikuiditasProdukOjkEntity | null): any[] {
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[LikuiditasProdukService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<LikuiditasProdukOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_likuiditas_produk.get<LikuiditasProdukOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<LikuiditasProdukOjkEntity | null> {
    ...
    try {
      const response = await api_likuiditas_produk.get<any>(url, { params });
  ...
  async getAll(): Promise<LikuiditasProdukOjkEntity[]> {
    const url = this.baseUrl;
    this.logApiCall('GET', url);

    try {
      const response = await api_likuiditas_produk.get<any>(url);
  ...
  async getById(id: number): Promise<LikuiditasProdukOjkEntity> {
  ...
  async create(createDto: CreateLikuiditasProdukInherentDto): Promise<LikuiditasProdukOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_likuiditas_produk.post<LikuiditasProdukOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateLikuiditasProdukInherentDto): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_likuiditas_produk.put<LikuiditasProdukOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_likuiditas_produk.put<LikuiditasProdukOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateLikuiditasProdukInherentDto['summary']): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_likuiditas_produk.put<LikuiditasProdukOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async remove(id: number): Promise<{ message: string; id: number }> {
    ...
    try {
      const response = await api_likuiditas_produk.delete<{ message: string; id: number }>(url);
      return response.data;
    } catch (error: any) {
  ...
  async addParameter(inherentId: number, dto: CreateParameterDto): Promise<LikuiditasProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_likuiditas_produk.post<LikuiditasProdukOjkEntity>(url, payload);
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateParameterDto): Promise<LikuiditasProdukOjkEntity> {
    ...
    try {
      ...
      const response = await api_likuiditas_produk.put<LikuiditasProdukOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(inherentId: number, parameterId: number): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_likuiditas_produk.post<LikuiditasProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(inherentId: number, parameterId: number): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_likuiditas_produk.delete<LikuiditasProdukOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(inherentId: number, parameterIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_likuiditas_produk.put<{ message: string }>(url, { parameterIds });
      return response.data;
    } catch (error: any) {
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateNilaiDto): Promise<LikuiditasProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<LikuiditasProdukOjkEntity> {
  ...
  async copyNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<LikuiditasProdukOjkEntity> {
  ...
  async removeNilai(inherentId: number, parameterId: number, nilaiId: number): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(inherentId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    ...
    try {
      const response = await api_likuiditas_produk.put<{ message: string }>(url, { nilaiIds });
      return response.data;
    } catch (error: any) {
  ...
  async importFromExcel(importData: any): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_likuiditas_produk.post<LikuiditasProdukOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async getReferences(type?: string): Promise<ReferenceItem[]> {
    ...
    try {
      const response = await api_likuiditas_produk.get<ReferenceItem[]>(url, { params });
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: LikuiditasProdukOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<LikuiditasProdukOjkEntity> {
    ...
    try {
      ...
      const createDto: CreateLikuiditasProdukInherentDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
      ...
      const fallbackData: LikuiditasProdukOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: LikuiditasProdukOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_likuiditas_produk.get(this.baseUrl, { timeout: 5000 });
...
export const likuiditasProdukService = new LikuiditasProdukService();
export default likuiditasProdukService;