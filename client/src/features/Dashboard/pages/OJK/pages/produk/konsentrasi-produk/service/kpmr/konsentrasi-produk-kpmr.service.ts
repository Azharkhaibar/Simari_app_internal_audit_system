...
// ========== CREATE DTOs ==========
export interface CreateKpmrKonsentrasiOjkDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface CreateKpmrAspekKonsentrasiDto {
  nomor?: string;
  judul: string;
  bobot: number;
  deskripsi?: string;
...
export interface CreateKpmrPertanyaanKonsentrasiDto {
  nomor?: string;
  pertanyaan: string;
  skor?: KpmrSkorDto;
...
// ========== UPDATE DTOs ==========
export interface UpdateKpmrAspekKonsentrasiDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  deskripsi?: string;
...
export interface UpdateKpmrPertanyaanKonsentrasiDto {
  nomor?: string;
  pertanyaan?: string;
  skor?: KpmrSkorDto;
  indicator?: KpmrIndicatorDto;
...
// ========== SERVICE CLASS ==========
class KpmrKonsentrasiApiService {
  private readonly BASE_PATH = '/kpmr-konsentrasi';
  ...
  private handleApiError(error: any, defaultMessage: string): never {
    console.error(`❌ [KPMR Konsentrasi Service] Error:`, error);
  ...
  async getKpmrByYearQuarter(year: number, quarter: number, withRelations: boolean = true): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_konsentrasi_produk.get(`${url}${params}`);
  ...
  async getKpmrWithRelations(id: number | string): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_konsentrasi_produk.get(`${this.BASE_PATH}/${cleanId}?withRelations=true`);
      return response.data;
    } catch (error: any) {
  ...
  async createAspek(kpmrId: number | string, data: CreateKpmrAspekKonsentrasiDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_konsentrasi_produk.post(`${this.BASE_PATH}/${cleanKpmrId}/aspek`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updateAspek(id: number | string, data: UpdateKpmrAspekKonsentrasiDto): Promise<FrontendAspekResponse> {
    try {
      const cleanId = this.cleanId(id);
      const response = await api_konsentrasi_produk.patch(`${this.BASE_PATH}/aspek/${cleanId}`, data);
      return response.data;
    } catch (error: any) {
  ...
  async deleteAspek(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_konsentrasi_produk.delete(`${this.BASE_PATH}/aspek/${cleanId}`);
    } catch (error: any) {
  ...
  async createPertanyaan(aspekId: number | string, data: CreateKpmrPertanyaanKonsentrasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_konsentrasi_produk.post(`${this.BASE_PATH}/aspek/${cleanAspekId}/pertanyaan`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updatePertanyaan(id: number | string, data: UpdateKpmrPertanyaanKonsentrasiDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanId = this.cleanId(id);
      ...
      console.log('[KPMR Konsentrasi Service] Update pertanyaan payload:', JSON.stringify(payload, null, 2));

      const response = await api_konsentrasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async deletePertanyaan(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_konsentrasi_produk.delete(`${this.BASE_PATH}/pertanyaan/${cleanId}`);
    } catch (error: any) {
  ...
  async createKpmr(data: CreateKpmrKonsentrasiOjkDto): Promise<FrontendKpmrResponse> {
    try {
      console.log(`🆕 [KPMR Konsentrasi Service] Creating KPMR:`, data);
      ...
      console.log('📦 [KPMR Konsentrasi Service] Sending payload:', payload);

      const response = await api_konsentrasi_produk.post(this.BASE_PATH, payload);

      if (!response || !response.data) {
        throw new Error('Response dari server tidak valid');
      }

      console.log('✅ [KPMR Konsentrasi Service] Response:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('❌ [KPMR Konsentrasi Service] Create error:', {
  ...
  async updateSkor(id: number | string, data: UpdateSkorDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_konsentrasi_produk.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}/skor`, data);
      return response.data;
    } catch (error: any) {
...
export const kpmrKonsentrasiApiService = new KpmrKonsentrasiApiService();
export default kpmrKonsentrasiApiService;