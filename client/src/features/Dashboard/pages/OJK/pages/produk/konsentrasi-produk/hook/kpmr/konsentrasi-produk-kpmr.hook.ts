// konsentrasi-kpmr.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import { useToast } from '../../components/use-toast';
import kpmrKonsentrasiApiService, {
  FrontendKpmrResponse,
  FrontendAspekResponse,
  FrontendPertanyaanResponse,
  CreateKpmrAspekKonsentrasiDto,
  CreateKpmrPertanyaanKonsentrasiDto,
  UpdateKpmrAspekKonsentrasiDto,
  UpdateKpmrPertanyaanKonsentrasiDto,
  UpdateSkorDto,
  CreateKpmrKonsentrasiOjkDto,
} from '../../service/kpmr/konsentrasi-produk-kpmr.service';

interface UseKpmrKonsentrasiReturn {
  // State
  ...
  // Aspek operations
  addAspek: (kpmrId: number | string, aspekData: CreateKpmrAspekKonsentrasiDto) => Promise<FrontendAspekResponse>;
  updateAspek: (id: number | string, aspekData: UpdateKpmrAspekKonsentrasiDto) => Promise<FrontendAspekResponse>;
  deleteAspek: (id: number | string) => Promise<void>;

  // Pertanyaan operations
  addPertanyaan: (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanKonsentrasiDto) => Promise<FrontendPertanyaanResponse>;
  updatePertanyaan: (id: number | string, pertanyaanData: UpdateKpmrPertanyaanKonsentrasiDto) => Promise<FrontendPertanyaanResponse>;
...
export function useKpmrKonsentrasi(): UseKpmrKonsentrasiReturn {
  ...
  const cleanId = useCallback((id: string | number): number => {
    return kpmrKonsentrasiApiService.cleanId(id);
  }, []);
  ...
  const loadKpmrByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      ...
      try {
        const data = await kpmrKonsentrasiApiService.getKpmrByYearQuarter(targetYear, targetQuarter, true);

        if (!mountedRef.current) return;

        safeSet(setKpmr, data);
        safeSet(setCurrentKpmrId, data.id);
        const frontendRows = kpmrKonsentrasiApiService.convertToFrontendFormat(data);
  ...
  const createKpmr = useCallback(
    async (year: number, quarter: number): Promise<FrontendKpmrResponse | null> => {
      ...
      try {
        ...
        const payload: CreateKpmrKonsentrasiOjkDto = {
          year: yearNum,
        ...
        const data = await kpmrKonsentrasiApiService.createKpmr(payload);
  ...
  const refreshKpmrData = useCallback(async (): Promise<FrontendAspekResponse[]> => {
    ...
    try {
      ...
      const freshData = await kpmrKonsentrasiApiService.getKpmrWithRelations(cleanIdNum);

      if (!mountedRef.current) return [];

      safeSet(setKpmr, freshData);
      const frontendRows = kpmrKonsentrasiApiService.convertToFrontendFormat(freshData);
      safeSet(setRows, frontendRows);
  ...
  const addAspek = useCallback(
    async (kpmrId: number | string, aspekData: CreateKpmrAspekKonsentrasiDto): Promise<FrontendAspekResponse> => {
      ...
      try {
        ...
        const payload: CreateKpmrAspekKonsentrasiDto = {
        ...
        const newAspek = await kpmrKonsentrasiApiService.createAspek(cleanKpmrId, payload);
  ...
  const updateAspek = useCallback(
    async (id: number | string, aspekData: UpdateKpmrAspekKonsentrasiDto): Promise<FrontendAspekResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedAspek = await kpmrKonsentrasiApiService.updateAspek(cleanIdNum, aspekData);
  ...
  const deleteAspek = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrKonsentrasiApiService.deleteAspek(cleanIdNum);
  ...
  const addPertanyaan = useCallback(
    async (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanKonsentrasiDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const payload: CreateKpmrPertanyaanKonsentrasiDto = {
        ...
        const newPertanyaan = await kpmrKonsentrasiApiService.createPertanyaan(cleanAspekId, payload);
  ...
  const updatePertanyaan = useCallback(
    async (id: number | string, pertanyaanData: UpdateKpmrPertanyaanKonsentrasiDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrKonsentrasiApiService.updatePertanyaan(cleanIdNum, pertanyaanData);
  ...
  const deletePertanyaan = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrKonsentrasiApiService.deletePertanyaan(cleanIdNum);
  ...
  const updateSkor = useCallback(
    async (id: number | string, quarter: string, skor: number): Promise<FrontendPertanyaanResponse> => {
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrKonsentrasiApiService.updateSkor(cleanIdNum, updateSkorDto);
...
export default useKpmrKonsentrasi;