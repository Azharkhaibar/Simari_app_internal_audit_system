// src/ojk/likuiditas-produk/likuiditas-produk-ojk/likuiditas.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import LikuiditasProdukInherentWrapper from './likuiditas-produk-inherent';
import LikuiditasProdukKpmrPage from './likuiditas-produk-kpmr';
...
import useKpmrLikuiditas from '../hook/kpmr/likuiditas-produk-kpmr';
...
export default function LikuiditasOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useLikuiditasProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrLikuiditas();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [LikuiditasOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [LikuiditasOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [LikuiditasOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    if (kpmrHookError) {
      ...
      if (!is404) {
        setKpmrLoadError(kpmrHookError || 'Gagal memuat KPMR');
      } else {
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [LikuiditasOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [LikuiditasOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [LikuiditasOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [LikuiditasOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [LikuiditasOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [LikuiditasOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const handleKpmrRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [LikuiditasOJK] Error refreshing KPMR:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    return <LikuiditasProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Likuiditas Produk" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && (
          <LikuiditasProdukInherentWrapper
            rows={inherentRows}
            setRows={setInherentRows}
            search={search}
            active
