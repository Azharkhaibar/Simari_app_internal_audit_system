// src/ojk/kredit-produk/kredit-produk-ojk/kredit-produk.jsx
import React, { useEffect, useState, useRef, useCallback, useMemo } from 'react';
import Header from '../../../../components/ui/header';
import RiskTabs from '../../../../components/ui/risk-tabs';
import KreditProdukInherentWrapper from './kredit-produk-inherent';
import KreditProdukKpmrPage from './kredit-produk-kpmr';
import { useHeaderStore } from '../../../../store/header';
import { exportInherent } from '../../../../utils/export/export-inherent';
import { exportKpmr } from '../../../../utils/export/export-kpmr';
import useKreditProdukIntegration from '../hook/inherent/kredit-produk.hook';
import useKpmrKreditProduk from '../hook/kpmr/kredit-produk-kpmr.hook';
...
export default function KreditProdukOJK() {
  ...
  const {
    ...
    formatBobot,
    formatKategori,
  } = useKreditProdukIntegration(year, activeQuarter);

  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr } = useKpmrKreditProduk();
  ...
  useEffect(() => {
    ...
    const fetchInherent = async () => {
      console.log(`📡 [KreditProdukOJK] Loading inherent for ${year}-Q${activeQuarter}`);

      try {
        await loadData(year, activeQuarter, false);

        if (!cancelled) {
          console.log(`✅ [KreditProdukOJK] Inherent loaded for ${year}-Q${activeQuarter}`);
        }
      } catch (err) {
        if (!cancelled) {
          console.error('❌ [KreditProdukOJK] Error loading inherent:', err);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadKpmrData = async () => {
      ...
      try {
        ...
        console.log(`📡 [KreditProdukOJK] Loading KPMR for ${year}-Q${targetQuarter}`);
        ...
        if (data) {
          console.log(`✅ [KreditProdukOJK] KPMR loaded: ID ${data.id}`);
          setKpmrId(data.id);
          setKpmrRows(formatKpmrRowsFromBackend(data.aspekList || []));
        } else {
          console.log(`ℹ️ [KreditProdukOJK] KPMR not found`);
          setKpmrId(null);
          setKpmrRows([]);
        }
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [KreditProdukOJK] Creating new KPMR for year ${year}`);
      const targetQuarter = 1;
      const data = await createKpmr(year, targetQuarter);

      if (data) {
        console.log(`✅ [KreditProdukOJK] KPMR created: ID ${data.id}`);
        setKpmrDataLoaded(true);
        setKpmrLoadError(null);
        setLoadedYear(year);
        setKpmrId(data.id);
      ...
      console.error('❌ [KreditProdukOJK] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  const kpmrPage = useMemo(() => {
    return <KreditProdukKpmrPage key={`kpmr-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
