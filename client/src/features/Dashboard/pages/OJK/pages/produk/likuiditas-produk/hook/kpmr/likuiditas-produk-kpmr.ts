...
import kpmrLikuiditasApiService, {
  ...
  CreateKpmrAspekLikuiditasDto,
  CreateKpmrPertanyaanLikuiditasDto,
  UpdateKpmrAspekLikuiditasDto,
  UpdateKpmrPertanyaanLikuiditasDto,
  UpdateSkorDto,
  CreateKpmrLikuiditasOjkDto,
} from '../../service/kpmr/likuiditas-produk-kpmr.service';

interface UseKpmrLikuiditasReturn {
  // State
  ...
  // Aspek operations
  addAspek: (kpmrId: number | string, aspekData: CreateKpmrAspekLikuiditasDto) => Promise<FrontendAspekResponse>;
  updateAspek: (id: number | string, aspekData: UpdateKpmrAspekLikuiditasDto) => Promise<FrontendAspekResponse>;
  deleteAspek: (id: number | string) => Promise<void>;

  // Pertanyaan operations
  addPertanyaan: (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanLikuiditasDto) => Promise<FrontendPertanyaanResponse>;
  updatePertanyaan: (id: number | string, pertanyaanData: UpdateKpmrPertanyaanLikuiditasDto) => Promise<FrontendPertanyaanResponse>;
...
export function useKpmrLikuiditasProduk(): UseKpmrLikuiditasReturn {
  ...
  const cleanId = useCallback((id: string | number): number => {
    return kpmrLikuiditasApiService.cleanId(id);
  }, []);
  ...
  const loadKpmrByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      ...
      try {
        const data = await kpmrLikuiditasApiService.getKpmrByYearQuarter(targetYear, targetQuarter, true);
  ...
  const refreshKpmrData = useCallback(async (): Promise<FrontendAspekResponse[]> => {
    ...
    try {
      ...
      const freshData = await kpmrLikuiditasApiService.getKpmrWithRelations(cleanIdNum);

      if (!mountedRef.current) return [];

      safeSet(setKpmr, freshData);
      const frontendRows = kpmrLikuiditasApiService.convertToFrontendFormat(freshData);
      safeSet(setRows, frontendRows);
  ...
  const addAspek = useCallback(
    async (kpmrId: number | string, aspekData: CreateKpmrAspekLikuiditasDto): Promise<FrontendAspekResponse> => {
      ...
      try {
        ...
        const payload: CreateKpmrAspekLikuiditasDto = {
        ...
        const newAspek = await kpmrLikuiditasApiService.createAspek(cleanKpmrId, payload);
  ...
  const updateAspek = useCallback(
    async (id: number | string, aspekData: UpdateKpmrAspekLikuiditasDto): Promise<FrontendAspekResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedAspek = await kpmrLikuiditasApiService.updateAspek(cleanIdNum, aspekData);
  ...
  const deleteAspek = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrLikuiditasApiService.deleteAspek(cleanIdNum);
  ...
  const addPertanyaan = useCallback(
    async (aspekId: number | string, pertanyaanData: CreateKpmrPertanyaanLikuiditasDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const newPertanyaan = await kpmrLikuiditasApiService.createPertanyaan(cleanAspekId, payload);
  ...
  const updatePertanyaan = useCallback(
    async (id: number | string, pertanyaanData: UpdateKpmrPertanyaanLikuiditasDto): Promise<FrontendPertanyaanResponse> => {
      if (loadingRef.current) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrLikuiditasApiService.updatePertanyaan(cleanIdNum, pertanyaanData);
  ...
  const deletePertanyaan = useCallback(
    async (id: number | string): Promise<void> => {
      ...
      try {
        const cleanIdNum = cleanId(id);
        await kpmrLikuiditasApiService.deletePertanyaan(cleanIdNum);
  ...
  const updateSkor = useCallback(
    async (id: number | string, quarter: string, skor: number): Promise<FrontendPertanyaanResponse> => {
      ...
      try {
        ...
        const updatedPertanyaan = await kpmrLikuiditasApiService.updateSkor(cleanIdNum, updateSkorDto);
