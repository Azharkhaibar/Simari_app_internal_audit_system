// src/ojk/pasar-produk/hook/inherent/pasar-produk.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import pasarProdukService, { CreateNilaiDto, CreateParameterDto, PasarProdukOjkEntity, UpdateNilaiDto, UpdateParameterDto } from '../../service/inherent/pasar-produk.service';

export const usePasarProdukIntegration = (initialYear?: number, initialQuarter?: number) => {
  ...
  const [currentInherentData, setCurrentInherentData] = useState<PasarProdukOjkEntity | null>(null);
  ...
  const cacheRef = useRef<
    Map<
      string,
      {
        ...
        entity: PasarProdukOjkEntity | null;
        timestamp: number;
      }
    >
  >(new Map());
  ...
  const getOrCreateData = useCallback(async (targetYear: number, targetQuarter: number, forceReload = false) => {
    ...
    try {
      ...
      const result = await pasarProdukService.findOrCreate(targetYear, targetQuarter);
      ...
      const formattedRows = pasarProdukService.formatToFrontend(result.data);
  ...
  const updateCache = useCallback(
    (y: number, q: number, data: { rows: any[]; inherentId: number | null; entity: PasarProdukOjkEntity | null }) => {
  ...
  const formatParameterJudul = useCallback(
    (judul: any): string => {
      ...
      return pasarProdukService.formatParameterJudul(judul);
    },
    [validateParameterJudul],
  );

  const formatNilaiJudul = useCallback((judul: any): CreateNilaiDto['judul'] => {
    return pasarProdukService.formatNilaiJudul(judul);
  }, []);
  ...
  const handleAddParameter = useCallback(
    async (dto: Partial<CreateParameterDto>) => {
      ...
      try {
        ...
        await pasarProdukService.loadOrCreateData(context.year, context.quarter);

        // Tambahkan parameter
        await pasarProdukService.addParameter(context.inherentId, finalPayload);
    ...
    async (parameterId: string, dto: UpdateParameterDto) => {
      ...
      try {
        ...
        if (dto.judul !== undefined) {
          ...
          payload.judul = pasarProdukService.formatParameterJudul(dto.judul);
        }
        ...
        await pasarProdukService.updateParameter(context.inherentId, parameterIdNum, payload);
  ...
  const handleCopyParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await pasarProdukService.copyParameter(context.inherentId, parameterIdNum);
  ...
  const handleDeleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      try {
        ...
        await pasarProdukService.removeParameter(context.inherentId, parameterIdNum);
  ...
  const handleAddNilai = useCallback(
    async (parameterId: string, dto: CreateNilaiDto) => {
      ...
      try {
        ...
        // Tambahkan nilai
        await pasarProdukService.addNilai(context.inherentId, parameterIdNum, payload);
  ...
  const handleUpdateNilai = useCallback(
    async (parameterId: string, nilaiId: string, dto: UpdateNilaiDto) => {
      ...
      try {
        ...
        await pasarProdukService.updateNilai(context.inherentId, parameterIdNum, nilaiIdNum, payload);
  ...
  const handleCopyNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await pasarProdukService.copyNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleDeleteNilai = useCallback(
    async (parameterId: string, nilaiId: string) => {
      ...
      try {
        ...
        await pasarProdukService.removeNilai(context.inherentId, parameterIdNum, nilaiIdNum);
  ...
  const handleReorderParameters = useCallback(
    async (parameterIds: string[]) => {
      ...
      try {
        ...
        await pasarProdukService.reorderParameters(context.inherentId, parameterIdsNum);
  ...
  const handleReorderNilai = useCallback(
    async (parameterId: string, nilaiIds: string[]) => {
      ...
      try {
        ...
        await pasarProdukService.reorderNilai(context.inherentId, parameterIdNum, nilaiIdsNum);
...
export default usePasarProdukIntegration;